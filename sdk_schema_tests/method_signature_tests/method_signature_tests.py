...
class TestGenerativeModelMethodSignatures(unittest.TestCase):
    ...
    def test_vertex_ai_generate_content_method_signature(self):
        self._test_method_argument_key_in_both_sdks(
            method_under_test=VertexAIGenerativeModel.generate_content,
            expected_method_arg_keys=common_contract.expected_generate_content_common_arg_keys,
            sdk_name=_VERTEX_AI_SDK_NAME,
        )

    def test_google_ai_generate_content_method_signature(self):
        self._test_method_argument_key_in_both_sdks(
