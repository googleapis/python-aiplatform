...
def test_search_migratable_resources_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v1/{parent=projects/*/locations/*}/migratableResources:search" % client.transport._host, args[1])
...
def test_batch_migrate_resources_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{parent=projects/*/locations/*}/migratableResources:batchMigrate" % client.transport._host, args[1])
