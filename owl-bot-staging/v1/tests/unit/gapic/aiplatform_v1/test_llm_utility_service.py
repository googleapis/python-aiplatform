...
def test_count_tokens_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(()) & set(("endpoint", "model", "instances", "contents", )))
...
def test_count_tokens_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        req.return_value._content = prediction_service.CountTokensResponse.to_json(prediction_service.CountTokensResponse())
...
def test_count_tokens_rest_bad_request(transport: str = 'rest', request_type=prediction_service.CountTokensRequest):
    client = LlmUtilityServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
