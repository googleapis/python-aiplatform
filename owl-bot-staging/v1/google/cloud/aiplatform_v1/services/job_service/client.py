...
class JobServiceClient(metaclass=JobServiceClientMeta):
    ...
    @staticmethod
    def nas_job_path(project: str,location: str,nas_job: str,) -> str:
        """Returns a fully-qualified nas_job string."""
        return "projects/{project}/locations/{location}/nasJobs/{nas_job}".format(project=project, location=location, nas_job=nas_job, )

    @staticmethod
    def parse_nas_job_path(path: str) -> Dict[str,str]:
        """Parses a nas_job path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/nasJobs/(?P<nas_job>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def nas_trial_detail_path(project: str,location: str,nas_job: str,nas_trial_detail: str,) -> str:
        """Returns a fully-qualified nas_trial_detail string."""
        return "projects/{project}/locations/{location}/nasJobs/{nas_job}/nasTrialDetails/{nas_trial_detail}".format(project=project, location=location, nas_job=nas_job, nas_trial_detail=nas_trial_detail, )

    @staticmethod
    def parse_nas_trial_detail_path(path: str) -> Dict[str,str]:
        """Parses a nas_trial_detail path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/nasJobs/(?P<nas_job>.+?)/nasTrialDetails/(?P<nas_trial_detail>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def __init__(self, *,
        ...
        self._universe_domain = get_universe_domain(universe_domain_opt, self._universe_domain_env, default_universe=JobServiceClient._DEFAULT_UNIVERSE)
    ...
    def list_nas_trial_details(self,
