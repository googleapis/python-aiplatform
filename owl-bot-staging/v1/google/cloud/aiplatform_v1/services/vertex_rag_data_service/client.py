...
class VertexRagDataServiceClientMeta(type):
    ...
    objects.
    """
    _transport_registry = OrderedDict()  # type: Dict[str, Type[VertexRagDataServiceTransport]]
...
class VertexRagDataServiceClient(metaclass=VertexRagDataServiceClientMeta):
    ...
    @staticmethod
    def rag_engine_config_path(project: str,location: str,) -> str:
        """Returns a fully-qualified rag_engine_config string."""
        return "projects/{project}/locations/{location}/ragEngineConfig".format(project=project, location=location, )

    @staticmethod
    def parse_rag_engine_config_path(path: str) -> Dict[str,str]:
        """Parses a rag_engine_config path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/ragEngineConfig$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def rag_file_path(project: str,location: str,rag_corpus: str,rag_file: str,) -> str:
        """Returns a fully-qualified rag_file string."""
        return "projects/{project}/locations/{location}/ragCorpora/{rag_corpus}/ragFiles/{rag_file}".format(project=project, location=location, rag_corpus=rag_corpus, rag_file=rag_file, )

    @staticmethod
    def parse_rag_file_path(path: str) -> Dict[str,str]:
        """Parses a rag_file path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/ragCorpora/(?P<rag_corpus>.+?)/ragFiles/(?P<rag_file>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, VertexRagDataServiceTransport, Callable[..., VertexRagDataServiceTransport]]] = None,
        ...
        self._client_cert_source = VertexRagDataServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = get_universe_domain(universe_domain_opt, self._universe_domain_env, default_universe=VertexRagDataServiceClient._DEFAULT_UNIVERSE)
        ...
        self._api_endpoint = (self._api_endpoint or
            get_api_endpoint(
                ...
                default_endpoint_template=VertexRagDataServiceClient._DEFAULT_ENDPOINT_TEMPLATE,
        ...
        if not transport_provided:
            transport_init: Union[Type[VertexRagDataServiceTransport], Callable[..., VertexRagDataServiceTransport]] = (
