...
class SessionServiceClient(metaclass=SessionServiceClientMeta):
    ...
    @staticmethod
    def session_event_path(project: str,location: str,reasoning_engine: str,session: str,event: str,) -> str:
        """Returns a fully-qualified session_event string."""
        return "projects/{project}/locations/{location}/reasoningEngines/{reasoning_engine}/sessions/{session}/events/{event}".format(project=project, location=location, reasoning_engine=reasoning_engine, session=session, event=event, )

    @staticmethod
    def parse_session_event_path(path: str) -> Dict[str,str]:
        """Parses a session_event path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/reasoningEngines/(?P<reasoning_engine>.+?)/sessions/(?P<session>.+?)/events/(?P<event>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def __init__(self, *,
        ...
        self._universe_domain = get_universe_domain(universe_domain_opt, self._universe_domain_env, default_universe=SessionServiceClient._DEFAULT_UNIVERSE)
