...
async def test_list_deployment_resource_pools_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_deployment_resource_pools(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
async def test_query_deployed_models_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.query_deployed_models(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
