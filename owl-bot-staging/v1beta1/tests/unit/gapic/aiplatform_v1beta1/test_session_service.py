...
def test_create_session_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v1beta1/{parent=projects/*/locations/*}/sessions" % client.transport._host, args[1])
...
def test_get_session_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta1/{name=projects/*/locations/*/sessions/*}" % client.transport._host, args[1])
...
def test_update_session_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {'session': {'name': 'projects/sample1/locations/sample2/sessions/sample3'}}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta1/{session.name=projects/*/locations/*/sessions/*}" % client.transport._host, args[1])
...
def test_create_session_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request_init["session"] = {'name': 'name_value', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'display_name': 'display_name_value', 'session_state': {'fields': {}}, 'user_id': 'user_id_value'}
...
def test_update_session_rest_bad_request(request_type=session_service.UpdateSessionRequest):
    ...
    # send a request that will satisfy transcoding
    request_init = {'session': {'name': 'projects/sample1/locations/sample2/sessions/sample3'}}
    request = request_type(**request_init)
...
def test_update_session_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'session': {'name': 'projects/sample1/locations/sample2/sessions/sample3'}}
    request_init["session"] = {'name': 'projects/sample1/locations/sample2/sessions/sample3', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'display_name': 'display_name_value', 'session_state': {'fields': {}}, 'user_id': 'user_id_value'}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748
...
def test_append_event_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'name': 'projects/sample1/locations/sample2/reasoningEngines/sample3/sessions/sample4'}
    request_init["event"] = {'name': 'name_value', 'author': 'author_value', 'content': {'role': 'role_value', 'parts': [{'text': 'text_value', 'inline_data': {'mime_type': 'mime_type_value', 'data': b'data_blob'}, 'file_data': {'mime_type': 'mime_type_value', 'file_uri': 'file_uri_value'}, 'function_call': {'id': 'id_value', 'name': 'name_value', 'args': {'fields': {}}}, 'function_response': {'id': 'id_value', 'name': 'name_value', 'response': {}}, 'executable_code': {'language': 1, 'code': 'code_value'}, 'code_execution_result': {'outcome': 1, 'output': 'output_value'}, 'video_metadata': {'start_offset': {'seconds': 751, 'nanos': 543}, 'end_offset': {}}, 'thought': True}]}, 'invocation_id': 'invocation_id_value', 'actions': {'skip_summarization': True, 'state_delta': {}, 'artifact_delta': {}, 'transfer_to_agent': True, 'escalate': True, 'requested_auth_configs': {}}, 'timestamp': {'seconds': 751, 'nanos': 543}, 'error_code': 'error_code_value', 'error_message': 'error_message_value', 'event_metadata': {'grounding_metadata': {'web_search_queries': ['web_search_queries_value1', 'web_search_queries_value2'], 'search_entry_point': {'rendered_content': 'rendered_content_value', 'sdk_blob': b'sdk_blob_blob'}, 'retrieval_queries': ['retrieval_queries_value1', 'retrieval_queries_value2'], 'grounding_chunks': [{'web': {'uri': 'uri_value', 'title': 'title_value'}, 'retrieved_context': {'uri': 'uri_value', 'title': 'title_value', 'text': 'text_value'}}], 'grounding_supports': [{'segment': {'part_index': 1070, 'start_index': 1189, 'end_index': 942, 'text': 'text_value'}, 'grounding_chunk_indices': [2436, 2437], 'confidence_scores': [0.1789, 0.17900000000000002]}], 'retrieval_metadata': {'google_search_dynamic_retrieval_score': 0.3902}}, 'partial': True, 'turn_complete': True, 'interrupted': True, 'long_running_tool_ids': ['long_running_tool_ids_value1', 'long_running_tool_ids_value2'], 'branch': 'branch_value'}}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748
...
def test_session_event_path():
    ...
    session = "winkle"
    event = "nautilus"
    expected = "projects/{project}/locations/{location}/sessions/{session}/events/{event}".format(project=project, location=location, session=session, event=event, )
