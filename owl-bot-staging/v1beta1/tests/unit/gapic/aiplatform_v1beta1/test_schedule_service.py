...
def test_create_schedule(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
        ...
        call.return_value = gca_schedule.Schedule(
            ...
            catch_up=True,
            cron='cron_value',
            create_pipeline_job_request=pipeline_service.CreatePipelineJobRequest(parent='parent_value'),
        )
...
async def test_list_schedules_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_schedules(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
