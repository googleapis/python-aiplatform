...
def test__get_default_mtls_endpoint():
    ...
    assert MigrationServiceClient._get_default_mtls_endpoint(custom_endpoint) == custom_endpoint
...
async def test_search_migratable_resources_async(request_type, transport: str = 'grpc_asyncio'):
    client = MigrationServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
async def test_batch_migrate_resources_async(request_type, transport: str = 'grpc_asyncio'):
    client = MigrationServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_search_migratable_resources_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.MigrationServiceRestInterceptor, "post_search_migratable_resources_with_metadata") as post_with_metadata, \
        ...
        post_with_metadata.return_value = migration_service.SearchMigratableResourcesResponse(), metadata
...
def test_batch_migrate_resources_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.MigrationServiceRestInterceptor, "post_batch_migrate_resources_with_metadata") as post_with_metadata, \
...
async def test_search_migratable_resources_rest_asyncio_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.AsyncMigrationServiceRestInterceptor, "post_search_migratable_resources_with_metadata") as post_with_metadata, \
...
async def test_batch_migrate_resources_rest_asyncio_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.AsyncMigrationServiceRestInterceptor, "post_batch_migrate_resources_with_metadata") as post_with_metadata, \
...
def test_auto_ml_dataset_path():
    project = "cuttlefish"
    location = "mussel"
    dataset = "winkle"
    expected = "projects/{project}/locations/{location}/datasets/{dataset}".format(project=project, location=location, dataset=dataset, )
    actual = MigrationServiceClient.auto_ml_dataset_path(project, location, dataset)
    assert expected == actual


def test_parse_auto_ml_dataset_path():
    expected = {
        "project": "nautilus",
        "location": "scallop",
        "dataset": "abalone",
    }
    path = MigrationServiceClient.auto_ml_dataset_path(**expected)

    # Check that the path construction is reversible.
    actual = MigrationServiceClient.parse_auto_ml_dataset_path(path)
    assert expected == actual

def test_auto_ml_model_path():
    project = "squid"
    location = "clam"
    model = "whelk"
    expected = "projects/{project}/locations/{location}/models/{model}".format(project=project, location=location, model=model, )
    actual = MigrationServiceClient.auto_ml_model_path(project, location, model)
    assert expected == actual


def test_parse_auto_ml_model_path():
    expected = {
        "project": "octopus",
        "location": "oyster",
        "model": "nudibranch",
    }
    path = MigrationServiceClient.auto_ml_model_path(**expected)

    # Check that the path construction is reversible.
    actual = MigrationServiceClient.parse_auto_ml_model_path(path)
    assert expected == actual

def test_data_labeling_dataset_path():
    project = "cuttlefish"
    dataset = "mussel"
    expected = "projects/{project}/datasets/{dataset}".format(project=project, dataset=dataset, )
    actual = MigrationServiceClient.data_labeling_dataset_path(project, dataset)
    assert expected == actual


def test_parse_data_labeling_dataset_path():
    expected = {
        "project": "winkle",
        "dataset": "nautilus",
    }
    path = MigrationServiceClient.data_labeling_dataset_path(**expected)

    # Check that the path construction is reversible.
    actual = MigrationServiceClient.parse_data_labeling_dataset_path(path)
    assert expected == actual
