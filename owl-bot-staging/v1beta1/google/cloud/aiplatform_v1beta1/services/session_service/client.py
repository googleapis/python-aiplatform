...
class SessionServiceClient(metaclass=SessionServiceClientMeta):
    ...
    @staticmethod
    def session_event_path(project: str,location: str,session: str,event: str,) -> str:
        """Returns a fully-qualified session_event string."""
        return "projects/{project}/locations/{location}/sessions/{session}/events/{event}".format(project=project, location=location, session=session, event=event, )

    @staticmethod
    def parse_session_event_path(path: str) -> Dict[str,str]:
        """Parses a session_event path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/sessions/(?P<session>.+?)/events/(?P<event>.+?)$", path)
        return m.groupdict() if m else {}
