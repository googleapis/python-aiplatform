...
"""Mocks for getting an existing Dataset, i.e.
...
def mock_get_time_series_dataset(mock_time_series_dataset):
  with patch.object(aiplatform,
                    "TimeSeriesDataset") as mock_get_time_series_dataset:
...
"""Mocks for creating a new Dataset, i.e.
...
def mock_create_image_dataset(mock_image_dataset):
  with patch.object(aiplatform.ImageDataset,
                    "create") as mock_create_image_dataset:
...
def mock_create_tabular_dataset(mock_tabular_dataset):
  with patch.object(aiplatform.TabularDataset,
                    "create") as mock_create_tabular_dataset:
...
def mock_create_time_series_dataset(mock_time_series_dataset):
  with patch.object(aiplatform.TimeSeriesDataset,
                    "create") as mock_create_time_series_dataset:
...
def mock_create_text_dataset(mock_text_dataset):
  with patch.object(aiplatform.TextDataset,
                    "create") as mock_create_text_dataset:
...
def mock_create_video_dataset(mock_video_dataset):
  with patch.object(aiplatform.VideoDataset,
                    "create") as mock_create_video_dataset:
...
def mock_get_featurestore(mock_featurestore):
  with patch.object(aiplatform.featurestore,
                    "Featurestore") as mock_get_featurestore:
...
def mock_get_entity_type(mock_entity_type):
  with patch.object(aiplatform.featurestore,
                    "EntityType") as mock_get_entity_type:
...
def mock_create_featurestore(mock_featurestore):
  with patch.object(aiplatform.featurestore.Featurestore,
                    "create") as mock_create_featurestore:
...
def mock_create_entity_type(mock_entity_type):
  with patch.object(aiplatform.featurestore.EntityType,
                    "create") as mock_create_entity_type:
...
def mock_create_feature(mock_feature):
  with patch.object(aiplatform.featurestore.Feature,
                    "create") as mock_create_feature:
...
def mock_batch_serve_to_bq(mock_featurestore):
  with patch.object(mock_featurestore,
                    "batch_serve_to_bq") as mock_batch_serve_to_bq:
    yield mock_batch_serve_to_bq


@pytest.fixture
def mock_batch_create_features(mock_entity_type):
  with patch.object(mock_entity_type,
                    "batch_create_features") as mock_batch_create_features:
...
def mock_import_feature_values(mock_entity_type):
  with patch.object(mock_entity_type,
                    "ingest_from_gcs") as mock_import_feature_values:
...
def mock_get_time_series_metrics(mock_time_series_metrics, mock_experiment_run):
  with patch.object(
      mock_experiment_run,
      "get_time_series_data_frame") as mock_get_time_series_metrics:
