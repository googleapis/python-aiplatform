...
class TestLanggraphAgent:
    ...
    def test_initialization(self):
        agent = langgraph.LanggraphAgent(model=_TEST_MODEL)
    ...
    def test_initialization_with_tools(self):
        ...
        agent = langgraph.LanggraphAgent(
            model=_TEST_MODEL,
            tools=tools,
    ...
    def test_enable_tracing(
        ...
        agent = langgraph.LanggraphAgent(
            model=_TEST_MODEL,
            enable_tracing=True,
            model_builder=lambda **kwargs: mock.Mock(),
            runnable_builder=lambda **kwargs: mock.Mock(),
        )
    ...
    def test_enable_tracing_warning(
        self,
        cloud_trace_exporter_mock,
        tracer_provider_mock,
        simple_span_processor_mock,
        langchain_instrumentor_none_mock,
    ):
        agent = langgraph.LanggraphAgent(
            model=_TEST_MODEL,
            enable_tracing=True,
...
class TestConvertToolsOrRaiseErrors:
    def test_raise_untyped_input_args(self, vertexai_init_mock):
        with pytest.raises(TypeError, match=r"has untyped input_arg"):
            langgraph.LanggraphAgent(model=_TEST_MODEL, tools=[_return_input_no_typing])
