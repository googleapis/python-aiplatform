...
class TestAG2Agent:
    ...
    def test_tracing_setup(
        ...
        autogen_instrumentor_class = (
            autogen_instrumentor_mock.return_value.AutogenInstrumentor
        )
        autogen_instrumentor_class.assert_called_once_with()
        autogen_instrumentor_class.return_value.uninstrument.assert_called_once_with()
        autogen_instrumentor_class.return_value.instrument.assert_called_once_with()
        assert (
            agent._tmpl_attrs.get("instrumentor")
            is autogen_instrumentor_class.return_value
        )
...
class TestToJsonSerializableAutoGenObject:
    ...
    def test_autogen_empty_run_response(self):
        ...
        mock_response.context_variables = None
        mock_response.last_speaker = None
