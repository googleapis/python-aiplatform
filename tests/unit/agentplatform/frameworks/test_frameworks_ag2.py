...
class TestAG2Agent:
    ...
    def test_initialization(self):
        agent = ag2.AG2Agent(model=_TEST_MODEL, runnable_name=_TEST_RUNNABLE_NAME)
...
class TestToJsonSerializableAutoGenObject:
    ...
    def test_autogen_chat_result(
        ...
        class DummyAutogenChatResult:
            pass

        test_obj = DummyAutogenChatResult()

        with mock.patch.object(
            _agent_engines_utils, "AutogenChatResult", new=DummyAutogenChatResult
        ):
            _agent_engines_utils.to_json_serializable_autogen_object(test_obj)

            dataclasses_is_dataclass_mock.assert_called_once_with(test_obj)
            dataclasses_asdict_mock.assert_called_once_with(test_obj)
...
class TestDataClassToJsonSerializable:
    """Tests for `_agent_engines_utils_agent_engines_utils._dataclass_to_dict_or_raise`."""
