...
def test_optimize_prompt(client):
    ...
    response = client.prompts.optimize(prompt=test_prompt)
...
def test_optimize_prompt_w_few_shot_optimization_target(client):
    ...
    if response.parsed_response:
        assert isinstance(response.parsed_response, types.prompts.ParsedResponse)
...
@pytest.mark.asyncio
async def test_async_optimize_prompt(client):
    """Tests the optimize request parameters method."""

    test_prompt = "Generate system instructions for analyzing medical articles"
    response = await client.aio.prompts.optimize(prompt=test_prompt)
    assert isinstance(response, types.OptimizeResponse)
    assert response.raw_text_response


@pytest.mark.asyncio
async def test_async_optimize_prompt_w_optimization_target(client):
    """Tests the optimize request parameters method with optimization target."""
    test_prompt = "Generate system instructions for analyzing medical articles"
    response = await client.aio.prompts.optimize(
        prompt=test_prompt,
        config=types.OptimizeConfig(
            optimization_target=types.OptimizeTarget.OPTIMIZATION_TARGET_GEMINI_NANO,
        ),
...
@pytest.mark.asyncio
async def test_async_optimize_prompt_w_few_shot_optimization_target(client):
    """Tests the optimize request parameters method with few shot optimization target."""
    test_prompt = "Generate system instructions for analyzing medical articles"
    df = pd.DataFrame(
        {
...
@pytest.mark.asyncio
async def test_async_optimize_prompt_w_few_shot_optimization_rubrics(client):
    """Tests the optimize request parameters method with few shot optimization target."""
    test_prompt = "Generate system instructions for analyzing medical articles"
    df = pd.DataFrame(
        {
