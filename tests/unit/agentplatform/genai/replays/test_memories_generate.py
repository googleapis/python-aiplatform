...
def test_generate_and_rollback_memories(client):
    ...
    try:
        ...
        client.memory_banks.memories.generate(
            ...
            config=types.GenerateMemoriesConfig(revision_labels={"key": "value"}),
        )
        memories = list(
            client.memory_banks.memories.list(
                name=memory_bank.name,
        ...
        memory = client.memory_banks.memories.get(name=memories[0].name)
        ...
        memory_revisions = list(
            client.memory_banks.memories.revisions.list(name=memories[0].name)
        )
...
def test_direct_memories_source(client):
    memory_bank = client.memory_banks.create()
    try:
        client.memory_banks.memories.generate(
            name=memory_bank.name,
            ...
            config=types.GenerateMemoriesConfig(wait_for_completion=True),
        )
        assert (
            len(
                list(
...
def test_generate_with_metadata(client):
    memory_bank = client.memory_banks.create()
    try:
        metadata = {
        ...
        operation = client.memory_banks.memories.generate(
            ...
            config=types.GenerateMemoriesConfig(metadata=metadata),
        )
...
pytestmark = pytest_helper.setup(
    ...
    test_method="memory_banks.memories.generate",
)
...
async def test_generate_and_rollback_memories_async(client):
    memory_bank = client.memory_banks.create()
    try:
        await client.aio.memory_banks.memories.generate(
            name=memory_bank.name,
            scope={"user_id": "test-user-id"},
            direct_memories_source=types.GenerateMemoriesRequestDirectMemoriesSource(
                direct_memories=[
        ...
        memories_pager = await client.aio.memory_banks.memories.list(
            name=memory_bank.name
        )
        memory_list = [item async for item in memories_pager]
        assert len(memory_list) >= 1

        revisions_pager = await client.aio.memory_banks.memories.revisions.list(
        ...
        memory = await client.aio.memory_banks.memories.get(name=memory_list[0].name)
        assert memory.fact == "This is temporary"

        # Rollback to the revision with the original fact that was created by the
        # generation request.
        await client.aio.memory_banks.memories.rollback(
            name=memory_list[0].name,
            target_revision_id=revision_name.split("/")[-1],
        )
        memory = await client.aio.memory_banks.memories.get(name=memory_list[0].name)
...
def test_generate_with_allowed_topics(client):
    memory_bank = client.memory_banks.create()
    try:
        client.memory_banks.memories.generate(
            name=memory_bank.name,
                ...
                events=[
                    ...
                    types.GenerateMemoriesRequestDirectContentsSourceEvent(
                        content=genai_types.Content(
                            role="user",
                            parts=[
                                genai_types.Part(text="I like to write replay tests.")
                            ],
                        )
                    ),
                ]
