...
def test_delete_memory(client):
    ...
    try:
        memory_operation = client.memory_banks.memories.create(
            name=memory_bank.name,
            fact="memory_fact",
            scope={"user_id": "123"},
        )
        assert isinstance(memory_operation, types.MemoryOperation)
        delete_memory_operation = client.memory_banks.memories.delete(
            name=memory_operation.response.name,
        )
        assert isinstance(delete_memory_operation, types.DeleteMemoryOperation)
    finally:
...
pytestmark = pytest_helper.setup(
    ...
    test_method="memory_banks.memories.delete",
)
...
async def test_delete_memory_async(client):
    ...
    try:
        ...
        operation = await client.aio.memory_banks.memories.delete(name=memory.name)
        assert isinstance(operation, types.DeleteMemoryOperation)
