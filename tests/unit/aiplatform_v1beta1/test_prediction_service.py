...
def test_prediction_service_client_from_service_account_file():
    ...
    with mock.patch.object(service_account.Credentials, 'from_service_account_file') as factory:
        ...
        client = PredictionServiceClient.from_service_account_file("dummy/file/path.json")
        assert client._transport._credentials == creds

        client = PredictionServiceClient.from_service_account_json("dummy/file/path.json")
        assert client._transport._credentials == creds
...
def test_prediction_service_client_client_options():
    ...
    assert PredictionServiceClient.DEFAULT_OPTIONS.api_endpoint == 'aiplatform.googleapis.com'

    # Check that options can be customized.
    options = client_options.ClientOptions(api_endpoint="squid.clam.whelk")
    with mock.patch('google.cloud.aiplatform_v1beta1.services.prediction_service.PredictionServiceClient.get_transport_class') as gtc:
...
def test_predict(transport: str = 'grpc'):
    ...
    with mock.patch.object(
            type(client._transport.predict),
            '__call__') as call:
...
def test_explain(transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client._transport.explain),
            '__call__') as call:
        # Designate an appropriate return value for the call.
