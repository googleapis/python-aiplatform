...
_TEST_LOCAL_SCRIPT_FILE_PATH = os.path.join(_TEST_TEMPDIR,
...
def get_training_job_custom_mock():
  with patch.object(pipeline_service_client.PipelineServiceClient,
                    "get_training_pipeline") as get_training_job_custom_mock:
...
def get_training_job_tabular_mock():
  with patch.object(pipeline_service_client.PipelineServiceClient,
                    "get_training_pipeline") as get_training_job_tabular_mock:
...
class TestTrainingScriptPythonPackager:
  ...
  def test_packager_creates_and_copies_python_package(self):
    ...
    assert pathlib.Path(
        f"{tsp._ROOT_MODULE}-{tsp._SETUP_PY_VERSION}.tar.gz").is_file()
...
def make_training_pipeline(state, add_training_task_metadata=True):
  return gca_training_pipeline.TrainingPipeline(
      ...
      } if add_training_task_metadata else None,
  )
...
def mock_pipeline_service_get(make_call=make_training_pipeline):
  with mock.patch.object(pipeline_service_client.PipelineServiceClient,
...
def mock_pipeline_service_cancel():
  with mock.patch.object(
      pipeline_service_client.PipelineServiceClient,
      "cancel_training_pipeline") as mock_cancel_training_pipeline:
...
def mock_python_package_to_gcs():
  with mock.patch.object(source_utils._TrainingScriptPythonPackager,
                         "package_and_copy_to_gcs") as mock_package_to_copy_gcs:
...
class TestCustomTrainingJob:
  ...
  def test_run_call_pipeline_service_create_with_tabular_dataset(
    ...
    env = [
        gca_env_var.EnvVar(name=str(key), value=str(value)) for key, value in
        _TEST_MODEL_SERVING_CONTAINER_ENVIRONMENT_VARIABLES.items()
    ]
  ...
  def test_run_call_pipeline_service_create_with_scheduling(self, sync, caplog):
    ...
    assert (job._gca_resource.state ==
            gca_pipeline_state.PipelineState.PIPELINE_STATE_SUCCEEDED)
    assert job._gca_resource.training_task_inputs[
        "timeout"] == f"{_TEST_TIMEOUT}s"
    assert (
  ...
  def test_run_call_pipeline_service_create_distributed_training_with_reduction_server(
    ...
    true_worker_pool_spec = [
        ...
        },
        {
            ...
            "machine_spec": {
                "machine_type": _TEST_REDUCTION_SERVER_MACHINE_TYPE
            },
            "container_spec": {
                "image_uri": _TEST_REDUCTION_SERVER_CONTAINER_URI
            },
            "disk_spec": {
                "boot_disk_type": _TEST_BOOT_DISK_TYPE_DEFAULT,
                "boot_disk_size_gb": _TEST_BOOT_DISK_SIZE_GB_DEFAULT,
  ...
  def test_unique_supported_training_schemas(self):
    ...
    schemas = [
        schema for c in aiplatform.training_jobs._TrainingJob.__subclasses__()
  ...
  def test_run_pipeline_for_versioned_model(
    ...
    job_args = {
        ...
        "model_instance_schema_uri":
            _TEST_MODEL_INSTANCE_SCHEMA_URI,
        "model_parameters_schema_uri":
            _TEST_MODEL_PARAMETERS_SCHEMA_URI,
        "model_prediction_schema_uri":
            _TEST_MODEL_PREDICTION_SCHEMA_URI,
        "model_serving_container_command":
            _TEST_MODEL_SERVING_CONTAINER_COMMAND,
        "model_serving_container_args":
            _TEST_MODEL_SERVING_CONTAINER_ARGS,
        "model_serving_container_environment_variables":
            _TEST_MODEL_SERVING_CONTAINER_ENVIRONMENT_VARIABLES,
        "model_serving_container_ports":
            _TEST_MODEL_SERVING_CONTAINER_PORTS,
        "model_description":
            _TEST_MODEL_DESCRIPTION,
        "labels":
            _TEST_LABELS,
    }
    ...
    assert training_pipeline.parent_model == (
        _TEST_MODEL_NAME if parent else "")
    assert training_pipeline.model_to_upload.version_aliases == goal
    assert (training_pipeline.model_to_upload.version_description ==
