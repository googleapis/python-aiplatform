...
_MOCK_MODEL_EVAL_SLICE = gca_model_evaluation_slice.ModelEvaluationSlice.Slice(
    dimension="slice",
    value="0",
    slice_spec=gca_model_evaluation_slice.ModelEvaluationSlice.Slice.SliceSpec(
        configs={
            "test": gca_model_evaluation_slice.ModelEvaluationSlice.Slice.SliceSpec.SliceConfig(
                value=gca_model_evaluation_slice.ModelEvaluationSlice.Slice.SliceSpec.Value(
                    string_value="test"
                ),
            )
        }
...
@pytest.fixture
def mock_list_model_eval_slices():
    with mock.patch.object(
        model_service_client.ModelServiceClient, "list_model_evaluation_slices"
    ) as mock_get_model_eval_slice:
        mock_get_model_eval_slice.return_value = [
            gca_model_evaluation_slice.ModelEvaluationSlice(
                name="test-eval-slice-name",
                slice_=_MOCK_MODEL_EVAL_SLICE,
                metrics=struct_pb2.Value(),
            )
        ]
        yield mock_get_model_eval_slice
...
class TestModelEvaluationJob:
    ...
    def test_get_model_evaluation_slices(
        self,
        mock_pipeline_service_create,
        job_spec,
        mock_load_yaml_and_json,
    ...
    def test_get_model_evalution_slices(
        self,
        get_model_mock,
        mock_model_eval_get,
        mock_pipeline_service_create,
        ...
        sliced_metrics = eval_resource.get_model_evaluation_slices()

        assert "overall" in sliced_metrics
        assert "0" in sliced_metrics
