...
class TestEndToEnd:
  ...
  def test_dataset_create_to_model_predict(
    ...
    true_worker_pool_spec = {
        ...
        "disk_spec": {
            "boot_disk_type":
                test_training_jobs._TEST_BOOT_DISK_TYPE_DEFAULT,
            "boot_disk_size_gb":
                test_training_jobs._TEST_BOOT_DISK_SIZE_GB_DEFAULT,
        },
        "python_package_spec": {
            "executor_image_uri":
                test_training_jobs._TEST_TRAINING_CONTAINER_IMAGE,
            "python_module":
                test_training_jobs._TEST_MODULE_NAME,
            "package_uris": [
  ...
  def test_dataset_create_to_model_predict_with_pipeline_fail(
    ...
    with pytest.raises(RuntimeError):
      endpoint_deploy_return = created_endpoint.deploy(
          model_from_job, sync=sync)
      assert endpoint_deploy_return is None
      created_endpoint.wait()
    ...
    assert (job._gca_resource is
            mock_pipeline_service_create_and_get_with_fail[1].return_value)
