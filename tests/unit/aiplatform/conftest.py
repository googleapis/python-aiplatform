...
@pytest.fixture
def mock_pipeline_service_get(make_call=test_constants.make_training_pipeline):
    with mock.patch.object(
...
def predict_client_predict_mock():
    with mock.patch.object(
        ...
        predict_mock.return_value = prediction_service.PredictResponse(
            deployed_model_id=test_constants.ModelConstants._TEST_ID,
            model_version_id=test_constants.ModelConstants._TEST_VERSION_ID,
            model=test_constants.ModelConstants._TEST_MODEL_RESOURCE_NAME,
        )
