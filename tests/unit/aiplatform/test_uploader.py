...
def _create_mock_client():
  ...
  def create_experiment_response(
    ...
    tensorboard_experiment_id = ("{}/experiments/{}".format(
        parent, tensorboard_experiment_id)
                                 if parent else tensorboard_experiment_id)
  ...
  def create_run_response(
    ...
    tensorboard_run_id = ("{}/runs/{}".format(parent, tensorboard_run_id)
                          if parent else tensorboard_run_id)
    return tensorboard_run_type.TensorboardRun(name=tensorboard_run_id)

  def create_tensorboard_time_series(tensorboard_time_series=None, parent=None):  # pylint: disable=unused-argument
    name = ("{}/timeSeries/{}".format(parent,
                                      tensorboard_time_series.display_name)
            if parent else tensorboard_time_series.display_name)
...
def _create_scalar_request_sender(experiment_resource_id,
                                  api=_USE_DEFAULT,
...
class FileWriter(tf.compat.v1.summary.FileWriter):
  ...
  def add_test_tensor_summary(self,
                              tag,
                              tensor,
                              step=None,
                              value_metadata=None):
  ...
  def add_event(self, event):
    if isinstance(event, event_pb2.Event):
      ...
      if not isinstance(event, bytes):
        logging.error("Added TensorFlow event proto. "
  ...
  def add_summary(self, summary, global_step=None):
    if isinstance(summary, summary_pb2.Summary):
      ...
      if not isinstance(summary, bytes):
        logging.error("Added TensorFlow summary proto. "
                      "Please prefer TensorBoard copy of the proto")
  ...
  def add_session_log(self, session_log, global_step=None):
    if isinstance(session_log, event_pb2.SessionLog):
      ...
      if not isinstance(session_log, bytes):
        logging.error("Added TensorFlow session_log proto. "
                      "Please prefer TensorBoard copy of the proto")
...
class TensorboardUploaderTest(tf.test.TestCase):
  ...
  def test_start_uploading_scalars(self):
    ...
    mock_logdir_loader.get_run_events.side_effect = [
        {
            "run 1":
                _apply_compat(
                    [_scalar_event("1.1", 5.0),
                     _scalar_event("1.2", 5.0)]),
            "run 2":
                _apply_compat(
                    [_scalar_event("2.1", 5.0),
                     _scalar_event("2.2", 5.0)]),
        },
        {
            "run 3":
                _apply_compat(
                    [_scalar_event("3.1", 5.0),
                     _scalar_event("3.2", 5.0)]),
            "run 4":
                _apply_compat(
                    [_scalar_event("4.1", 5.0),
                     _scalar_event("4.2", 5.0)]),
            "run 5":
                _apply_compat(
                    [_scalar_event("5.1", 5.0),
                     _scalar_event("5.2", 5.0)]),
        },
        AbortUploadError,
    ]
    ...
    with mock.patch.object(
      ...
      uploader.start_uploading()
    self.assertEqual(5,
                     mock_client.write_tensorboard_experiment_data.call_count)
  ...
  def test_start_uploading_scalars_one_shot(self):
    ...
    def batch_create_runs(parent, requests):
      ...
      for request in requests:
        ...
        tb_run.name = "{}/runs/{}".format(request.parent,
                                          request.tensorboard_run_id)
    ...
    mock_logdir_loader.get_run_events.side_effect = [
        {
            "run 1":
                _apply_compat([
                    _scalar_event("tag_1.1", 5.0),
                    _scalar_event("tag_1.2", 5.0)
                ]),
            "run 2":
                _apply_compat([
                    _scalar_event("tag_2.1", 5.0),
                    _scalar_event("tag_2.2", 5.0)
                ]),
        },
        # Note the lack of AbortUploadError here.
    ]
    mock_logdir_loader_pre_create = mock.create_autospec(
        logdir_loader.LogdirLoader)
    ...
    with mock.patch.object(uploader, "_logdir_loader", mock_logdir_loader):
      with mock.patch.object(uploader, "_logdir_loader_pre_create",
                             mock_logdir_loader_pre_create):
        uploader.start_uploading()
  ...
  def test_upload_full_logdir(self):
    ...
    writer.add_test_tensor_summary(
        ...
        tensor=tensor_pb2.TensorProto(
            dtype=types_pb2.DT_FLOAT, float_val=[8.0]),
        step=3,
    ...
    expected_request1 = [
        data(
            ...
            value_type=tensorboard_time_series_type.TensorboardTimeSeries
            .ValueType.SCALAR,
            values=[
  ...
  def test_start_uploading_graphs(self):
    ...
    for call in mock_client.write_tensorboard_experiment_data.call_args_list:
      ...
      self.assertEqual(time_series_data[0].tensorboard_time_series_id,
                       _TEST_TIME_SERIES_NAME)
...
class BatchedRequestSenderTest(tf.test.TestCase):
  ...
  def _populate_run_from_events(self,
                                n_scalar_events,
                                events,
                                allowed_plugins=_USE_DEFAULT):
    mock_client = _create_mock_client()
  ...
  def test_expands_multiple_values_in_event(self):
    ...
    time_series_data = tensorboard_data.TimeSeriesData(
        tensorboard_time_series_id="foo",
        value_type=tensorboard_time_series_type.TensorboardTimeSeries.ValueType
        .SCALAR,
        values=[
...
class ScalarBatchedRequestSenderTest(tf.test.TestCase):
  ...
  def test_aggregation_by_tag(self):
    ...
    tag_data = {
        ts.tensorboard_time_series_id: [(
            ...
            value.scalar.value,
        ) for value in ts.values] for ts in ts_data
    }
    self.assertEqual(
        tag_data,
        {
  ...
  def test_v1_summary(self):
    ...
    self.assertEqual(_TEST_EXPERIMENT_NAME,
                     call_args[1]["tensorboard_experiment"])
    self.assertEqual(
        [
  ...
  def test_break_at_tag_boundary(self):
    ...
    request1 = call_args_list[0][1]["write_run_data_requests"][
        0].time_series_data
    _clear_wall_times(request1)
  ...
  def test_wall_time_precision(self):
    ...
    self.assertEqual(
        ...
        [0].time_series_data[0].values[0].wall_time,
    )
    self.assertEqual(
        datetime_helpers.DatetimeWithNanoseconds.from_timestamp_pb(
            _timestamp_pb(1000000002)),
        call_args[1]["write_run_data_requests"]
        [0].time_series_data[0].values[1].wall_time,
    )
...
class FileRequestSenderTest(tf.test.TestCase):
  ...
  def test_multi_file_upload(self):
    ...
    files = None
    with tempfile.NamedTemporaryFile() as f1, tempfile.NamedTemporaryFile(
    ) as f2:
    ...
    self.assertEqual(
        files,
        [
            x.id for x in call_args_list["time_series_data"]
            [0].values[0].blobs.values
        ],
    )
