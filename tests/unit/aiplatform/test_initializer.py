...
class TestInit:
  ...
  def test_create_client_returns_client(self):
    ...
    assert (client._transport._host ==
            f"{_TEST_LOCATION}-{constants.API_BASE_PATH}:443")
  ...
  def test_get_client_options(
    ...
    assert (initializer.global_config.get_client_options(
        location_override=location_override).api_endpoint == expected_endpoint)
...
class TestThreadPool:
  ...
  @pytest.mark.parametrize("cpu_count, expected", [(4, 20), (32, 32), (None, 4),
                                                   (2, 10)])
