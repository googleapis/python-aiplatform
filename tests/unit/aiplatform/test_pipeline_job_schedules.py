...
from google.cloud.aiplatform.aiplatform import pipeline_job_schedules
...
_TEST_NETWORK = f"projects/{_TEST_PROJECT}/global/networks/{_TEST_PIPELINE_JOB_SCHEDULE_ID}"
...
def mock_schedule_bucket_exists():
    ...
    with mock.patch(
        "google.cloud.aiplatform.aiplatform.utils.gcs_utils.create_gcs_bucket_for_pipeline_artifacts_if_it_does_not_exist",
...
def mock_schedule_service_get_with_fail():
    with mock.patch.object(
        ...
        mock_get_schedule.side_effect = [
            make_schedule(gca_schedule.Schedule.State.ACTIVE),
            make_schedule(gca_schedule.Schedule.State.ACTIVE),
            make_schedule(gca_schedule.Schedule.State.STATE_UNSPECIFIED), # should be FAILED
        ]
...
@pytest.fixture
def mock_schedule_service_list_jobs():
...
class TestPipelineJobSchedule:
    ...
    def test_get_schedule(self, mock_schedule_service_get):
        ...
        pipeline_job_schedule = pipeline_job_schedules.PipelineJobSchedule.get(resource_name=_TEST_PIPELINE_JOB_SCHEDULE_ID)

        mock_schedule_service_get.assert_called_once_with(
            name=_TEST_PIPELINE_JOB_SCHEDULE_NAME, retry=base._DEFAULT_RETRY
        )
        assert isinstance(pipeline_job_schedule, pipeline_job_schedules.PipelineJobSchedule)
    ...
    def test_pause_resume_schedule_service(
        self, mock_schedule_service_pause,
        mock_schedule_service_resume, mock_load_yaml_and_json,
    ):
        aiplatform.init(
            project=_TEST_PROJECT,
            staging_bucket=_TEST_GCS_BUCKET_NAME,
    ...
    def test_list_schedules(
        self, mock_schedule_service_list,
    ):
        aiplatform.init(
            project=_TEST_PROJECT,
            staging_bucket=_TEST_GCS_BUCKET_NAME,
        ...
        pipeline_job_schedules.PipelineJobSchedule.list()
    ...
    def test_list_jobs(
        self, mock_schedule_service_list_jobs,
    ):
        raise NotImplementedError
