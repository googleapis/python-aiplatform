...
_TEST_DEPLOYED_MODELS = [
    ...
    gca_endpoint.DeployedModel(
        id=_TEST_ID_2, display_name=_TEST_DISPLAY_NAME_2),
    gca_endpoint.DeployedModel(
        id=_TEST_ID_3, display_name=_TEST_DISPLAY_NAME_3),
]
...
_TEST_EXPLANATION_METADATA = explain.ExplanationMetadata(
    inputs={
        "features":
            explain.ExplanationMetadata.InputMetadata({
...
def update_endpoint_mock():
  with mock.patch.object(endpoint_service_client.EndpointServiceClient,
                         "update_endpoint") as update_endpoint_mock:
...
def undeploy_model_mock():
  with mock.patch.object(endpoint_service_client.EndpointServiceClient,
                         "undeploy_model") as undeploy_model_mock:
...
def delete_endpoint_mock():
  with mock.patch.object(endpoint_service_client.EndpointServiceClient,
                         "delete_endpoint") as delete_endpoint_mock:
...
def sdk_undeploy_all_mock():
  ...
  with mock.patch.object(aiplatform.Endpoint,
                         "undeploy_all") as sdk_undeploy_all_mock:
...
def list_endpoints_mock():
  with mock.patch.object(endpoint_service_client.EndpointServiceClient,
                         "list_endpoints") as list_endpoints_mock:
...
def create_private_endpoint_mock():
  with mock.patch.object(endpoint_service_client.EndpointServiceClient,
                         "create_endpoint") as create_private_endpoint_mock:
...
def sdk_undeploy_mock():
  ...
  with mock.patch.object(aiplatform.PrivateEndpoint,
                         "undeploy") as sdk_undeploy_mock:
    sdk_undeploy_mock.return_value = None
    yield sdk_undeploy_mock
...
class TestEndpoint:
  ...
  def test_lazy_constructor_calls_get_on_property_access(
      self, get_endpoint_mock):
    ep = models.Endpoint(_TEST_ENDPOINT_NAME)
    assert ep._gca_resource.name == _TEST_ENDPOINT_NAME
    assert ep._skipped_getter_call()
    assert not get_endpoint_mock.called
  ...
  def test_constructor_with_custom_credentials(self,
  ...
  @pytest.mark.usefixtures("get_endpoint_mock")
  @pytest.mark.parametrize("sync", [True, False])
  def test_create_with_timeout_not_explicitly_set(self, create_endpoint_mock,
                                                  sync):
    my_endpoint = models.Endpoint.create(
        display_name=_TEST_DISPLAY_NAME,
        encryption_spec_key_name=_TEST_ENCRYPTION_KEY_NAME,
  ...
  @pytest.mark.usefixtures("get_endpoint_mock", "get_model_mock")
  @pytest.mark.parametrize("sync", [True, False])
  def test_deploy_with_explanations(self, deploy_model_with_explanations_mock,
                                    sync):
    test_endpoint = models.Endpoint(_TEST_ENDPOINT_NAME)
    test_model = models.Model(_TEST_ID)
    test_model._gca_resource.supported_deployment_resources_types.append(
  ...
  @pytest.mark.usefixtures("get_endpoint_mock")
  def test_predict_with_timeout_not_explicitly_set(self,
  ...
  @pytest.mark.usefixtures("get_endpoint_mock")
  def test_explain_with_timeout_not_explicitly_set(self,
  ...
  def test_list_endpoint_has_prediction_client(self):
    ...
    assert all([
        isinstance(e._prediction_client,
                   aiplatform.utils.PredictionClientWithOverride)
        for e in ep_list
    ])
  ...
  def test_list_endpoint_order_by_time(self, list_endpoints_mock):
    ...
    list_endpoints_mock.assert_called_once_with(request={
        "parent": _TEST_PARENT,
    ...
    assert ep_list[0].create_time > ep_list[1].create_time > ep_list[
        2].create_time
  ...
  def test_list_endpoint_order_by_display_name(self, list_endpoints_mock):
    ...
    assert (ep_list[0].display_name < ep_list[1].display_name <
            ep_list[2].display_name)
...
class TestPrivateEndpoint(TestEndpoint):
  ...
  @pytest.mark.usefixtures("get_private_endpoint_with_model_mock")
  @pytest.mark.parametrize("sync", [True, False])
  def test_delete_without_force(self, sdk_undeploy_mock, delete_endpoint_mock,
                                sync):
  ...
  @pytest.mark.usefixtures("get_private_endpoint_with_model_mock")
  @pytest.mark.parametrize("sync", [True, False])
  def test_delete_with_force(self, sdk_undeploy_mock, delete_endpoint_mock,
                             sync):
