...
_TEST_DEDICATED_ENDPOINT_DNS = f"{_TEST_ID}.{_TEST_PROJECT}-aiplatform.vertex.goog"
...
@pytest.fixture
def predict_endpoint_httpx_mock():
    resp = requests.Response()
    resp.status_code = 200
    resp._content = json.dumps(
        {
    ...
    with mock.patch.object(httpx.AsyncClient, "post") as predict_mock:
        predict_mock.return_value = resp
        yield predict_mock
...
class TestEndpoint:
    ...
    @pytest.mark.asyncio
    @pytest.mark.usefixtures("get_dedicated_endpoint_mock")
    async def test_predict_async_dedicated_endpoint(self, predict_endpoint_httpx_mock):
        """Tests the Endpoint.predict_async method."""
        test_endpoint = models.Endpoint(_TEST_ID)
        test_prediction = await test_endpoint.predict_async(
            instances=_TEST_INSTANCES,
        ...
        assert true_prediction == test_prediction
        predict_endpoint_httpx_mock.assert_called_once_with(
            url=f"https://{_TEST_DEDICATED_ENDPOINT_DNS}/v1/projects/{_TEST_PROJECT}/locations/{_TEST_LOCATION}/endpoints/{_TEST_ID}:rawPredict",
            data=_TEST_RAW_INPUTS,
            headers={
