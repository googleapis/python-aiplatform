...
def test_validate_region_tpc_hardcoded():
    """Test that u-us-prp1 is natively supported in constants."""
    assert aiplatform.utils.validate_region(_TEST_LOCATION_TPC)


def test_validate_region_tpc_fails_without_context():
    """Test that non-hardcoded TPC regions still fail without context."""
    with pytest.raises(ValueError, match="Unsupported region for Vertex AI"):
        aiplatform.utils.validate_region("u-us-prp2")
