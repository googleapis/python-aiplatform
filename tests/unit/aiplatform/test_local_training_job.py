...
class TestLocalModelClass:
    ...
    class LinearRegression(aiplatform.experimental.vertex_model.VertexModel): 
        _data_serialization_mapping Dict[Type:Tuple[Callable(str,...), Callable(Type:)]]= {
            pd.DataFrame : (pd.read_csv, lambda df: df.to_csv)}
        ...
        def fit(self, data, epochs, learning_rate, dataset: pandas.DataFrame):
    ...
    def test_create_custom_job(self):
        ...
        """ 
        model.training_mode = 'local'

        # fake paths here
        training_data = pandas.read_csv("/path/to/training_data")
        test_data = pandas.read_csv("/path/to/test_data")

        model.fit(training_data, epoch=10, learning_rate=0.1)
