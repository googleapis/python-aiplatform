...
class TestSandbox:
    ...
    def test_generate_access_token(self):
        ...
        ) as mock_iam_credentials_client:
            mock_iam_credentials_client.return_value.sign_jwt.return_value.signed_jwt = (
                "test_signed_jwt"
            )
            token = self.client.agent_engines.sandboxes.generate_access_token(
                service_account_email=_TEST_SERVICE_ACCOUNT_EMAIL,
                sandbox_id=_TEST_SANDBOX_RESOURCE_NAME,
                timeout=3600,
            )
            assert token == "test_signed_jwt"
            mock_iam_credentials_client.assert_called_once()
            call_args = mock_iam_credentials_client.return_value.sign_jwt.call_args
            assert (
                call_args.kwargs["request"].name
                == f"projects/-/serviceAccounts/{_TEST_SERVICE_ACCOUNT_EMAIL}"
            )
    ...
    def test_send_command(self):
        ...
        with mock.patch(
            "google.cloud.aiplatform.vertexai._genai.sandboxes.genai.Client"
        ) as mock_genai_client:
            mock_http_client = mock_genai_client.return_value
            mock_http_client._api_client.request.return_value = (
            ...
            mock_http_client._api_client.request.assert_called_with(
                "GET", "test/path", {}
            )
