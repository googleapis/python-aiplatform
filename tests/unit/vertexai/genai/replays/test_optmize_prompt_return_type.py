...
def test_optimize_prompt(client):
  ...
  logger.error("test_optimize_prompt -- CONNIEBONNIE")
  client._api_client._http_options.base_url = (
      "https://us-central1-autopush-aiplatform.sandbox.googleapis.com"
  )
  test_prompt = "Generate system instructions for analyzing medical articles"
  prompt = types.Content(parts=[types.Part(text=test_prompt)], role="user")
  response = client.prompt_optimizer._optimize_prompt(content=prompt)
