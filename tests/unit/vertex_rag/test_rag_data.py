...
def import_files_request_eq(returned_request, expected_request):
    ...
    assert (
        returned_request.import_rag_files_config.partial_failure_gcs_sink
        == expected_request.import_rag_files_config.partial_failure_gcs_sink
    )
    assert (
        returned_request.import_rag_files_config.partial_failure_bigquery_sink
        == expected_request.import_rag_files_config.partial_failure_bigquery_sink
    )
...
class TestRagDataManagement:
    ...
    def test_import_files_with_partial_failures_gcs_sink(self, import_files_mock):
        response = rag.import_files(
            corpus_name=test_rag_constants.TEST_RAG_CORPUS_RESOURCE_NAME,
            paths=[test_rag_constants.TEST_GCS_PATH],
            partial_failures_sink=test_rag_constants.TEST_PARTIAL_FAILURES_GCS_SINK,
        )
        import_files_mock.assert_called_once()

        assert response.imported_rag_files_count == 2

    def test_import_files_with_partial_failures_bigquery_sink(self, import_files_mock):
        response = rag.import_files(
            corpus_name=test_rag_constants.TEST_RAG_CORPUS_RESOURCE_NAME,
            paths=[test_rag_constants.TEST_GCS_PATH],
            partial_failures_sink=test_rag_constants.TEST_PARTIAL_FAILURES_BIGQUERY_SINK,
        )
        import_files_mock.assert_called_once()
    ...
    @pytest.mark.asyncio
    async def test_import_files_with_partial_failures_gcs_sink_async(
    ...
    @pytest.mark.asyncio
    async def test_import_files_with_partial_failures_bigquery_sink_async(
        self, import_files_async_mock
    ):
        response = await rag.import_files_async(
            corpus_name=test_rag_constants.TEST_RAG_CORPUS_RESOURCE_NAME,
