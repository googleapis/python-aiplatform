...
def list_rag_corpora_pager_mock():
    with mock.patch.object(
        ...
        list_rag_corpora_pager_mock.return_value = [
            ListRagCorporaResponse(
                rag_corpora=[tc.TEST_GAPIC_RAG_CORPUS,],
...
def list_rag_files_pager_mock():
    with mock.patch.object(
        ...
        list_rag_files_pager_mock.return_value = [
            ListRagFilesResponse(
                rag_files=[tc.TEST_GAPIC_RAG_FILE,],
                next_page_token=tc.TEST_PAGE_TOKEN,
            ),
        ]
...
def import_files_request_eq(returned_request, expected_request):
    ...
    assert returned_request.import_rag_files_config.gcs_source.uris == expected_request.import_rag_files_config.gcs_source.uris
    assert returned_request.import_rag_files_config.google_drive_source.resource_ids == expected_request.import_rag_files_config.google_drive_source.resource_ids
