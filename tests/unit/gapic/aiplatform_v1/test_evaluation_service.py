...
from google.cloud.aiplatform_v1.services.evaluation_service import EvaluationServiceAsyncClient
...
def test__get_client_cert_source():
    ...
    assert EvaluationServiceClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert EvaluationServiceClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert EvaluationServiceClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert EvaluationServiceClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source
...
def test_evaluation_service_client_from_service_account_info(client_class, transport_name):
...
def test_evaluation_service_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_evaluation_service_client_from_service_account_file(client_class, transport_name):
    creds = ga_credentials.AnonymousCredentials()
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    ...
    (EvaluationServiceAsyncClient, transports.EvaluationServiceGrpcAsyncIOTransport, "grpc_asyncio"),
    (EvaluationServiceClient, transports.EvaluationServiceRestTransport, "rest"),
])
@mock.patch.object(EvaluationServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(EvaluationServiceClient))
@mock.patch.object(EvaluationServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(EvaluationServiceAsyncClient))
def test_evaluation_service_client_client_options(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (EvaluationServiceClient, transports.EvaluationServiceGrpcTransport, "grpc", "true"),
    (EvaluationServiceAsyncClient, transports.EvaluationServiceGrpcAsyncIOTransport, "grpc_asyncio", "true"),
    (EvaluationServiceClient, transports.EvaluationServiceGrpcTransport, "grpc", "false"),
    (EvaluationServiceAsyncClient, transports.EvaluationServiceGrpcAsyncIOTransport, "grpc_asyncio", "false"),
    (EvaluationServiceClient, transports.EvaluationServiceRestTransport, "rest", "true"),
    (EvaluationServiceClient, transports.EvaluationServiceRestTransport, "rest", "false"),
])
@mock.patch.object(EvaluationServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(EvaluationServiceClient))
@mock.patch.object(EvaluationServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(EvaluationServiceAsyncClient))
@mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "auto"})
def test_evaluation_service_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@pytest.mark.parametrize("client_class", [
    EvaluationServiceClient, EvaluationServiceAsyncClient
])
@mock.patch.object(EvaluationServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(EvaluationServiceClient))
@mock.patch.object(EvaluationServiceAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(EvaluationServiceAsyncClient))
...
def test_evaluation_service_client_client_api_endpoint(client_class):
    ...
    default_endpoint = EvaluationServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = EvaluationServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)
...
def test_evaluation_service_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (EvaluationServiceClient, transports.EvaluationServiceGrpcTransport, "grpc", grpc_helpers),
    (EvaluationServiceAsyncClient, transports.EvaluationServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
    (EvaluationServiceClient, transports.EvaluationServiceRestTransport, "rest", None),
])
def test_evaluation_service_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_evaluation_service_client_client_options_from_dict():
    with mock.patch('google.cloud.aiplatform_v1.services.evaluation_service.transports.EvaluationServiceGrpcTransport.__init__') as grpc_transport:
...
def test_evaluation_service_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(
        credentials_file="credentials.json"
    )
...
def test_evaluate_instances(request_type, transport: str = 'grpc'):
    client = EvaluationServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            type(client.transport.evaluate_instances),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = evaluation_service.EvaluateInstancesResponse(
        )
    ...
    assert isinstance(response, evaluation_service.EvaluateInstancesResponse)


def test_evaluate_instances_non_empty_request_with_auto_populated_field():
    ...
    with mock.patch.object(
        ...
        client.evaluate_instances(request=request)
        call.assert_called()
        _, args, _ = call.mock_calls[0]
...
def test_evaluate_instances_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.evaluate_instances in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.evaluate_instances] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_evaluate_instances_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.evaluate_instances in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.evaluate_instances] = mock_rpc
...
async def test_evaluate_instances_async(request_type, transport: str = 'grpc_asyncio'):
    client = EvaluationServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(evaluation_service.EvaluateInstancesResponse(
        ))
...
def test_evaluate_instances_field_headers():
    client = EvaluationServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
    )
...
@pytest.mark.asyncio
async def test_evaluate_instances_field_headers_async():
    client = EvaluationServiceAsyncClient(
        credentials=async_anonymous_credentials(),
    )
...
def test_evaluate_instances_rest_required_fields(request_type=evaluation_service.EvaluateInstancesRequest):
    ...
    default_values = getattr(
        transport_class._BaseEvaluateInstances,
        "_BaseEvaluateInstances__REQUIRED_FIELDS_DEFAULT_VALUES",
        {},
    )
    ...
    return_value = evaluation_service.EvaluateInstancesResponse()
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        with mock.patch.object(path_template, 'transcode') as transcode:
            ...
            return_value = evaluation_service.EvaluateInstancesResponse.pb(return_value)
...
def test_evaluate_instances_empty_call_grpc():
    client = EvaluationServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = evaluation_service.EvaluateInstancesResponse()
        client.evaluate_instances(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_evaluate_instances_empty_call_grpc_asyncio():
    client = EvaluationServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.evaluate_instances(request=None)
...
def test_evaluate_instances_rest_bad_request(request_type=evaluation_service.EvaluateInstancesRequest):
    client = EvaluationServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_evaluate_instances_rest_call_success(request_type):
    client = EvaluationServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_evaluate_instances_rest_interceptors(null_interceptor):
    transport = transports.EvaluationServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.EvaluationServiceRestInterceptor(),
        )
    client = EvaluationServiceClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.EvaluationServiceRestInterceptor, "post_evaluate_instances") as post, \
        mock.patch.object(transports.EvaluationServiceRestInterceptor, "post_evaluate_instances_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.EvaluationServiceRestInterceptor, "pre_evaluate_instances") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = evaluation_service.EvaluateInstancesRequest.pb(evaluation_service.EvaluateInstancesRequest())
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = evaluation_service.EvaluateInstancesResponse.to_json(evaluation_service.EvaluateInstancesResponse())
        ...
        post_with_metadata.return_value = evaluation_service.EvaluateInstancesResponse(), metadata

        client.evaluate_instances(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_evaluate_instances_empty_call_rest():
    client = EvaluationServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_transport_kind_rest_asyncio():
    ...
    transport = EvaluationServiceAsyncClient.get_transport_class("rest_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
@pytest.mark.asyncio
async def test_evaluate_instances_rest_asyncio_bad_request(request_type=evaluation_service.EvaluateInstancesRequest):
    if not HAS_ASYNC_REST_EXTRA:
        pytest.skip("the library must be installed with the `async_rest` extra to test this feature.")
    client = EvaluationServiceAsyncClient(
        credentials=async_anonymous_credentials(),
...
async def test_evaluate_instances_rest_asyncio_call_success(request_type):
    if not HAS_ASYNC_REST_EXTRA:
        pytest.skip("the library must be installed with the `async_rest` extra to test this feature.")
    client = EvaluationServiceAsyncClient(
        credentials=async_anonymous_credentials(),
...
@pytest.mark.asyncio
@pytest.mark.parametrize("null_interceptor", [True, False])
async def test_evaluate_instances_rest_asyncio_interceptors(null_interceptor):
    if not HAS_ASYNC_REST_EXTRA:
        pytest.skip("the library must be installed with the `async_rest` extra to test this feature.")
    transport = transports.AsyncEvaluationServiceRestTransport(
        credentials=async_anonymous_credentials(),
        interceptor=None if null_interceptor else transports.AsyncEvaluationServiceRestInterceptor(),
        )
    client = EvaluationServiceAsyncClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.AsyncEvaluationServiceRestInterceptor, "post_evaluate_instances") as post, \
        mock.patch.object(transports.AsyncEvaluationServiceRestInterceptor, "post_evaluate_instances_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.AsyncEvaluationServiceRestInterceptor, "pre_evaluate_instances") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = evaluation_service.EvaluateInstancesRequest.pb(evaluation_service.EvaluateInstancesRequest())
        ...
        await client.evaluate_instances(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_evaluate_instances_empty_call_rest_asyncio():
    if not HAS_ASYNC_REST_EXTRA:
        pytest.skip("the library must be installed with the `async_rest` extra to test this feature.")
    client = EvaluationServiceAsyncClient(
        credentials=async_anonymous_credentials(),
...
def test_evaluation_service_base_transport():
    ...
    with mock.patch('google.cloud.aiplatform_v1.services.evaluation_service.transports.EvaluationServiceTransport.__init__') as Transport:
...
def test_evaluation_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.aiplatform_v1.services.evaluation_service.transports.EvaluationServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_evaluation_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.aiplatform_v1.services.evaluation_service.transports.EvaluationServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.EvaluationServiceTransport()
        adc.assert_called_once()
...
@pytest.mark.parametrize("transport_class", [transports.EvaluationServiceGrpcTransport, transports.EvaluationServiceGrpcAsyncIOTransport])
def test_evaluation_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.EvaluationServiceGrpcTransport, transports.EvaluationServiceGrpcAsyncIOTransport])
def test_evaluation_service_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
def test_client_with_default_client_info():
    ...
    with mock.patch.object(transports.EvaluationServiceTransport, '_prep_wrapped_messages') as prep:
        client = EvaluationServiceClient(
            credentials=ga_credentials.AnonymousCredentials(),
...
@pytest.mark.parametrize("client_class,transport_class", [
    ...
    (EvaluationServiceAsyncClient, transports.EvaluationServiceGrpcAsyncIOTransport),
])
