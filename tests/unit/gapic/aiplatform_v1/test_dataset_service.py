...
def test__get_default_mtls_endpoint():
    ...
    assert (
        DatasetServiceClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch(
            ...
            assert DatasetServiceClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch(
            ...
            assert DatasetServiceClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(
            ...
            with pytest.raises(ValueError):
                DatasetServiceClient._use_client_cert_effective()
...
async def test_search_data_items_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.search_data_items(request={})).pages:
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
