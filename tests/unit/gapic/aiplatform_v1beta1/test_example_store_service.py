...
def test__get_default_mtls_endpoint():
    ...
    assert (
        ExampleStoreServiceClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
...
async def test_list_example_stores_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_example_stores(request={})).pages:
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
async def test_fetch_examples_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.fetch_examples(request={})).pages:
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
