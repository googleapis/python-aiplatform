...
@pytest.mark.parametrize("client_class", [DatasetServiceClient, DatasetServiceAsyncClient])
...
@pytest.mark.asyncio
async def test_list_annotations_async(transport: str = 'grpc_asyncio'):
    client = DatasetServiceAsyncClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_dataset_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file') as load_creds, mock.patch('google.cloud.aiplatform_v1beta1.services.dataset_service.transports.DatasetServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_dataset_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.cloud.aiplatform_v1beta1.services.dataset_service.transports.DatasetServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.DatasetServiceTransport()
        adc.assert_called_once()
