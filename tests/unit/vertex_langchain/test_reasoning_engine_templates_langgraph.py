...
class TestLanggraphAgent:
    ...
    def test_initialization_with_tools(self, mock_chatvertexai):
        ...
        agent = reasoning_engines.LanggraphAgent(model=_TEST_MODEL, tools=tools)
    ...
    def test_get_state_after_update_state_with_config(self):
        agent = reasoning_engines.LanggraphAgent(
            model=_TEST_MODEL,
            checkpointer_builder=_checkpointer_builder,
        )
        init_message = {"messages": [{"role": "user", "content": "user message"}]}
        agent.update_state(
            config=_TEST_CONFIG,
            values=init_message,
        )
        state = agent.get_state(config=_TEST_CONFIG)
        assert _get_state_messages(state) == ["user message"]
        ...
        agent.update_state(
            config=_TEST_CONFIG,
            values=updated_message,
        )
        state = agent.get_state(config=_TEST_CONFIG)
        assert _get_state_messages(state) == [
            "user message",
            "assistant message",
        ]

    def test_get_state_history_after_update_state_with_config(self):
        agent = reasoning_engines.LanggraphAgent(
            model=_TEST_MODEL,
            checkpointer_builder=_checkpointer_builder,
        )
        init_messages = {"messages": [{"role": "user", "content": "user message"}]}
        agent.update_state(
            config=_TEST_CONFIG,
            values=init_messages,
        )
        history = list(agent.get_state_history(config=_TEST_CONFIG))
        assert len(history) == 1
        assert _get_state_messages(history[0]) == ["user message"]
        ...
        agent.update_state(
            config=_TEST_CONFIG,
            values=updated_messages,
        )
        history = list(agent.get_state_history(config=_TEST_CONFIG))
        assert len(history) == 2
        assert _get_state_messages(history[0]) == ["user message", "assistant message"]
        assert _get_state_messages(history[1]) == ["user message"]
