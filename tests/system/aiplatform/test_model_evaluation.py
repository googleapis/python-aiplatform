...
_TEST_PERMANENT_AUTOML_MODEL_CLASSIFICATION_RESOURCE_NAME = "TODO"
...
class TestModelEvaluationJob(e2e_base.TestEndToEnd):
    ...
    def test_automl_model_with_sliced_metrics(self, staging_bucket, shared_state):

        # TODO: add this automl model resource to the tests project
        automl_tabular_model = aiplatform.Model(
            model_name=_TEST_PERMANENT_AUTOML_MODEL_CLASSIFICATION_RESOURCE_NAME
        )

        eval_job = automl_tabular_model.evaluate(
            gcs_source_uris=[_TEST_EVAL_DATA_URI],
            prediction_type="classification",
            class_labels=_TEST_MODEL_EVAL_CLASS_LABELS,
            target_field_name=_TEST_TARGET_FIELD_NAME,
        ...
        sliced_metrics = model_eval.get_model_evaluation_slices()

        # TODO: confirm this is the right assert for the sliced_metrics dict
        assert "overall" in sliced_metrics
        assert "sepal_length" in sliced_metrics
