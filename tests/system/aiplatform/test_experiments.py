...
class TestExperiments(e2e_base.TestEndToEnd):
  ...
  def test_log_execution_and_artifact(self, shared_state):
    ...
    output_artifacts.sort(
        key=lambda artifact: artifact.schema_title, reverse=True)
    ...
    assert sorted([artifact.name for artifact in artifacts]) == sorted(
        [ds.name, model.name,
         run._tensorboard_run_id(run.resource_id)])
  ...
  def test_add_pipeline_job_to_experiment(self, shared_state):
    ...
    @component
    def trainer(learning_rate: float, dropout_rate: float,
  ...
  def test_get_experiments_df(self):
    ...
    true_df_dict_1[
        "run_type"] = aiplatform.metadata.constants.SYSTEM_EXPERIMENT_RUN
    true_df_dict_1[f"time_series_metric.{_TIME_SERIES_METRIC_KEY}"] = 4.0

    true_df_dict_2 = {
        f"metric.{key}": value for key, value in _METRICS_2.items()
    }
    ...
    assert sorted(
        ...
        df.fillna(0.0).to_dict("records"), key=lambda d: d["run_name"])
