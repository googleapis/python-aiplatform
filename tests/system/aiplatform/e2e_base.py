...
class TestEndToEnd(metaclass=abc.ABCMeta):
  ...
  def _temp_prefix(cls) -> str:
    """Prefix to staging bucket and display names created by this end-to-end test.
        ...
        Keep the string as short as possible and use kebab case, starting with a
        lowercase letter.
  ...
  def prepare_staging_bucket(
      ...
      shared_state: Dict[str,
                         Any]) -> Generator[storage.bucket.Bucket, None, None]:
    ...
    project_number = (
        resourcemanager.ProjectsClient().get_project(
            name=f"projects/{_PROJECT}").name.split("/", 1)[1])

    service_account = f"{project_number}-compute@developer.gserviceaccount.com"
    bucket_iam_policy = bucket.get_iam_policy()
    bucket_iam_policy.setdefault("roles/storage.objectCreator",
                                 set()).add(f"serviceAccount:{service_account}")
    bucket_iam_policy.setdefault("roles/storage.objectViewer",
                                 set()).add(f"serviceAccount:{service_account}")
  ...
  def prepare_bigquery_dataset(
    ...
    dataset_name = f"{self._temp_prefix.lower()}_{uuid.uuid4()}".replace(
        "-", "_")
  ...
  def delete_bigquery_dataset(self, shared_state: Dict[str, Any]):
    ...
    bigquery_client.delete_dataset(
        bigquery_dataset.dataset_id, delete_contents=True,
  ...
  def bigquery_dataset(self) -> Generator[bigquery.dataset.Dataset, None, None]:
    ...
    bigquery_client.delete_dataset(
        dataset.dataset_id, delete_contents=True,
        not_found_ok=True)  # Make an API request.
  ...
  def tear_down_resources(self, shared_state: Dict[str, Any]):
    ...
    shared_state["resources"].sort(
        key=lambda r: 1 if isinstance(r, aiplatform.Endpoint) or isinstance(
            r, aiplatform.MatchingEngineIndexEndpoint) else 2)
