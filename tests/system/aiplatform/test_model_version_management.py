...
class TestVersionManagement(e2e_base.TestEndToEnd):
  ...
  def test_upload_deploy_manage_versioned_model(self, shared_state):
    ...
    assert (model_info.version_aliases == list_info.version_aliases ==
            model.version_aliases)
    assert (model_info.version_description == list_info.version_description ==
            model.version_description)
    assert (model_info.model_display_name == list_info.model_display_name ==
            model.display_name)
    assert (model_info.version_update_time == list_info.version_update_time ==
