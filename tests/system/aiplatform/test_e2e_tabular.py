...
class TestEndToEndTabular:
    ...
    def prepare_staging_bucket(self, shared_state):
        """Create a staging bucket, download CSV data, and copy into bucket."""

        new_staging_bucket = f"{_TEMP_PREFIX}-staging-{uuid.uuid4()}"[:63]
    ...
    def delete_staging_bucket(self, shared_state):
        ...
        new_staging_bucket = shared_state["staging_bucket"]

        # Get the staging bucket used for testing and wipe it
        bucket = storage_client.get_bucket(new_staging_bucket)
    ...
    def test_end_to_end_tabular(self, shared_state):
        ...
        aiplatform.init(
            project=_PROJECT,
            location=_LOCATION,
            staging_bucket=shared_state["staging_bucket"],
        )

        # Create and import to single managed dataset for both training jobs

        ds = aiplatform.TabularDataset.create(
            display_name=f"{_TEMP_PREFIX}-dataset-{uuid.uuid4()}",
            gcs_source=[f'gs://{shared_state["staging_bucket"]}/{_BLOB_PATH}'],
            sync=False,
        )
        ...
        automl_job = aiplatform.AutoMLTabularTrainingJob(
            display_name=f"{_TEMP_PREFIX}-train-housing-automl-{uuid.uuid4()}",
        ...
        custom_model = custom_job.run(
            ds,
            replica_count=1,
            model_display_name=f"{_TEMP_PREFIX}-custom-housing-model-{uuid.uuid4()}",
            sync=False,
        )
        ...
        automl_model = automl_job.run(
            ...
            model_display_name=f"{_TEMP_PREFIX}-automl-housing-model-{uuid.uuid4()}",
