...
class TestTokenization(e2e_base.TestEndToEnd):
    ...
    @pytest.mark.skip(reason="Waiting for bug fixes and server rollout.")
    @pytest.mark.parametrize(
        "model_name",
        _MODELS,
    )
    ...
    def test_count_tokens_tool_is_function_declaration(self, model_name):
        ...
        assert tokenizer.count_tokens("hello", tools=[tool2]).total_tokens
        assert (
        ...
        assert (
            ...
            == model.count_tokens("hello", tools=[tool2]).total_tokens
        )
