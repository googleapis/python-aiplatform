...
def _GetEndpointParameters_to_vertex(
    ...
    to_object: dict[str, Any] = {}
    if getv(from_object, ["name"]) is not None:
        setv(to_object, ["_url", "name"], t.t_endpoint(getv(from_object, ["name"])))
...
def _UndeployModelRequestParameters_to_vertex(
    ...
    to_object: dict[str, Any] = {}
    if getv(from_object, ["name"]) is not None:
        setv(to_object, ["_url", "endpoint"], getv(from_object, ["name"]))
        ...
        *,
        ...
        if "endpoints" in name:
            ...
            if deployed_endpoint.dedicated_endpoint_enabled:
                if deployed_endpoint.dedicated_endpoint_dns:
                    dedicated_endpoint_dns = deployed_endpoint.dedicated_endpoint_dns
                else:
                    raise ValueError(
                        "Dedicated endpoint is enabled but no dedicated endpoint DNS is provided."
                    )

                # Inject dedicated endpoint if enabled.
                if config.http_options is None:
                    config.http_options = {
                        "base_url": "https://" + dedicated_endpoint_dns,
                        "api_version": "v1",
                    }
                else:
                    config.http_options.base_url = "https://" + dedicated_endpoint_dns
                    config.http_options.api_version = (
                        "v1"  # Otherwise may see 429 errors
                    )
