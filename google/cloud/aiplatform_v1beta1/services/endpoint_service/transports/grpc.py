...
class EndpointServiceGrpcTransport(EndpointServiceTransport):
  ...
  def create_endpoint(
      self,
  ) -> Callable[[endpoint_service.CreateEndpointRequest],
  ...
  def list_endpoints(
    ...
    if "list_endpoints" not in self._stubs:
      self._stubs["list_endpoints"] = self.grpc_channel.unary_unary(
          ...
          response_deserializer=endpoint_service.ListEndpointsResponse
          .deserialize,
      )
  ...
  def update_endpoint(
      self,
  ) -> Callable[[endpoint_service.UpdateEndpointRequest],
  ...
  def deploy_model(
      self,
  ) -> Callable[[endpoint_service.DeployModelRequest],
                operations_pb2.Operation]:
  ...
  def undeploy_model(
      self,
  ) -> Callable[[endpoint_service.UndeployModelRequest],
                operations_pb2.Operation]:
