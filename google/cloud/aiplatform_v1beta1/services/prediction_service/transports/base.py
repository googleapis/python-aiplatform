...
class PredictionServiceTransport(abc.ABC):
  ...
  def predict(
      self,
  ) -> Callable[[prediction_service.PredictRequest],
  ...
  def raw_predict(
      self,
  ) -> Callable[[prediction_service.RawPredictRequest], Union[
      httpbody_pb2.HttpBody, Awaitable[httpbody_pb2.HttpBody]],]:
    raise NotImplementedError()

  @property
  def explain(
      self,
  ) -> Callable[[prediction_service.ExplainRequest],
                Union[prediction_service.ExplainResponse,
