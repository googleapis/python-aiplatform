...
class CreateTrainingPipelineRequest(proto.Message):
  r"""Request message for
    ...
    Attributes:
        ...
        training_pipeline
          (google.cloud.aiplatform_v1beta1.types.TrainingPipeline): Required.
          The TrainingPipeline to create.
  """
...
class GetTrainingPipelineRequest(proto.Message):
  r"""Request message for
    ...
    Attributes:
        name (str): Required. The name of the TrainingPipeline resource. Format:
  ...
  r"""Request message for
    ...
    Attributes:
        parent (str): Required. The resource name of the Location to list the
          ...
          ``projects/{project}/locations/{location}``
        filter (str): The standard list filter. Supported fields:  -
          ``display_name`` supports = and !=.  -  ``state`` supports = and !=.
...
class ListTrainingPipelinesResponse(proto.Message):
  r"""Response message for
    ...
    Attributes:
        training_pipelines
          (Sequence[google.cloud.aiplatform_v1beta1.types.TrainingPipeline]):
...
class DeleteTrainingPipelineRequest(proto.Message):
  r"""Request message for
    ...
    Attributes:
        name (str): Required. The name of the TrainingPipeline resource to be
          deleted. Format:
          ``projects/{project}/locations/{location}/trainingPipelines/{training_pipeline}``
  """
  ...
  r"""Request message for
    ...
    Attributes:
        ...
        pipeline_job_id (str): The ID to use for the PipelineJob, which will
          become the final component of the PipelineJob name. If not provided,
          an ID will be automatically generated.  This value should be less than
          128 characters, and valid characters are /[a-z][0-9]-/.
  """
...
class ListPipelineJobsRequest(proto.Message):
  r"""Request message for
    ...
    Attributes:
        ...
        filter (str): Lists the PipelineJobs that match the filter expression.
          The following fields are supported:  -  ``pipeline_name``: Supports
          ``=`` and ``!=`` comparisons. -  ``display_name``: Supports ``=``,
          ``!=`` comparisons, and ``:`` wildcard. -  ``pipeline_job_user_id``:
          Supports ``=``, ``!=`` comparisons, and ``:`` wildcard. for example,
          can check if pipeline's display_name contains *step* by doing
               display_name:"*step*" -  ``state``: Supports ``=`` and ``!=``
                 comparisons. -  ``create_time``: Supports ``=``, ``!=``, ``<``,
                 ``>``, ``<=``, and ``>=`` comparisons. Values must be in RFC
                 3339 format. -  ``update_time``: Supports ``=``, ``!=``, ``<``,
                 ``>``, ``<=``, and ``>=`` comparisons. Values must be in RFC
                 3339 format. -  ``end_time``: Supports ``=``, ``!=``, ``<``,
                 ``>``, ``<=``, and ``>=`` comparisons. Values must be in RFC
                 3339 format. -  ``labels``: Supports key-value equality and key
                 presence. -  ``template_uri``: Supports ``=``, ``!=``
                 comparisons, and ``:`` wildcard. -
                 ``template_metadata.version``: Supports ``=``, ``!=``
                 comparisons, and ``:`` wildcard.  Filter expressions can be
                 combined together using logical operators (``AND`` & ``OR``).
                 For example: ``pipeline_name="test" AND
                 create_time>"2020-05-18T13:30:00Z"``.  The syntax to define
                 filter expression is based on
            https://google.aip.dev/160.
            Examples:  -  ``create_time>"2021-05-18T00:00:00Z" OR
              update_time>"2020-05-18T00:00:00Z"`` PipelineJobs created or
              updated after 2020-05-18 00:00:00 UTC. -  ``labels.env = "prod"``
        ...
        page_token (str): The standard list page token. Typically obtained via
          ...
          [PipelineService.ListPipelineJobs][google.cloud.aiplatform.v1beta1.PipelineService.ListPipelineJobs]
          call.
        order_by (str): A comma-separated list of fields to order by. The
          default sort order is in ascending order. Use "desc" after a field
          name for descending. You can have multiple order_by fields provided
          e.g. "create_time desc, end_time", "end_time, start_time, update_time"
          For example, using "create_time desc, end_time" will order results by
          create time in descending order, and if there are multiple jobs having
          the same create time, order them by the end time in ascending order.
          if order_by is not specified, it will order by default order is create
          time in descending order. Supported
            fields:  -  ``create_time`` -  ``update_time`` -  ``end_time`` -
              ``start_time``
  """
...
class ListPipelineJobsResponse(proto.Message):
  r"""Response message for
    ...
    Attributes:
        pipeline_jobs
          (Sequence[google.cloud.aiplatform_v1beta1.types.PipelineJob]): List of
          PipelineJobs in the requested page.
        next_page_token (str): A token to retrieve the next page of results.
          Pass to
