...
class ModelMonitoringConfig(proto.Message):
  r"""Next ID: 5
    ...
    Attributes:
        objective_configs
          (Sequence[google.cloud.aiplatform_v1beta1.types.ModelMonitoringObjectiveConfig]):
          Model monitoring objective config.
        alert_config
          (google.cloud.aiplatform_v1beta1.types.ModelMonitoringAlertConfig):
          Model monitoring alert config.
        analysis_instance_schema_uri (str): YAML schema file uri in Cloud
          Storage describing the format of a single instance that you want
          Tensorflow Data Validation (TFDV) to analyze. If there are any data
          type differences between predict instance and TFDV instance, this
          field can be used to override the schema. For models trained with
          Vertex AI, this field must be set as all the fields in predict
...
class ModelMonitoringObjectiveConfig(proto.Message):
  ...
  class TrainingDataset(proto.Message):
    r"""Training Dataset information.
        ...
        Attributes:
            dataset (str): The resource name of the Dataset used to train this
              Model.  This field is a member of `oneof`_ ``data_source``.
            gcs_source (google.cloud.aiplatform_v1beta1.types.GcsSource): The
              Google Cloud Storage uri of the unmanaged Dataset used to train
              this Model.  This field is a member of `oneof`_ ``data_source``.
            bigquery_source
              (google.cloud.aiplatform_v1beta1.types.BigQuerySource): The
              BigQuery table of the unmanaged Dataset used to train this Model.
              This field is a member of `oneof`_ ``data_source``.
            data_format (str): Data format of the dataset, only applicable if
              the input is from Google Cloud Storage. The possible formats are:
              "tf-record" The source file is a TFRecord file.  "csv" The source
              file is a CSV file. "jsonl" The source file is a JSONL file.
            target_field (str): The target field name the model is to predict.
              This field will be excluded when doing Predict and (or) Explain
              for the training data.
            logging_sampling_strategy
              (google.cloud.aiplatform_v1beta1.types.SamplingStrategy): Strategy
              to sample data from Training Dataset. If not set, we process the
              whole dataset.
    """
  ...
  class TrainingPredictionSkewDetectionConfig(proto.Message):
    r"""The config for Training & Prediction data skew detection.
    ...
    It
        ...
        Attributes:
            skew_thresholds (Mapping[str,
              google.cloud.aiplatform_v1beta1.types.ThresholdConfig]): Key is
              the feature name and value is the threshold. If a feature needs to
              be monitored for skew, a value threshold must be configured for
              that feature. The threshold here is against feature distribution
              distance between the training and prediction feature.
            attribution_score_skew_thresholds (Mapping[str,
              google.cloud.aiplatform_v1beta1.types.ThresholdConfig]): Key is
              the feature name and value is the threshold. The threshold here is
              against attribution score distance between the training and
              prediction feature.
            default_skew_threshold
              (google.cloud.aiplatform_v1beta1.types.ThresholdConfig): Skew
              anomaly detection threshold used by all features. When the
  ...
  class PredictionDriftDetectionConfig(proto.Message):
    r"""The config for Prediction data drift detection.
        ...
        Attributes:
            drift_thresholds (Mapping[str,
              google.cloud.aiplatform_v1beta1.types.ThresholdConfig]): Key is
              the feature name and value is the threshold. If a feature needs to
              be monitored for drift, a value threshold must be configured for
              that feature. The threshold here is against feature distribution
              distance between different time windws.
            attribution_score_drift_thresholds (Mapping[str,
              google.cloud.aiplatform_v1beta1.types.ThresholdConfig]): Key is
              the feature name and value is the threshold. The threshold here is
              against attribution score distance between different time windows.
            default_drift_threshold
              (google.cloud.aiplatform_v1beta1.types.ThresholdConfig): Drift
              anomaly detection threshold used by all features. When the
              per-feature thresholds are not set, this field can be used to
              specify a threshold for all features.
    """
  ...
  class ExplanationConfig(proto.Message):
    r"""The config for integrating with Vertex Explainable AI.

    Only
        applicable if the Model has explanation_spec populated.

        Attributes:
            enable_feature_attributes (bool): If want to analyze the Vertex
              Explainable AI feature attribute scores or not. If set to true,
              Vertex AI will log the feature attributions from explain response
              and do the skew/drift detection for them.
      ...
      r"""Output from
            ...
            Attributes:
                gcs (google.cloud.aiplatform_v1beta1.types.GcsDestination):
                  Cloud Storage location for BatchExplain output.  This field is
                  a member of `oneof`_ ``destination``.
                bigquery
                  (google.cloud.aiplatform_v1beta1.types.BigQueryDestination):
                  BigQuery location for BatchExplain output.  This field is a
                ...
                prediction_format
                  ...
                  The storage format of the predictions generated
                  BatchPrediction job.
      """
...
class ModelMonitoringAlertConfig(proto.Message):
  r"""Next ID: 3
    ...
    Attributes:
        ...
        enable_logging (bool): Dump the anomalies to Cloud Logging. The
          anomalies will be put to json payload encoded from proto
  ...
  class EmailAlertConfig(proto.Message):
    r"""The config for email alert.
        ...
        Attributes:
            user_emails (Sequence[str]): The email addresses to send the alert.
    """
...
class ThresholdConfig(proto.Message):
  r"""The config for feature monitoring threshold.
    ...
    Attributes:
        value (float): Specify a threshold value that can trigger the alert. If
          this threshold config is for feature distribution distance:   1. For
          ...
          is calculated by      Jensen–Shannon divergence. Each feature must
          have a non-zero threshold if they need to be monitored. Otherwise no
          alert will be triggered for that feature.  This field is a member of
          `oneof`_ ``threshold``.
  """
...
class SamplingStrategy(proto.Message):
  ...
  class RandomSampleConfig(proto.Message):
    r"""Requests are randomly selected.
        ...
        Attributes:
            sample_rate (float): Sample rate (0, 1]
    """
