...
class FeatureStatsAnomaly(proto.Message):
  r"""Stats and Anomaly generated at specific timestamp for specific
    ...
    Attributes:
        score (float): Feature importance score, only populated when
          cross-feature monitoring is enabled. For now only used to represent
        ...
        stats_uri (str): Path of the stats file for current feature values in
          Cloud Storage bucket. Format:
            ...
            gs://monitoring_bucket/feature_name/stats. Stats are stored as
              ...
              <https://github.com/tensorflow/metadata/blob/master/tensorflow_metadata/proto/v0/statistics.proto>`__.
        anomaly_uri (str): Path of the anomaly file for current feature values
          in Cloud Storage bucket. Format:
        ...
        distribution_deviation (float): Deviation from the current stats to
          baseline stats.   1. For categorical feature, the distribution
          distance is calculated by L-inifinity norm. 2. For numerical feature,
          the distribution distance is calculated by      Jensen–Shannon
          divergence.
        anomaly_detection_threshold (float): This is the threshold used when
          detecting anomalies. The threshold can be changed by user, so this one
          might be different from
          [ThresholdConfig.value][google.cloud.aiplatform.v1beta1.ThresholdConfig.value].
        start_time (google.protobuf.timestamp_pb2.Timestamp): The start
          timestamp of window where stats were generated. For objectives where
          time window doesn't make sense (e.g. Featurestore Snapshot
          Monitoring), start_time is only used to indicate the monitoring
          intervals, so it always equals to (end_time - monitoring_interval).
        end_time (google.protobuf.timestamp_pb2.Timestamp): The end timestamp of
          window where stats were generated. For objectives where time window
          doesn't make sense (e.g. Featurestore Snapshot Monitoring), end_time
          indicates the timestamp of the data used to generate stats (e.g.
