...
class CustomJob(proto.Message):
  r"""Represents a job that runs custom workloads such as a Docker
    ...
    Attributes:
        name (str): Output only. Resource name of a CustomJob.
        display_name (str): Required. The display name of the CustomJob. The
        ...
        job_spec (google.cloud.aiplatform_v1beta1.types.CustomJobSpec):
          Required. Job spec.
        state (google.cloud.aiplatform_v1beta1.types.JobState): Output only. The
          detailed state of the job.
        create_time (google.protobuf.timestamp_pb2.Timestamp): Output only. Time
          when the CustomJob was created.
        start_time (google.protobuf.timestamp_pb2.Timestamp): Output only. Time
          when the CustomJob for the first time entered the
          ``JOB_STATE_RUNNING`` state.
        end_time (google.protobuf.timestamp_pb2.Timestamp): Output only. Time
          when the CustomJob entered any of the following states:
          ``JOB_STATE_SUCCEEDED``, ``JOB_STATE_FAILED``,
          ``JOB_STATE_CANCELLED``.
        update_time (google.protobuf.timestamp_pb2.Timestamp): Output only. Time
          when the CustomJob was most recently updated.
        error (google.rpc.status_pb2.Status): Output only. Only populated when
          job's state is ``JOB_STATE_FAILED`` or ``JOB_STATE_CANCELLED``.
        labels (Mapping[str, str]): The labels with user-defined metadata to
          organize CustomJobs. Label keys and values can be no longer than 64
        ...
        web_access_uris (Mapping[str, str]): Output only. URIs for accessing
          `interactive shells
          <https://cloud.google.com/vertex-ai/docs/training/monitor-debug-interactive-shell>`__
          (one URI for each training node). Only available if
          [job_spec.enable_web_access][google.cloud.aiplatform.v1beta1.CustomJobSpec.enable_web_access]
          is ``true``.  The keys are names of each node in the training job; for
          example, ``workerpool0-0`` for the primary node, ``workerpool1-0`` for
          the first node in the second worker pool, and ``workerpool1-1`` for
          the second node in the second worker pool.  The values are the URIs
...
class CustomJobSpec(proto.Message):
  r"""Represents the spec of a CustomJob.
    ...
    Attributes:
        worker_pool_specs
          (Sequence[google.cloud.aiplatform_v1beta1.types.WorkerPoolSpec]):
        ...
        service_account (str): Specifies the service account for workload run-as
          ...
          Agent
          <https://cloud.google.com/vertex-ai/docs/general/access-control#service-agents>`__
          for the CustomJob's project is used.
        network (str): The full name of the Compute Engine `network
          </compute/docs/networks-and-firewalls#networks>`__ to which the Job
          should be peered. For example,
          ``projects/12345/global/networks/myVPC``. `Format
          </compute/docs/reference/rest/v1/networks/insert>`__ is of the form
          ...
          <https://cloud.google.com/vertex-ai/docs/general/vpc-peering>`__.  If
          this field is left unspecified, the job is not peered with any
          network.
        reserved_ip_ranges (Sequence[str]): Optional. A list of names for the
          reserved ip ranges under the VPC network that can be used for this
          job.  If set, we will deploy the job within the provided ip ranges.
        ...
        base_output_directory
          ...
          Storage location to store the output of this CustomJob or
          HyperparameterTuningJob. For HyperparameterTuningJob, the
          baseOutputDirectory of each child CustomJob backing a Trial is set to
          a subdirectory of name [id][google.cloud.aiplatform.v1beta1.Trial.id]
          under its parent HyperparameterTuningJob's baseOutputDirectory.  The
          following Vertex AI environment variables will be passed to containers
          or python modules when this field is set:  For CustomJob:  -
          AIP_MODEL_DIR = ``<base_output_directory>/model/`` -
          AIP_CHECKPOINT_DIR = ``<base_output_directory>/checkpoints/`` -
          AIP_TENSORBOARD_LOG_DIR = ``<base_output_directory>/logs/``  For
          CustomJob backing a Trial of HyperparameterTuningJob:  -
          AIP_MODEL_DIR = ``<base_output_directory>/<trial_id>/model/`` -
          AIP_CHECKPOINT_DIR =
          ``<base_output_directory>/<trial_id>/checkpoints/`` -
          AIP_TENSORBOARD_LOG_DIR = ``<base_output_directory>/<trial_id>/logs/``
        tensorboard (str): Optional. The name of a Vertex AI
        ...
        enable_web_access (bool): Optional. Whether you want Vertex AI to enable
          `interactive shell access
          <https://cloud.google.com/vertex-ai/docs/training/monitor-debug-interactive-shell>`__
          to training containers.  If set to ``true``, you can access
          interactive shells at the URIs given by
  ...
  r"""Represents the spec of a worker pool in a job.
    ...
    Attributes:
        container_spec (google.cloud.aiplatform_v1beta1.types.ContainerSpec):
          The custom container task.  This field is a member of `oneof`_
          ``task``.
        python_package_spec
          (google.cloud.aiplatform_v1beta1.types.PythonPackageSpec): The Python
          packaged task.  This field is a member of `oneof`_ ``task``.
        machine_spec (google.cloud.aiplatform_v1beta1.types.MachineSpec):
          Optional. Immutable. The specification of a single machine.
        replica_count (int): Optional. The number of worker replicas to use for
          this worker pool.
        nfs_mounts (Sequence[google.cloud.aiplatform_v1beta1.types.NfsMount]):
          Optional. List of NFS mount spec.
        disk_spec (google.cloud.aiplatform_v1beta1.types.DiskSpec): Disk spec.
  """
...
class ContainerSpec(proto.Message):
  r"""The spec of a Container.
    ...
    Attributes:
        image_uri (str): Required. The URI of a container image in the Container
          Registry that is to be run on each worker replica.
        command (Sequence[str]): The command to be invoked when the container is
          started. It overrides the entrypoint instruction in Dockerfile when
          provided.
        args (Sequence[str]): The arguments to be passed when starting the
          container.
        env (Sequence[google.cloud.aiplatform_v1beta1.types.EnvVar]):
          Environment variables to be passed to the container. Maximum limit is
          100.
  """
...
class PythonPackageSpec(proto.Message):
  r"""The spec of a Python packaged code.
    ...
    Attributes:
        executor_image_uri (str): Required. The URI of a container image in
          Artifact Registry that will run the provided Python package. Vertex AI
          ...
          <https://cloud.google.com/vertex-ai/docs/training/pre-built-containers>`__.
          You must use an image from this list.
        package_uris (Sequence[str]): Required. The Google Cloud Storage
          location of the Python package files which are the training program
          and its dependent packages. The maximum number of package URIs is 100.
        python_module (str): Required. The Python module name to run after
          installing the packages.
        args (Sequence[str]): Command line arguments to be passed to the Python
          task.
        env (Sequence[google.cloud.aiplatform_v1beta1.types.EnvVar]):
          Environment variables to be passed to the python module. Maximum limit
          is 100.
  """
...
class Scheduling(proto.Message):
  r"""All parameters related to queuing and scheduling of custom
    ...
    Attributes:
        timeout (google.protobuf.duration_pb2.Duration): The maximum job running
          time. The default is 7 days.
        restart_job_on_worker_restart (bool): Restarts the entire CustomJob if a
          worker gets restarted. This feature can be used by distributed
