...
class CompletionStats(proto.Message):
  r"""Success and error statistics of processing multiple entities
    ...
    Attributes:
        successful_count (int): Output only. The number of entities that had
          been processed successfully.
        failed_count (int): Output only. The number of entities for which any
          error was encountered.
        incomplete_count (int): Output only. In cases when enough errors are
        ...
        successful_forecast_point_count (int): Output only. The number of the
