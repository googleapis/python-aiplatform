...
class ListTrainingPipelinesPager:
  """A pager for iterating through ``list_training_pipelines`` requests.
    ...
    :class:`google.cloud.aiplatform_v1.types.ListTrainingPipelinesResponse`
    object, and
...
class ListTrainingPipelinesAsyncPager:
  ...
  @property
  async def pages(
      self,) -> AsyncIterator[pipeline_service.ListTrainingPipelinesResponse]:
    yield self._response
    while self._response.next_page_token:
      self._request.page_token = self._response.next_page_token
...
class ListPipelineJobsPager:
  """A pager for iterating through ``list_pipeline_jobs`` requests.
    ...
    This class thinly wraps an initial
    :class:`google.cloud.aiplatform_v1.types.ListPipelineJobsResponse` object,
    and
    provides an ``__iter__`` method to iterate through its
    ``pipeline_jobs`` field.
    ...
    All the usual
    :class:`google.cloud.aiplatform_v1.types.ListPipelineJobsResponse`
    attributes are available on the pager. If multiple requests are made, only
    the most recent response is retained, and thus used for attribute lookup.
    """
    ...
    """Instantiate the pager.
        ...
        Args:
            ...
            request (google.cloud.aiplatform_v1.types.ListPipelineJobsRequest):
              The initial request object. response
              (google.cloud.aiplatform_v1.types.ListPipelineJobsResponse): The
...
class ListPipelineJobsAsyncPager:
  ...
  @property
  async def pages(
      self) -> AsyncIterator[pipeline_service.ListPipelineJobsResponse]:
    yield self._response
    while self._response.next_page_token:
      self._request.page_token = self._response.next_page_token
      self._response = await self._method(
