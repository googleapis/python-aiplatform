...
_SCALE_TYPE_MAP = {
    "linear":
        gca_study_compat.StudySpec.ParameterSpec.ScaleType.UNIT_LINEAR_SCALE,
    "log":
        gca_study_compat.StudySpec.ParameterSpec.ScaleType.UNIT_LOG_SCALE,
    "reverse_log":
        gca_study_compat.StudySpec.ParameterSpec.ScaleType
...
class _ParameterSpec(metaclass=abc.ABCMeta):
  ...
  def _proto_parameter_value_spec(self) -> proto.Message:
    ...
    for self_attr_key, proto_attr_key in self._parameter_value_map:
      setattr(proto_parameter_value_spec, proto_attr_key,
              getattr(self, self_attr_key))
    return proto_parameter_value_spec

  def _to_parameter_spec(
      self, parameter_id: str) -> gca_study_compat.StudySpec.ParameterSpec:
...
class DoubleParameterSpec(_ParameterSpec):
  ...
  def __init__(
    ...
    """
        ...
        Args:
            min (float): Required. Inclusive minimum value of the parameter.
            max (float): Required. Inclusive maximum value of the parameter.
            scale (str): Required. The type of scaling that should be applied to
              this parameter.
...
class CategoricalParameterSpec(_ParameterSpec):
  ...
  def __init__(
    ...
    """Value specification for a parameter in ``CATEGORICAL`` type.
        ...
        Args:
            values (Sequence[str]): Required. The list of possible categories.
    """
