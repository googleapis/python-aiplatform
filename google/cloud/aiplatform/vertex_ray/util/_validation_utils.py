...
def get_image_uri(ray_version, python_version, enable_cuda):
    ...
    if python_version not in SUPPORTED_RAY_VERSIONS_FROM_PYTHON_VERSIONS:
        raise ValueError("[Ray on Vertex AI]: The supported Python versions are 3.10 or 3.11.")
...
def get_versions_from_image_uri(image_uri):
    ...
    if py_version in SUPPORTED_RAY_VERSIONS_FROM_PYTHON_VERSIONS and ray_version in SUPPORTED_RAY_VERSIONS_FROM_PYTHON_VERSIONS[py_version]:
