...
class _ExperimentRow:
  """Class for representing a run row in an Experiments Dataframe.
    ...
    Attributes:
        params (Dict[str, Union[float, int, str]]): Optional. The parameters of
          this run.
        metrics (Dict[str, Union[float, int, str]]): Optional. The metrics of
          this run.
        time_series_metrics (Dict[str, float]): Optional. The latest time series
          metrics of this run.
...
class Experiment:
  ...
  def __init__(
    ...
    """
        ...
        Args:
            experiment_name (str): Required. The name or resource name of this
              experiment.  Resource name is of the format:
              projects/123/locations/us-central1/experiments/my-experiment
  ...
  def _validate_experiment_context(experiment_context: context._Context):
    """Validates this context is an experiment context.
        ...
        Raises:
            ValueError: If Metadata context is not an experiment context or a
  ...
  def create(
    ...
    """Creates a new experiment in Vertex AI Experiments.
        ...
        ```
        my_experiment = aiplatform.Experiment.create('my-experiment',
  ...
  def get_or_create(
    ...
    """Gets experiment if one exists with this experiment_name in Vertex AI Experiments.
        ...
        ```
        my_experiment = aiplatform.Experiment.get_or_create('my-experiment',
        description='my description')
        ```
        ...
        Args:
            ...
            description (str): Optional. Describes this experiment's purpose.
            project (str): Optional. Project where this experiment will be
              retrieved from or created. Overrides project set in
              aiplatform.init.
            location (str): Optional. Location where this experiment will be
              retrieved from or created. Overrides location set in
              aiplatform.init.
            credentials (auth_credentials.Credentials): Optional. Custom
              credentials used to retrieve or create this experiment. Overrides
              credentials set in aiplatform.init.
  ...
  def list(
    ...
    """List all Vertex AI Experiments in the given project.
        ...
        Args:
            project (str): Optional. Project to list these experiments from.
              Overrides project set in aiplatform.init.
            location (str): Optional. Location to list these experiments from.
              Overrides location set in aiplatform.init.
            credentials (auth_credentials.Credentials): Optional. Custom
              credentials to list these experiments. Overrides credentials set
              in aiplatform.init.
  ...
  def delete(self, *, delete_backing_tensorboard_runs: bool = False):
    """Deletes this experiment all the experiment runs under this experiment
        ...
        Does not delete Pipeline runs, Artifacts, or Executions associated to
        this experiment
        ...
        Args:
            delete_backing_tensorboard_runs (bool): Optional. If True will also
              delete the Tensorboard Runs associated to the experiment runs
              under this experiment that we used to store time series metrics.
    """

    experiment_runs = _SUPPORTED_LOGGABLE_RESOURCES[context._Context][
        constants.SYSTEM_EXPERIMENT_RUN].list(experiment=self)
    ...
    for metadata_context in contexts:
      row_dict = (
          _SUPPORTED_LOGGABLE_RESOURCES[context._Context][
              metadata_context.schema_title]._query_experiment_row(
                  metadata_context).to_dict())
    ...
    for metadata_execution in executions:
      row_dict = (
          _SUPPORTED_LOGGABLE_RESOURCES[execution.Execution][
              metadata_execution.schema_title]._query_experiment_row(
                  metadata_execution).to_dict())
      row_dict.update({"experiment_name": self.name})
      rows.append(row_dict)
  ...
  def _lookup_backing_tensorboard(
      self) -> Optional[tensorboard_resource.Tensorboard]:
  ...
  def assign_backing_tensorboard(
    ...
    """Assigns tensorboard as backing tensorboard to support time series metrics logging.
        ...
        Args:
            tensorboard (Union[aiplatform.Tensorboard, str]): Required.
              Tensorboard resource or resource name to associate to this
              experiment.

        Raises:
            ValueError: If this experiment already has a previously set backing
            tensorboard resource.
            ValueError: If Tensorboard is not in same project and location as
            this experiment.
        """
    ...
    if backing_tensorboard:
      tensorboard_resource_name = (
          tensorboard
          if isinstance(tensorboard, str) else tensorboard.resource_name)
    ...
    self._metadata_context.update(metadata={
        constants._BACKING_TENSORBOARD_RESOURCE_KEY: tensorboard.resource_name
    })

  def _log_experiment_loggable(self,
    ...
    """Associates a Vertex resource that can be logged to an Experiment as run of this experiment.
        ...
        Args:
            experiment_loggable (_ExperimentLoggable): A Vertex Resource that
              can be logged to an Experiment directly.
    """
...
class _ExperimentLoggableSchema(NamedTuple):
  ...
  title: str
  type: Union[Type[context._Context],
              Type[execution.Execution]] = context._Context
...
class _ExperimentLoggable(abc.ABC):
  ...
  def __init_subclass__(
      cls, *, experiment_loggable_schemas: Tuple[_ExperimentLoggableSchema],
      **kwargs):
    """Register the metadata_schema for the subclass so Experiment can use it to retrieve the associated types.
        ...
        Args:
            experiment_loggable_schemas: Tuple of the schema_title and type
              pairs that represent this resource. Note that a single item in the
              tuple will be most common. Currently only experiment run has
              multiple representation for backwards compatibility. Almost all
              schemas should be Contexts and Execution is currently only
              supported for backwards compatibility of experiment runs.
    """
  ...
  def _query_experiment_row(
      cls, node: Union[context._Context,
                       execution.Execution]) -> _ExperimentRow:
  ...
  def _validate_experiment(self, experiment: Union[str, Experiment]):
    """Validates experiment is accessible.

    Can be used by subclass to throw before creating the intended
    resource.

        Args:
            experiment (Union[str, Experiment]): The experiment that this
              resource will be associated to.

        Raises:
            RuntimeError: If service raises any exception when trying to access
            this experiment.
            ValueError: If resource project or location do not match experiment
            project or location.
        """
  ...
  def _associate_to_experiment(self, experiment: Union[str, Experiment]):
    """Associates this resource to the provided Experiment.
        ...
        Args:
            experiment (Union[str, Experiment]): Required. Experiment name or
              experiment instance.

        Raises:
            RuntimeError: If Metadata service cannot associate resource to
            Experiment.
        """
    experiment_name = experiment if isinstance(experiment,
                                               str) else experiment.name
    _LOGGER.info("Associating %s to Experiment: %s" %
                 (self.resource_name, experiment_name))
...
_SUPPORTED_LOGGABLE_RESOURCES: Dict[Union[Type[context._Context],
