...
class _LegacyExperimentService:
  ...
  def get_pipeline_df(pipeline: str) -> "pd.DataFrame":  # noqa: F821
    ...
    pipeline_resource_name = (
        _LegacyExperimentService._get_experiment_or_pipeline_resource_name(
            name=pipeline,
            source=source,
            expected_schema=constants.SYSTEM_PIPELINE))
  ...
  @staticmethod
  def _get_experiment_or_pipeline_resource_name(name: str, source: str,
                                                expected_schema: str) -> str:
    """Get the full resource name of the Context representing an Experiment or Pipeline.

        Args:
            name (str): Name of the Experiment or Pipeline.
            source (str): Identify whether the this is an Experiment or a
              Pipeline.
            expected_schema (str): expected_schema identifies the expected
              schema used for Experiment or Pipeline.
  ...
  @staticmethod
  def _query_runs_to_data_frame(context_id: str, context_resource_name: str,
                                source: str) -> "pd.DataFrame":  # noqa: F821
    """Get metrics and parameters associated with a given Context into a Dataframe.

        Args:
            context_id (str): Name of the Experiment or Pipeline.
            context_resource_name (str): Full resource name of the Context
              associated with an Experiment or Pipeline.
            source (str): Identify whether the this is an Experiment or a
              Pipeline.
  ...
  def _execution_to_column_named_metadata(
      metadata_type: str,
      metadata: Dict,
      filter_prefix: Optional[str] = None) -> Dict[str, Union[int, float, str]]:
    """Returns a dict of the Execution/Artifact metadata with column names.
        ...
        Args:
          ...
          filter_prefix: Remove this prefix from the key of metadata field.
            Mainly used for removing "input:" from PipelineJob parameter keys
...
class _ExperimentTracker:
  ...
  def set_experiment(
      ...
      *,
      ...
      backing_tensorboard: Optional[Union[
          str, tensorboard_resource.Tensorboard]] = None,
  ):
    """Set the experiment.

    Will retrieve the Experiment if it exists or create one with the
    provided name.

        Args:
            experiment (str): Required. Name of the experiment to set.
            description (str): Optional. Description of an experiment.
              backing_tensorboard Union[str, aiplatform.Tensorboard]: Optional.
              If provided, assigns tensorboard as backing tensorboard to support
  ...
  def start_run(
    ...
    """Start a run to current session.
        ...
        Args: run(str): Required. Name of the run to assign current session
        with. tensorboard Union[str, tensorboard_resource.Tensorboard]:
        Optional. Backing Tensorboard Resource to enable and store time series
        ...
        currently set experiment.
            resume (bool): Whether to resume this run. If False a new run will
            be created.

        Raises:
            ValueError:
                if experiment is not set. Or if run execution or metrics
                artifact is already created
  ...
  def end_run(self,
              state: gapic.Execution.State = gapic.Execution.State.COMPLETE):
  ...
  def log_metrics(self, metrics: Dict[str, Union[float, int, str]]):
    """Log single or multiple Metrics with specified key and value pairs.
        ...
        Args:
            metrics (Dict[str, Union[float, int, str]]): Required. Metrics
              key/value pairs.
    """
  ...
  def get_experiment_df(self,
    ...
    """Returns a Pandas DataFrame of the parameters and metrics associated with one experiment.
        ...
        | experiment_name | run_name      | param.learning_rate |
        metric.accuracy |
        ---------------------------------------------------------------------------
        | exp-1           | run-1         | 0.1                 | 0.9
        |
        ...
        Args:
            experiment (str): Name of the Experiment to filter results. If not
              set, return results of current active experiment.
        ...
        Raise:
            ...
            ValueError if given experiment is not associated with a wrong
            schema.
        """
  ...
  def log(
    ...
    """Log Vertex AI Resources to the current experiment run.
        ...
        Args:
            pipeline_job (pipeline_jobs.PipelineJob): Optional. Vertex
              PipelineJob to associate to this Experiment Run.
    """
  ...
  def log_time_series_metrics(
    ...
    """Logs time series metrics to to this Experiment Run.
        ...
        Requires the experiment or experiment run has a backing Vertex
  ...
  def start_execution(
    ...
    """
        Create and starts a new Metadata Execution or resumes a previously
        created Execution.
        ...
        ```
        with
        aiplatform.start_execution(schema_title='system.ContainerExecution',
        display_name='trainer) as exc:
          exc.assign_input_artifacts([my_artifact])
          model = aiplatform.Artifact.create(uri='gs://my-uri',
        ...
        ```
        with aiplatform.start_execution(resource_id='my-exc', resume=True) as
        exc:
            ...
        ```
        Args:
            schema_title (str): Optional. schema_title identifies the schema
              title used by the Execution. Required if starting a new Execution.
            resource_id (str): Optional. The <resource_id> portion of the
              Execution name with the format. This is globally unique in a
              metadataStore:
            ...
            schema_version (str): Optional. schema_version specifies the version
              used by the Execution. If not set, defaults to use the latest
              version.
            metadata (Dict): Optional. Contains the metadata information that
              will be stored in the Execution.
            description (str): Optional. Describes the purpose of the Execution
              to be created.
            ...
            project (str): Optional. Project used to create this Execution.
              Overrides project set in aiplatform.init.
            location (str): Optional. Location used to create this Execution.
              Overrides location set in aiplatform.init.
            credentials (auth_credentials.Credentials): Optional. Custom
              credentials used to create this Execution. Overrides credentials
        ...
        Raises:
            ...
            ValueError: If creating a new executin and schema_title is not
            provided.
        """

    if (self._experiment_run and
        not self._experiment_run._is_legacy_experiment_run()):
