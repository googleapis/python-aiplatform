...
def _format_experiment_run_resource_id(experiment_name: str,
                                       run_name: str) -> str:
...
def _v1_not_supported(method: Callable) -> Callable:
  """Helpers wrapper for backward compatibility.

  Raises when using an API not support for legacy runs.
  """
...
class ExperimentRun(
  ...
  def __init__(
    ...
    """
        ...
        Args:
            ...
            experiment (Union[experiment_resources.Experiment, str]): Required.
              The name or instance of this experiment.
            project (str): Optional. Project where this experiment run is
              located. Overrides project set in aiplatform.init.
            location (str): Optional. Location where this experiment run is
              located. Overrides location set in aiplatform.init.
            credentials (auth_credentials.Credentials): Optional. Custom
              credentials used to retrieve this experiment run. Overrides
  ...
  def _v1_resolve_experiment_run(self, metadata_args: Dict[str, Any]):
    """Resolves preview Experiment.
        ...
        Args:
            metadata_args (Dict[str, Any): Arguments to pass to Execution
              constructor.
    """
  ...
  def resource_id(self) -> str:
    """The resource ID of this experiment run's Metadata context.
        ...
        ``projects/{project}/locations/{location}/metadataStores/{metadatastore}/contexts/{resource
        ID}``
        """
  ...
  def _get_experiment(
    ...
    """Helper method ot get the experiment by name(str) or instance.
        ...
        Args: experiment(str): Optional. The name of this experiment. Defaults
        to experiment set in aiplatform.init if not provided.
            project (str): Optional. Project where this experiment is located.
            Overrides project set in aiplatform.init.
            location (str): Optional. Location where this experiment is located.
            Overrides location set in aiplatform.init.
            credentials (auth_credentials.Credentials): Optional. Custom
            credentials used to retrieve this experiment. Overrides credentials
            set in aiplatform.init.

        Raises:
            ValueError if experiment is None and experiment has not been set
  ...
  def _is_backing_tensorboard_run_artifact(self,
                                           artifact: artifact.Artifact) -> bool:
    """Helper method to confirm tensorboard run metadata artifact is this run's tensorboard artifact.

        Args:
            artifact (artifact.Artifact): Required. Instance of metadata
              Artifact.

        Returns:
            bool whether the provided artifact is this run's TensorboardRun's
            artifact.
        """
    return all([
  ...
  def _lookup_tensorboard_run_artifact(
      self,) -> Optional[experiment_resources._VertexResourceWithMetadata]:
  ...
  def list(
    ...
    """List the experiment runs for a given aiplatform.Experiment.
        ...
        Args:
            experiment (Union[aiplatform.Experiment, str]): Optional. The
              experiment name or instance to list the experiment run from. If
              not provided, will use the experiment set in aiplatform.init.
            project (str): Optional. Project where this experiment is located.
              Overrides project set in aiplatform.init.
            location (str): Optional. Location where this experiment is located.
              Overrides location set in aiplatform.init.
            credentials (auth_credentials.Credentials): Optional. Custom
              credentials used to retrieve this experiment. Overrides
              credentials set in aiplatform.init.
    ...
    run_executions = execution.Execution.list(
        filter=filter_str, **metadata_args)
    ...
    def _initialize_v1_experiment_run(
        execution: execution.Execution,) -> ExperimentRun:
      this_experiment_run = cls.__new__(cls)
      this_experiment_run._experiment = experiment
    ...
    if run_contexts or run_executions:
      with concurrent.futures.ThreadPoolExecutor(
          max_workers=max([len(run_contexts),
                           len(run_executions)])) as executor:
        submissions = [
  ...
  def _query_experiment_row(
    ...
    """Retrieves the runs metric and parameters into an experiment run row.
        ...
        Args:
            node (Union[context._Context, execution.Execution]): Required.
              Metadata node instance that represents this run.
  ...
  def _get_latest_time_series_metric_columns(
      self) -> Dict[str, Union[float, int]]:
    """Determines the latest step for each time series metric.

        Returns:
            Dictionary mapping time series metric key to the latest step of that
            metric.
        """
  ...
  def _log_pipeline_job(self, pipeline_job: pipeline_jobs.PipelineJob):
    """Associate this PipelineJob's Context to the current ExperimentRun Context as a child context.
        ...
        Args:
            pipeline_job (pipeline_jobs.PipelineJob): Required. The PipelineJob
              to associate.
    """
  ...
  def log(
    ...
    """Log a Vertex Resource to this experiment run.
        ...
        Args:
            pipeline_job (aiplatform.PipelineJob): Optional. A Vertex
              PipelineJob.
    """
    if pipeline_job:
  ...
  def _validate_run_id(run_id: str):
    """Validates the run id
        ...
        Args: run_id(str): Required. The run id to validate.
  ...
  def create(
      ...
      *,
      experiment: Optional[Union[experiment_resources.Experiment, str]] = None,
      tensorboard: Optional[Union[tensorboard_resource.Tensorboard,
                                  str]] = None,
      state: gca_execution.Execution.State = gca_execution.Execution.State
      .RUNNING,
      project: Optional[str] = None,
      location: Optional[str] = None,
      credentials: Optional[auth_credentials.Credentials] = None,
    ...
    """Creates a new experiment run in Vertex AI Experiments.
        ...
        ```
        my_run = aiplatform.ExperimentRun.create('my-run',
        experiment='my-experiment')
        ```

        Args:
            run_name (str): Required. The name of this run.
            experiment (Union[aiplatform.Experiment, str]): Optional. The name
              or instance of the experiment to create this run under. If not
              provided, will default to the experiment set in `aiplatform.init`.
            tensorboard (Union[aiplatform.Tensorboard, str]): Optional. The
              resource name or instance of Vertex Tensorbaord to use as the
              backing Tensorboard for time series metric logging. If not
              provided, will default to the the backing tensorboard of parent
              experiment if set. Must be in same project and location as this
              experiment run.
            state (aiplatform.gapic.Execution.State): Optional. The state of
              this run. Defaults to RUNNING.
            project (str): Optional. Project where this experiment will be
              created. Overrides project set in aiplatform.init.
            location (str): Optional. Location where this experiment will be
              created. Overrides location set in aiplatform.init.
            credentials (auth_credentials.Credentials): Optional. Custom
              credentials used to create this experiment. Overrides credentials
              set in aiplatform.init.
  ...
  def _assign_backing_tensorboard(
    ...
    with experiment_resources._SetLoggerLevel(resource):
      tensorboard_run_metadata_artifact = artifact.Artifact._create(
          ...
          schema_title=constants._TENSORBOARD_RUN_REFERENCE_ARTIFACT
          .schema_title,
          schema_version=constants._TENSORBOARD_RUN_REFERENCE_ARTIFACT
          .schema_version,
          state=gca_artifact.Artifact.State.LIVE,
      )

    self._metadata_node.add_artifacts_and_executions(artifact_resource_names=[
        tensorboard_run_metadata_artifact.resource_name
    ])
  ...
  def assign_backing_tensorboard(
    ...
    """Assigns tensorboard as backing tensorboard to support timeseries metrics logging for this run.
        ...
        Args:
            tensorboard (Union[aiplatform.Tensorboard, str]): Required.
              Tensorboard instance or resource name.
    """
  ...
  def log_time_series_metrics(
    ...
    """Logs time series metrics to backing TensorboardRun of this Experiment Run.
        ...
        Args:
            metrics (Dict[str, Union[str, float]]): Required. Dictionary of
              where keys are metric names and values are metric values.
            step (int): Optional. Step index of this data point within the run.
              If not provided, the latest step amongst all time series metrics
              already logged will be used.
  ...
  def _soft_create_time_series(self, metric_keys: Set[str]):
    ...
    if any(key not in self._backing_tensorboard_run.resource
           ._time_series_display_name_to_id_mapping for key in metric_keys):
      self._backing_tensorboard_run.resource._sync_time_series_display_name_to_id_mapping(
      )

    for key in metric_keys:
      if (key not in self._backing_tensorboard_run.resource
          ._time_series_display_name_to_id_mapping):
        with experiment_resources._SetLoggerLevel(tensorboard_resource):
  ...
  def log_params(self, params: Dict[str, Union[float, int, str]]):
    """Log single or multiple parameters with specified key value pairs.
        ...
        Args:
            params (Dict[str, Union[float, int, str]]): Required. Parameter
              key/value pairs.
  ...
  def log_metrics(self, metrics: Dict[str, Union[float, int, str]]):
    """Log single or multiple Metrics with specified key and value pairs.
        ...
        Args:
            metrics (Dict[str, Union[float, int]]): Required. Metrics key/value
              pairs.
  ...
  def get_time_series_data_frame(self) -> "pd.DataFrame":  # noqa: F821
    ...
    } for name, ts in data.items() for entry in ts.values).groupby(
        ["step", "wall_time"]).first().reset_index())
  ...
  def __exit__(self, exc_type, exc_value, traceback):
    state = (
        ...
        if exc_type else gca_execution.Execution.State.COMPLETE)
  ...
  def end_run(
    ...
    """Ends this experiment run and sets state to COMPLETE.
        ...
        Args:
            state (aiplatform.gapic.Execution.State): Optional. Override the
              state at the end of run. Defaults to COMPLETE.
    """
  ...
  def delete(self, *, delete_backing_tensorboard_run: bool = False):
    """Deletes this experiment run.
        ...
        Does not delete the executions, artifacts, or resources logged to this
        run.

        Args:
            delete_backing_tensorboard_run (bool): Optional. Whether to delete
              the backing tensorboard run that stores time series metrics for
              this run.
    """
  ...
  def associate_execution(self, execution: execution.Execution):
    """Associate an execution to this experiment run.
        ...
        Args:
            execution (aiplatform.Execution): Execution to associate to this
              run.
    """
  ...
  def _association_wrapper(self, f: Callable[..., Any]) -> Callable[..., Any]:
    """Wraps methods and automatically associates all passed in Artifacts or Executions to this ExperimentRun.
        ...
        This is used to wrap artifact passing methods of Executions so they get
    ...
    def wrapper(*args, **kwargs):
      ...
      for value in [*args, *kwargs.values()]:
        ...
        for item in value:
          ...
          elif artifact._VertexResourceArtifactResolver.supports_metadata(item):
            artifacts.append(
                artifact._VertexResourceArtifactResolver
                .resolve_or_create_resource_artifact(item))
