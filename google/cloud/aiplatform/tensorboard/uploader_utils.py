...
class OnePlatformResourceManager(object):
  ...
  def __init__(self, experiment_resource_name: str,
               api: TensorboardServiceClient):
    """Constructor for OnePlatformResourceManager.

        Args:
            experiment_resource_name (str): Required. The resource id for the
  ...
  def batch_create_runs(
      self, run_names: List[str]) -> List[tensorboard_run.TensorboardRun]:
  ...
  def batch_create_time_series(
      self,
      run_tag_name_to_time_series: Dict[Tuple[
          str, str], tensorboard_time_series.TensorboardTimeSeries],
  ) -> List[tensorboard_time_series.TensorboardTimeSeries]:
    """Batch creates TensorboardTimeSeries.

        Args:
            run_tag_name_to_time_series: a dictionary of (run_name, tag_name) to
              TensorboardTimeSeries proto, containing the TensorboardTimeSeries
              to create.
    ...
    run_tag_name_to_time_series_entries = list(
        run_tag_name_to_time_series.items())
    ...
    for i in range(0, len(run_tag_name_to_time_series_entries), batch_size):
      ...
      self._run_tag_name_to_time_series_name.update({(
          run_resource_name_to_run_name[ts.name[:ts.name.index("/timeSeries")]],
          ts.display_name,
      ): ts.name for ts in time_series})
  ...
  def _create_or_get_run_resource(
      self, run_name: str) -> tensorboard_run.TensorboardRun:
    """Creates a new run resource in current tensorboard experiment resource.

        Args:
            run_name (str): Required. The display name of this run.
  ...
  def get_time_series_resource_name(
    ...
    """
        ...
        Args:
            ...
            time_series_resource_creator
              (tensorboard_time_series.TensorboardTimeSeries): Required. A
              constructor used for creating the time series on One Platform.
    ...
    if (run_name, tag_name) not in self._run_tag_name_to_time_series_name:
      ...
      self._run_tag_name_to_time_series_name[(run_name,
                                              tag_name)] = time_series.name
  ...
  def _create_or_get_time_series(
    ...
    """
        ...
        Args:
            tag_name (str): Required. The tag name of the time series in the
              Tensorboard log dir.
            time_series_resource_creator (Callable[[],
              tensorboard_time_series.TensorboardTimeSeries): Required. A
              callable that produces a TimeSeries for creation.

        Returns:
            time_series (tensorboard_time_series.TensorboardTimeSeries):
                A created or existing
                tensorboard_time_series.TensorboardTimeSeries.

        Raises:
            exceptions.InvalidArgument:
                Invalid run_resource_name, tag_name, or
                time_series_resource_creator.
            ExistingResourceNotFoundError:
    ...
    try:
      ...
      if "already exist" in e.message:
        ...
        if not time_series:
          raise ExistingResourceNotFoundError(
              "Could not find time series resource with display name: {}"
              .format(tag_name))

        if num != 1:
          raise ValueError(
              "More than one time series resource found with display_name: {}"
              .format(tag_name))
      else:
        raise
    return time_series
...
class TimeSeriesResourceManager(object):
  ...
  def __init__(self, run_resource_id: str, api: TensorboardServiceClient):
    """Constructor for TimeSeriesResourceManager.
        ...
        Args:
            run_resource_id (str): Required. The resource id for the run with
              the following format.
              projects/{project}/locations/{location}/tensorboards/{tensorboard}/experiments/{experiment}/runs/{run}
            api (TensorboardServiceClient): Required. A TensorboardServiceStub.
    """
  ...
  def get_or_create(
    ...
    """
        ...
        Raises:
            exceptions.InvalidArgument:
                The tag_name or time_series_resource_creator is an invalid
                argument
...
def get_source_bucket(logdir: str) -> Optional[storage.Bucket]:
  """Returns a storage bucket object given a log directory.
    ...
    Args:
        logdir (str): Required. Path of the log directory.
  ...
  """Context manager to log request size and duration.
    ...
    Args:
        request (tensorboard_service.WriteTensorboardRunDataRequest): Required.
          A request object that provides the size of the request.
