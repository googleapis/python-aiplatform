...
#
"""Upload projector sessions to Vertex AI Tensorboard."""
...
class ProjectorRequestSender(uploader_utils.RequestSender):
    """Helper class for building requests for the projector plugin.

    While the projector plugin does create event files when a projector run is performed
    ...
    projector directories and files.
    ...
    PROJECTOR_PATH = "plugins/projector"
    ...
    def __init__(
        ...
        """Constructs ProjectorRequestSender for the given experiment resource.
        ...
        self._run_to_projector_loaders: Dict[str, _ProjectorSessionLoader] = {}
    ...
    def _is_valid_event(self, run_name: str) -> bool:
        """Determines whether a valid projector session has occurred.

        Projector events are determined by whether a corresponding directory has
        been created for the projector plugin.
        ...
        Returns:
            True if is a valid projector plugin event, False otherwise.
        """

        return tf.io.gfile.isdir(self._projector_dir(run_name))

    def _projector_dir(self, run_name: str) -> str:
        """Converts run name to full projector path.
        ...
        """
        if run_name is None:
            return os.path.join(self._logdir, self.PROJECTOR_PATH)
        return os.path.join(self._logdir, run_name, self.PROJECTOR_PATH)
    ...
    def send_request(self, run_name: str):
        ...
        if not self._is_valid_event(run_name):
            logger.debug("No such projector run for %s", run_name)
            return

        # Create a projector loader if one is not created.
        # This will store any new runs that occur within the training.
        if run_name not in self._run_to_projector_loaders:
            self._run_to_projector_loaders[run_name] = _ProjectorSessionLoader(
                self._projector_dir(run_name)
            )
        ...
        # A training run can have multiple projector sessions.
        for prof_session, files in self._run_to_projector_loaders[
            run_name
...
class _ProjectorSessionLoader(object):
    """Loader for a projector session within a training run.

    The term 'session' refers to an instance of a projector, where
    one may have multiple projector sessions under a training run.
    """

    # A regular expression for the naming of a profiling path.
    PROF_PATH_REGEX = r".*\/plugins\/projector\/[0-9]{4}_[0-9]{2}_[0-9]{2}_[0-9]{2}_[0-9]{2}_[0-9]{2}\/?$"
    ...
    def __init__(
        ...
        Args:
            path (str):
                ...
                sessions. Path should end with '/projector/plugin'.
        """
        self._path = path
    ...
    def _path_filter(self, path: str) -> bool:
        ...
        Paths written by projector should be of form:
        /some/path/to/dir/plugins/projector/%Y_%m_%d_%H_%M_%S
    ...
    def _path_to_files(self, prof_session: str, path: str) -> List[str]:
        ...
        Files are generated by the projector and are added to an internal
    ...
    def prof_sessions_to_files(self) -> Generator[Tuple[str, List[str]], None, None]:
        """Map files to a projector session.
