...
class ProfileRequestSender(uploader_utils.RequestSender):
  """Helper class for building requests for the profiler plugin.
    ...
    While the profile plugin does create event files when a profile run is
    performed
    for a new training run, these event files do not contain any values
    like other events do. Instead, the plugin will create subdirectories and
    profiling
  ...
  def __init__(
    ...
    """Constructs ProfileRequestSender for the given experiment resource.
        ...
        Args:
            experiment_resource_name (str): Required. Name of the experiment
              resource of the form:
              projects/{project}/locations/{location}/tensorboards/{tensorboard}/experiments/{experiment}
            api (TensorboardServiceClient): Required. Tensorboard service stub
              used to interact with experiment resource.
            upload_limits (server_info_pb2.UploadLimits): Required. Upload
              limits for for api calls.
            blob_rpc_rate_limiter (util.RateLimiter): Required. A `RateLimiter`
              to use to limit write RPC frequency. Note this limit applies at
              the level of single RPCs in the Scalar and Tensor case, but at the
              level of an entire blob upload in the Blob case-- which may
              require a few preparatory RPCs and a stream of chunks. Note the
              chunk stream is internally rate-limited by backpressure from the
              server, so it is not a concern that we do not explicitly
              rate-limit within the stream here.
            blob_storage_bucket (storage.Bucket): Required. A `storage.Bucket`
              to send all blob files to.
            blob_storage_folder (str): Required. Name of the folder to save blob
              files to within the blob_storage_bucket.
            tracker (upload_tracker.UploadTracker): Required. Upload tracker to
              track information about uploads. logdir (str). Required. The log
              directory for the request sender to search.
            source_bucket (Optional[storage.Bucket]): Optional. The user's
              specified `storage.Bucket` to save events to. If a user is
              uploading from a local directory, this can be None.
    """
  ...
  def _is_valid_event(self, run_name: str) -> bool:
    """Determines whether a valid profile session has occurred.
        ...
        Args:
            run_name (str): Required. String representing the run name.
  ...
  def _profile_dir(self, run_name: str) -> str:
    """Converts run name to full profile path.
        ...
        Args:
            run_name (str): Required. Name of training run.
  ...
  def send_request(self, run_name: str):
    """Accepts run_name and sends an RPC request if an event is detected.
        ...
        Args:
            run_name (str): Required. Name of the training run.
    """
    ...
    if run_name not in self._run_to_file_request_sender:
      self._run_to_file_request_sender[
          run_name] = self._file_request_sender_factory(tb_run)
    ...
    for prof_session, files in self._run_to_profile_loaders[
        run_name].prof_sessions_to_files():
...
class _ProfileSessionLoader(object):
  ...
  def __init__(
    ...
    """Create a loader for profiling sessions with a training run.
        ...
        Args:
            path (str): Required. Path to the training run, which contains one
              or more profiling sessions. Path should end with
              '/profile/plugin'.
    """
    self._path = path
  ...
  def _path_filter(self, path: str) -> bool:
    """Determine which paths we should upload.
        ...
        Args:
            path (str): Required. String representing a full directory path.
  ...
  def _path_to_files(self, prof_session: str, path: str) -> List[str]:
    """Generates files that have not yet been tracked.
        ...
        Args:
            prof_session (str): Required. The profiling session name.
            path (str): Required. Directory of the profiling session.
  ...
  def prof_sessions_to_files(
      self) -> Generator[Tuple[str, List[str]], None, None]:
...
class _FileRequestSender(object):
  """Uploader for file based items.
    ...
    This sender is closely related to the `_BlobRequestSender`, however it
    expects
    file paths instead of blob files, so that data is not directly read in and
    instead
    files are moved between buckets. Additionally, this sender does not take
    event files
    as the other request sender objects do. The sender takes files from either
    local storage
  ...
  def __init__(
    ...
    """Creates a _FileRequestSender object.
        ...
        Args:
            run_resource_id (str): Required. Name of the run resource of the
              form:
              projects/{project}/locations/{location}/tensorboards/{tensorboard}/experiments/{experiment}/runs/{run}
            api (TensorboardServiceClient): Required. TensorboardServiceStub for
              calling various tensorboard services.
            rpc_rate_limiter (util.RateLimiter): Required. A `RateLimiter` to
              use to limit write RPC frequency. Note this limit applies at the
              level of single RPCs in the Scalar and Tensor case, but at the
              level of an entire blob upload in the Blob case-- which may
              require a few preparatory RPCs and a stream of chunks. Note the
              chunk stream is internally rate-limited by backpressure from the
              server, so it is not a concern that we do not explicitly
              rate-limit within the stream here.
            max_blob_request_size (int): Required. Maximum request size to send.
            max_blob_size (int): Required. Maximum size in bytes of the blobs to
              send.
            blob_storage_bucket (storage.Bucket): Required. Bucket to send event
              files to.
            blob_storage_folder (str): Required. The folder to save blob files
              to.
            tracker (upload_tracker.UploadTracker): Required. Track any uploads
              to backend.
            source_bucket (storage.Bucket): Optional. The source bucket to
              upload from. If not set, use local filesystem instead.
    """
  ...
  def add_files(
    ...
    """Attempts to add the given file to the current request.
        ...
        Files are flushed immediately, opposed to some of the other request
        senders.

        Args:
            files (List[str]): Required. The paths of the files to upload.
            tag (str): Required. A unique identifier for the blob sequence.
            plugin (str): Required. Name of the plugin making the request.
            event_timestamp (timestamp.Timestamp): Required. The time the event
              is created.
    """
  ...
  def flush(self):
    ...
    blob_path_prefix = ("{}/{}".format(self._folder, blob_path_prefix)
                        if self._folder else blob_path_prefix)
    sent_blob_ids = []
    ...
    data_point = tensorboard_data.TimeSeriesDataPoint(
        blobs=tensorboard_data.TensorboardBlobSequence(values=[
  ...
  def _file_too_large(self, filename: str) -> bool:
    """Determines if a file is too large to upload.
        ...
        Args:
            filename (str): Required. The filename to check.
  ...
  def _upload(self,
              filename: str,
              blob_path_prefix: Optional[str] = None) -> str:
    """Copies files between either a local directory or a bucket and the tenant bucket.

        Args:
            filename (str): Required. The full path of the file to upload.
            blob_path_prefix (str): Optional. Path prefix for the location to
              store the file.
    ...
    blob_path = ("{}/{}".format(blob_path_prefix, blob_id)
                 if blob_path_prefix else blob_id)
  ...
  def _copy_between_buckets(self, filename: str, blob_path: str):
    """Move files between the user's bucket and the tenant bucket.
        ...
        Args:
            filename (str): Required. Full path of the file to upload.
            blob_path (str): Required. A bucket path to upload the file to.
    """
  ...
  def _upload_from_local(self, filename: str, blob_path: str):
    """Uploads a local file to the tenant bucket.
        ...
        Args:
            filename (str): Required. Full path of the file to upload.
            blob_path (str): Required. A bucket path to upload the file to.a
    """
...
def _prune_empty_time_series_from_blob(
  ...
  """Removes empty time_series from request if there are no blob files.'
    ...
    Args:
        request (tensorboard_service.WriteTensorboardRunDataRequest): Required.
          A write request for blob files.
  """
