...
class TensorBoardUploader(object):
  ...
  def __init__(
    ...
    """Constructs a TensorBoardUploader.
        ...
        Args:
          ...
          rpc_rate_limiter: a `RateLimiter` to use to limit write RPC frequency.
            ...
            Blob case-- which may require a few preparatory RPCs and a stream of
            chunks. Note the chunk stream is internally rate-limited by
          ...
          verbosity: Level of verbosity, an integer. Supported value: 0 - No
            upload statistics is printed. 1 - Print upload statistics while
            uploading data (default).
          one_shot: Once uploading starts, upload only the existing data in the
            logdir and then return immediately, instead of the default behavior
            of continuing to listen for new data in the logdir and upload them
            when it appears.
          event_file_inactive_secs: Age in seconds of last write after which an
            ...
            considered inactive.
          run_name_prefix: If present, all runs created by this invocation will
            have their name prefixed by this value.
    """
    ...
    def active_filter(secs):
      return (not bool(event_file_inactive_secs) or
              secs + event_file_inactive_secs >= time.time())
    ...
    self._logdir_loader = logdir_loader.LogdirLoader(self._logdir,
  ...
  def _create_or_get_experiment(
      self) -> tensorboard_experiment.TensorboardExperiment:
    ...
    try:
      ...
      experiment_name = os.path.join(self._tensorboard_resource_name,
                                     "experiments", self._experiment_name)
  ...
  def _create_additional_senders(
      self) -> Dict[str, uploader_utils.RequestSender]:
  ...
  def start_uploading(self):
    ...
    if self._one_shot and not self._tracker.has_data():
      logger.warning("One-shot mode was used on a logdir (%s) "
    ...
    for (run_name, events) in run_to_events.items():
      ...
      for event in events:
        ...
        for value in event.summary.value:
          ...
          run_tag_name_to_time_series_proto[(
              run_name,
              value.tag)] = tensorboard_time_series.TensorboardTimeSeries(
...
class _BatchedRequestSender(object):
  ...
  def __init__(
    ...
    """Constructs _BatchedRequestSender for the given experiment resource.
        ...
        Args:
          ...
          api: Tensorboard service stub used to interact with experiment
            resource.
          allowed_plugins: The plugins supported by the Tensorboard.gcp
            resource.
  ...
  def get_metadata_and_validate(
    ...
    plugin_name = metadata.plugin_data.plugin_name
    if value.HasField("metadata") and (plugin_name !=
...
class _Dispatcher(object):
  ...
  def __init__(
      ...
      additional_senders: Optional[Dict[str,
                                        uploader_utils.RequestSender]] = None,
  ):
  ...
  def dispatch_requests(self, run_to_events: Dict[str,
                                                  Generator[tf.compat.v1.Event,
                                                            None, None]]):
    """Routes events to the appropriate sender.
        ...
        Args:
          run_to_events: Mapping from run name to generator of
            `tf.compat.v1.Event` values, as returned by
            `LogdirLoader.get_run_events`.
    """
    for (run_name, events) in run_to_events.items():
...
class _BaseBatchedRequestSender(object):
  ...
  def __init__(
    ...
    """Constructor for _BaseBatchedRequestSender.
        ...
        Args:
          experiment_resource_id: The resource id for the experiment with the
            following format
            projects/{project}/locations/{location}/tensorboards/{tensorboard}/experiments/{experiment}
          api: TensorboardServiceStub
          rpc_rate_limiter: until.RateLimiter to limit rate of this request
            sender
    ...
    self._run_to_tag_to_time_series_data: Dict[str, Dict[
        str, tensorboard_data.TimeSeriesData]] = defaultdict(defaultdict)
    self._new_request()
  ...
  def flush(self):
    ...
    for (
      ...
      r = tensorboard_service.WriteTensorboardRunDataRequest(
          tensorboard_run=self._one_platform_resource_manager
          .get_run_resource_name(run_name))
      ...
      with self._get_tracker():
        try:
          ...
          if (hasattr(e, "code") and
              getattr(e, "code")() == grpc.StatusCode.NOT_FOUND):
  ...
  def _create_time_series_data(
    ...
    self._run_to_tag_to_time_series_data[run_name][
        tag_name] = time_series_data_proto
  ...
  @abc.abstractmethod
  def _value_type(
      cls,) -> tensorboard_time_series.TensorboardTimeSeries.ValueType:
    """
  ...
  def _validate(
    ...
    try:
      ...
      raise ValueError(
          ...
          "Original error:\n%s" %
          (value.tag, event.step, event.wall_time, error))
    return True
...
class _ByteBudgetManager(object):
  ...
  def reset(
      self,
      base_request: tensorboard_service.WriteTensorboardExperimentDataRequest):
    ...
    self._byte_budget -= (base_request._pb.ByteSize())  # pylint: disable=protected-access
  ...
  def _create_data_point(
    ...
    m = re.match(
        ...
        self._one_platform_resource_manager.get_time_series_resource_name(
            ...
            lambda: tensorboard_time_series.TensorboardTimeSeries(
                display_name=value.tag,
                value_type=tensorboard_time_series.TensorboardTimeSeries.
                ValueType.BLOB_SEQUENCE,
                plugin_name=metadata.plugin_data.plugin_name,
                plugin_data=metadata.plugin_data.content,
            ),
        ),
...
def _prune_empty_time_series(
  ...
  for (time_series_idx,
       time_series_data) in reversed(list(enumerate(request.time_series_data))):
