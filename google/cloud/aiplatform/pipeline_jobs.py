...
def _set_enable_caching_value(pipeline_spec: Dict[str, Any],
  ...
  """Sets pipeline tasks caching options.
    ...
    Args:
     pipeline_spec (Dict[str, Any]): Required. The dictionary of pipeline spec.
     enable_caching (bool): Required. Whether to enable caching.
  """
...
class PipelineJob(
  ...
  def __init__(
    ...
    """Retrieves a PipelineJob resource and instantiates its
        ...
        Args:
            display_name (str): Required. The user-defined name of this
              Pipeline.
            template_path (str): Required. The path of PipelineJob or
              PipelineSpec JSON or YAML file. It can be a local path, a Google
              Cloud Storage URI (e.g. "gs://project.name"), or an Artifact
              Registry URI (e.g.
              "https://us-central1-kfp.pkg.dev/proj/repo/pack/latest").
            job_id (str): Optional. The unique ID of the job run. If not
              specified, pipeline name + timestamp will be used.
            pipeline_root (str): Optional. The root of the pipeline outputs.
              Default to be staging bucket.
            parameter_values (Dict[str, Any]): Optional. The mapping from
              runtime parameter names to its values that control the pipeline
              run.
            enable_caching (bool): Optional. Whether to turn on caching for the
              run.  If this is not set, defaults to the compile time settings,
              which are True for all tasks by default, while users may specify
              different caching options for individual tasks.  If this is set,
              the setting applies to all tasks in the pipeline.  Overrides the
            ...
            encryption_spec_key_name (str): Optional. The Cloud KMS resource
              ...
              the job. Has the
                form:
                  ...
                  created by the PipelineJob will be encrypted with the provided
                  encryption key.  Overrides encryption_spec_key_name set in
                  aiplatform.init.
            labels (Dict[str, str]): Optional. The user defined metadata to
              organize PipelineJob.
            credentials (auth_credentials.Credentials): Optional. Custom
              credentials to use to create this PipelineJob. Overrides
              credentials set in aiplatform.init.
            project (str): Optional. The project that you want to run this
              PipelineJob in. If not set, the project set in aiplatform.init
              will be used.
            location (str): Optional. Location to create PipelineJob. If not
              set, location set in aiplatform.init will be used.
            failure_policy (str): Optional. The failure policy - "slow" or
              "fast". Currently, the default of a pipeline is that the pipeline
              will continue to run until no more tasks can be executed, also
              known as PIPELINE_FAILURE_POLICY_FAIL_SLOW (corresponds to
              "slow"). However, if a pipeline is set to
              PIPELINE_FAILURE_POLICY_FAIL_FAST (corresponds to "fast"), it will
              stop scheduling any new tasks when a task has failed. Any
    ...
    if pipeline_json.get("pipelineSpec") is not None:
      ...
      pipeline_root = (
          pipeline_root or
          pipeline_job["pipelineSpec"].get("defaultPipelineRoot") or
          pipeline_job["runtimeConfig"].get("gcsOutputDirectory") or
  ...
  def run(
    ...
    """Run this configured PipelineJob and monitor the job until completion.
        ...
        Args:
            ...
            sync (bool): Optional. Whether to execute this method synchronously.
              If False, this method will unblock and it will be executed in a
              concurrent Future.
    ...
    """Run this configured PipelineJob.
        ...
        Args:
            ...
            experiment (Union[str, experiments_resource.Experiment]): Optional.
              The Vertex AI experiment name or instance to associate to this
              PipelineJob.  Metrics produced by the PipelineJob as system.Metric
              Artifacts will be associated as metrics to the current Experiment
              Run.  Pipeline parameters will be associated as parameters to the
  ...
  def _get_context(self) -> context._Context:
    """Returns the PipelineRun Context for this PipelineJob in the MetadataStore.
        ...
        Returns:
            System.PipelineRUn Context instance that represents this
            PipelineJob.

        Raises:
            RuntimeError if Pipeline has failed or system.PipelineRun context is
            not found.
        """
  ...
  def _query_experiment_row(
      cls, node: context._Context) -> experiment_resources._ExperimentRow:
    """Queries the PipelineJob metadata as an experiment run parameter and metric row.
        ...
        Args:
            node (context._Context): Required. System.PipelineRun context that
              represents a PipelineJob Run.
  ...
  def clone(
    ...
    """Returns a new PipelineJob object with the same settings as the original one.
        ...
        Args:
            display_name (str): Optional. The user-defined name of this cloned
              Pipeline. If not specified, original pipeline display name will be
              used.
            job_id (str): Optional. The unique ID of the job run. If not
              specified, "cloned" + pipeline name + timestamp will be used.
            pipeline_root (str): Optional. The root of the pipeline outputs.
              Default to be the same staging bucket as original pipeline.
            parameter_values (Dict[str, Any]): Optional. The mapping from
              runtime parameter names to its values that control the pipeline
              run. Defaults to be the same values as original PipelineJob.
            enable_caching (bool): Optional. Whether to turn on caching for the
              run. If this is not set, defaults to be the same as original
              pipeline. If this is set, the setting applies to all tasks in the
              pipeline.
            encryption_spec_key_name (str): Optional. The Cloud KMS resource
              identifier of the customer managed encryption key used to protect
              the job. Has the
                form:
                  ...
                  resource is created. If this is set, then all resources
                  created by the PipelineJob will be encrypted with the provided
                  encryption key. If not specified, encryption_spec of original
            ...
            credentials (auth_credentials.Credentials): Optional. Custom
              ...
              credentials set in aiplatform.init.
            project (str): Optional. The project that you want to run this
              PipelineJob in. If not set, the project set in original
              PipelineJob will be used.
            location (str): Optional. Location to create PipelineJob. If not
              set, location set in original PipelineJob will be used.
