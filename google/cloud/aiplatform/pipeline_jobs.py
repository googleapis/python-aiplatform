...
class PipelineJob(
    ...
    def from_component_file(
        ...
        """Creates PipelineJob from a component.yaml file.

        The component can be a container component or a graph component.

        Example:

            PipelineJob.from_component_file("./some/component.yaml").submit()

        Args:
            path (str):
                Required. Path of a component.yaml file (ComponentSpec).
            arguments (Dict[str, Any]):
                Optional. A dictionary mapping the component input names to arguments
                (constant strings).
            output_artifacts_gcs_dir (str):
        ...
        op = components.load_component_from_file(filename=path)
        return PipelineJob.from_pipeline_func(
            pipeline_func=op,
            context_name=context_name,
            parameter_values=arguments,
