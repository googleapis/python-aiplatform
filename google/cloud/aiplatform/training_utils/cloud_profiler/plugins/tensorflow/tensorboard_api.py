...
def _get_api_client() -> TensorboardClientWithOverride:
  ...
  api_client = aiplatform.initializer.global_config.create_client(
      ...
      api_base_path_override=training_utils.environment_variables
      .tensorboard_api_uri,
  )
...
def _get_blob_items(
  ...
  """Gets the blob storage items for the tensorboard resource.
    ...
    Args: api_client (): Required. Client go get information about the
...
def _get_or_create_experiment(api: TensorboardClientWithOverride,
                              experiment_name: str) -> str:
  """Creates a tensorboard experiment.

    Args:
        api (TensorboardClientWithOverride): Required. An api for interfacing
          with tensorboard resources.
        experiment_name (str): Required. The name of the experiment to get or
          create.
...
def create_profile_request_sender() -> profile_uploader.ProfileRequestSender:
  """Creates the `ProfileRequestSender` for the profile plugin.
    ...
    A profile request sender is created for the plugin so that after profiling
    runs
