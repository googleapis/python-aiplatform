...
_BASE_TB_ENV_WARNING = (
    "To set this environment variable, run your training with the "
    "'tensorboard' option. For more information on how to run with training "
    "with tensorboard, visit "
...
def _is_compatible_version(version: Version) -> bool:
  """Check if version is compatible with tf profiling.
    ...
    that are included in 2.4.0 >= are also needed for the
    ...
    Args:
        version (Version): Required. `Verison` of tensorflow.
...
def _check_tf() -> bool:
  ...
  if importlib.util.find_spec("tensorflow") is None:
    logger.warning(
        "Tensorflow not installed, cannot initialize profiling plugin")
    return False
...
def _host_to_grpc(hostname: str) -> str:
  """Format a hostname to a grpc address.
    ...
    Args:
        hostname (str): Required. Address in form: `{hostname}:{port}`

    Returns:
        Address in form of: 'grpc://{hostname}:{port}'
    """
  return ("grpc://" + "".join(hostname.split(":")[:-1]) + ":" +
...
def warn_tensorboard_env_var(var_name: str):
  ...
  """
  logging.warning(f"Environment variable `{var_name}` must be set. " +
                  _BASE_TB_ENV_WARNING)
...
class TFProfiler(base_plugin.BasePlugin):
  ...
  def get_routes(
    ...
    """List of routes to serve.
        ...
        Returns:
            A callable that takes an werkzeug env and start response and returns
            a response.
        """
  ...
  def capture_profile_wrapper(
      self, environ: wsgi_types.Environment,
  ...
  def can_initialize() -> bool:
    """Check that we can use the TF Profiler plugin.
        ...
        This function checks a number of dependencies for the plugin to ensure
        we have the
        right packages installed, the necessary versions, and the correct
