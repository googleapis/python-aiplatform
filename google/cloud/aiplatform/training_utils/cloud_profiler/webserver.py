...
class WebServer:
  ...
  def __init__(self, plugins: List[base_plugin.BasePlugin]):
    """Creates a web server to host plugin routes.
        ...
        Args:
            plugins (List[base_plugin.BasePlugin]): Required. A list of
              `BasePlugin` objects.
    ...
    for plugin in self._plugins:
      for route, handler in plugin.get_routes().items():
        if not route.startswith("/"):
          raise ValueError('Routes should start with a "/", '
  ...
  def dispatch_request(self, environ: wsgi_types.Environment,
                       start_response: wsgi_types.StartResponse) -> Response:
    """Handles the routing of requests.

        Args:
            environ (wsgi_types.Environment): Required. The WSGI environment.
            start_response (wsgi_types.StartResponse): Required. The response
              callable provided by the WSGI server.
  ...
  def wsgi_app(self, environ: wsgi_types.Environment,
               start_response: wsgi_types.StartResponse) -> Response:
