...
def _build_plugin(
    plugin: Type[base_plugin.BasePlugin],) -> Optional[base_plugin.BasePlugin]:
  """Builds the plugin given the object.

    Args:
        plugin (Type[base_plugin]): Required. An uninitialized plugin class.
...
def _run_app_thread(server: webserver.WebServer, port: int):
  """Run the webserver in a separate thread.
    ...
    Args:
        server (webserver.WebServer): Required. A webserver to accept requests.
        port (int): Required. The port to run the webserver on.
  """
...
def initialize(plugin: str = "tensorflow"):
  """Initializes the profiling SDK.
    ...
    Args:
        plugin (str): Required. Name of the plugin to initialize. Current
          options are ["tensorflow"]
  ...
  if not plugin_obj:
    raise ValueError("Plugin {} not available, must choose from {}".format(
