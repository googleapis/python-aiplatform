...
class _Dataset(base.VertexAiResourceNounWithFutureManager):
  ...
  def __init__(
    ...
    """Retrieves an existing managed dataset given a dataset name or ID.
        ...
        Args:
            dataset_name (str): Required. A fully-qualified dataset resource
              name or dataset ID.
                Example: "projects/123/locations/us-central1/datasets/456" or
                  "456" when project and location are initialized or passed.
            project (str): Optional project to retrieve dataset from. If not
              set, project set in aiplatform.init will be used.
            location (str): Optional location to retrieve dataset from. If not
              set, location set in aiplatform.init will be used.
            credentials (auth_credentials.Credentials): Custom credentials to
              use to retrieve this Dataset. Overrides credentials set in
              aiplatform.init.
    """
  ...
  def create(
    ...
    """Creates a new dataset and optionally imports data into dataset when
        ...
        Args:
            display_name (str): Required. The user-defined name of the Dataset.
              The name can be up to 128 characters long and can be consist of
              any UTF-8 characters.
            metadata_schema_uri (str): Required. Points to a YAML file stored on
              Google Cloud Storage describing additional information about the
              Dataset. The schema is defined as an OpenAPI 3.0.2 Schema Object.
            ...
            bq_source (str): BigQuery URI to the input table.
                example: "bq://project.dataset.table_name"
            import_schema_uri (str): Points to a YAML file stored on Google
            ...
            project (str): Project to upload this model to. Overrides project
              set in aiplatform.init.
            location (str): Location to upload this model to. Overrides location
              set in aiplatform.init.
            credentials (auth_credentials.Credentials): Custom credentials to
              use to upload this model. Overrides credentials set in
              aiplatform.init.
            request_metadata (Sequence[Tuple[str, str]]): Strings which should
  ...
  def _create_and_import(
    ...
    """Creates a new dataset and optionally imports data into dataset when
        ...
        Args:
            api_client (dataset_service_client.DatasetServiceClient): An
              instance of DatasetServiceClient with the correct api_endpoint
              already set based on user's preferences.
            parent (str): Required. Also known as common location path, that
              usually contains the project and location that the user provided
              to the upstream method.
            ...
            metadata_schema_uri (str): Required. Points to a YAML file stored on
              ...
              The schema files that can be used here are found in
              gs://google-cloud- aiplatform/schema/dataset/metadata/.
            datasource (_datasources.Datasource): Required. Datasource for
              creating a dataset for Vertex AI.
            project (str): Required. Project to upload this model to. Overrides
              project set in aiplatform.init.
            location (str): Required. Location to upload this model to.
            ...
            encryption_spec (Optional[gca_encryption_spec.EncryptionSpec]):
              Optional. The Cloud KMS customer managed encryption key used to
              protect the dataset. The key needs to be in the same region as
              where the compute resource is created.  If set, this Dataset and
            ...
            create_request_timeout (float): Optional. The timeout for the create
              request in seconds.
            import_request_timeout (float): Optional. The timeout for the import
              request in seconds.
    ...
    """Creates a new managed dataset by directly calling API client.
        ...
        Args:
            ...
            datasource (_datasources.Datasource): Required. Datasource for
              creating a dataset for Vertex AI.
            request_metadata (Sequence[Tuple[str, str]]): Strings which should
              be sent along with the create_dataset request as metadata. Usually
              to specify special dataset config.
            labels (Dict[str, str]): Optional. Labels with user-defined metadata
              to organize your Tensorboards. Label keys and values can be no
              longer than 64 characters (Unicode codepoints), can only contain
              lowercase letters, numeric characters, underscores and dashes.
  ...
  def _import(
    ...
    """Imports data into managed dataset by directly calling API client.
        ...
        Args:
            datasource (_datasources.DatasourceImportable): Required. Datasource
              for importing data to an existing dataset for Vertex AI.
            import_request_timeout (float): Optional. The timeout for the import
              request in seconds.
            ...
            gcs_source (Union[str, Sequence[str]]): Required. Google Cloud
              ...
              more information on wildcards, see
                ...
                examples:
                    str: "gs://bucket/file.csv" Sequence[str]:
                      ["gs://bucket/file1.csv", "gs://bucket/file2.csv"]
            import_schema_uri (str): Required. Points to a YAML file stored on
              Google Cloud Storage describing the import format. Validation will
              be done against the schema. The schema is defined as an `OpenAPI
              3.0.2 Schema Object <https://tinyurl.com/y538mdwt>`__.
            data_item_labels (Dict): Labels that will be applied to newly
              imported DataItems. If an identical DataItem as one being imported
              already exists in the Dataset, then these labels will be appended
  ...
  def export_data(self, output_dir: str) -> Sequence[str]:
    """Exports data to output dir to GCS.
        ...
        Args:
            output_dir (str): Required. The Google Cloud Storage location where
              the output is to be written to. In the given directory a new
              ...
              created to describe the output format.  If the uri doesn't end
              with '/', a '/' will be automatically appended. The directory is
  ...
  def update(
    ...
    """Update the dataset.
        ...
        Args:
            display_name (str): Optional. The user-defined name of the Dataset.
              The name can be up to 128 characters long and can be consist of
              any UTF-8 characters.
            labels (Dict[str, str]): Optional. Labels with user-defined metadata
              to organize your Tensorboards. Label keys and values can be no
  ...
  def list(
    ...
    dataset_subclass_filter = (lambda gapic_obj: gapic_obj.metadata_schema_uri
