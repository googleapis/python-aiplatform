...
class TextDataset(datasets._Dataset):
    ...
    * Sentiment analysis. For more information, see
    ...
    def create(
        ...
        Optionally imports data into dataset when source and `import_schema_uri`
        are passed in. The following is an example of how this method is used:
        ...
        Args:
            ...
            data_item_labels (Dict):
                A dictionary of label information. Each dictionary item contains
                a label and a label key. Each image in the dataset includes one
                dictionary of label information. If a data item is added or
                merged into a dataset, and that data item contains an image
                that's identical to an image that’s already in the dataset, then
                the data items are merged. If two identical labels are detected
                during the merge, each with a different label key, then one of
                the label and label key dictionary items is randomly chosen to
                be into the merged data item. A byte comparison determines if
                two images are identical. If annotation labels are referenced in
                a schema specified by the `import_schema_url` parameter, then
                the labels in the `data_item_labels` dictionary are overriden by
                the annotations.
            project (str):
            ...
            credentials (auth_credentials.Credentials):
                The credentials that are used to upload the `TextDataset`.
