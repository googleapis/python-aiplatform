...
class TabularDatasource(Datasource):
  ...
  def __init__(
    ...
    """Creates a tabular datasource.
        ...
        Args:
            gcs_source (Union[str, Sequence[str]]): Cloud Storage URI of one or
              more files. Only CSV files are supported. The first line of the
              CSV file is used as the header. If there are multiple files, the
              header is the first line of the lexicographically first file, the
              other files must either contain the exact same header or omit the
              header.
                examples:
                    str: "gs://bucket/file.csv" Sequence[str]:
                      ["gs://bucket/file1.csv", "gs://bucket/file2.csv"]
            bq_source (str): The URI of a BigQuery table.
...
class NonTabularDatasourceImportable(NonTabularDatasource,
  ...
  def __init__(
    ...
    """Creates a non-tabular datasource.
        ...
        Args:
            gcs_source (Union[str, Sequence[str]]): Required. The Google Cloud
              Storage location for the input content. Google Cloud Storage
              URI(-s) to the input file(s).
                Examples:
                    str: "gs://bucket/file.csv" Sequence[str]:
                      ["gs://bucket/file1.csv", "gs://bucket/file2.csv"]
    ...
    """
    super().__init__()
    self._gcs_source = [gcs_source] if isinstance(gcs_source,
                                                  str) else gcs_source
...
def create_datasource(
  ...
  """Creates a datasource
    ...
    Args:
        metadata_schema_uri (str): Required. Points to a YAML file stored on
          ...
          Dataset. The schema is defined as an OpenAPI 3.0.2 Schema Object. The
          schema files that can be used here are found in gs://google-cloud-
          aiplatform/schema/dataset/metadata/.
        import_schema_uri (str): Points to a YAML file stored on Google Cloud
        ...
        gcs_source (Union[str, Sequence[str]]): The Google Cloud Storage
          location for the input content. Google Cloud Storage URI(-s) to the
          input file(s).
            Examples:
                str: "gs://bucket/file.csv" Sequence[str]:
                  ["gs://bucket/file1.csv", "gs://bucket/file2.csv"]
        bq_source (str): BigQuery URI to the input table.
            example: "bq://project.dataset.table_name"
        data_item_labels (Dict): Labels that will be applied to newly imported
          ...
          the already existing one, and if labels with identical key is imported
          ...
          combined and if key collision happens in this case, one of the values
          will be picked randomly. Two DataItems are considered identical if
          their content bytes are identical (e.g. image bytes or pdf bytes).
          These labels will be overridden by Annotation labels specified inside
          index file refenced by ``import_schema_uri``, e.g. jsonl file.
    ...
    Raises:
        ...
        - either import_schema_uri or gcs_source is missing for creating
  ...
  if not import_schema_uri and not gcs_source:
    ...
    return NonTabularDatasourceImportable(gcs_source, import_schema_uri,
                                          data_item_labels)
  else:
    raise ValueError(
