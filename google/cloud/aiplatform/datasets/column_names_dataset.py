...
class _ColumnNamesDataset(datasets._Dataset):
  ...
  def _retrieve_gcs_source_columns(
    ...
    """Retrieve the columns from a comma-delimited CSV file stored on Google Cloud Storage
        ...
        Args:
            project (str): Required. Project to initiate the Google Cloud
              Storage client with.
            gcs_csv_file_path (str): Required. A full path to a CSV files stored
              on Google Cloud Storage. Must include "gs://" prefix.
            credentials (auth_credentials.Credentials): Credentials to use to
              with GCS Client.
    ...
    try:
      ...
      while first_new_line_index == -1:
        line += blob.download_as_bytes(
            start=start_index, end=start_index + increment - 1).decode("utf-8")
  ...
  def _get_bq_schema_field_names_recursively(
      schema_field: bigquery.SchemaField,) -> Set[str]:
    """Retrieve the name for a schema field along with ancestor fields.
        ...
        Args:
            project (str): Required. Project to initiate the BigQuery client
              with.
            bq_table_uri (str): Required. A URI to a BigQuery table. Can include
              "bq://" prefix but not required.
            credentials (auth_credentials.Credentials): Credentials to use with
              BQ Client.
    ...
    ancestor_names = {
        nested_field_name for field in schema_field.fields for nested_field_name
  ...
  def _retrieve_bq_source_columns(
    ...
    return {
        field_name for field in schema for field_name in
        _ColumnNamesDataset._get_bq_schema_field_names_recursively(field)
    }
