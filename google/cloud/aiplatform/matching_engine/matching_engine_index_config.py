...
class TreeAhConfig(AlgorithmConfig):
  """Configuration options for using the tree-AH algorithm (Shallow tree + Asymmetric Hashing).
    ...
    Args:
        ...
        leaf_nodes_to_search_percent (float): The default percentage of leaf
          nodes that any query may be searched. Must be in range 1-100,
          inclusive. The default value is 10 (means 10%) if not set.
  """
  ...
  """Configuration options for using the tree-AH algorithm (Shallow tree + Asymmetric Hashing).
    ...
    Args:
        dimensions (int): Required. The number of dimensions of the input
          vectors.
        algorithm_config (AlgorithmConfig): Required. The configuration with
          regard to the algorithms used for efficient search.
        approximate_neighbors_count (int): Optional. The default number of
          neighbors to find via approximate search before exact reordering is
          performed. Exact reordering is a procedure where results returned by
          an approximate search algorithm are reordered via a more expensive
          distance computation.  Required if tree-AH algorithm is used.
        distance_measure_type (DistanceMeasureType): Optional. The distance
          measure used in nearest neighbor search.
  """
