  ...
  r"""
    ...
    Attributes:
        ...
        optimization_objective (str): Objective function the model is optimizing
          ...
          optimization objectives:  -  "minimize-rmse" (default) - Minimize
          root-mean-squared error (RMSE).  -  "minimize-mae" - Minimize
        ...
        weight_column (str): Column name that should be used as the weight
          ...
          equal weight of 1.
        time_series_attribute_columns (Sequence[str]): Column names that should
          be used as attribute columns. The value of these columns does not vary
          as a function of time. For example, store ID or item color.
        unavailable_at_forecast_columns (Sequence[str]): Names of columns that
          are unavailable when a forecast is requested. This column contains
        ...
        available_at_forecast_columns (Sequence[str]): Names of columns that are
          available and provided when a forecast is requested. These columns
        ...
        forecast_horizon (int): The amount of time into the future for which
          forecasted values for the target are returned. Expressed in number of
          units defined by the ``data_granularity`` field.
        context_window (int): The amount of time into the past training and
        ...
        quantiles (Sequence[float]): Quantiles to use for minimize-quantile-loss
        ...
        validation_options (str): Validation options for the data validation
        ...
        additional_experiments (Sequence[str]): Additional experiment flags for
          the time series forcasting training.
  """
    ...
    r"""A duration of time expressed in time granularity units.
        ...
        Attributes:
            unit (str): The time granularity unit of this time period. The
              supported units are:  -  "minute"  -  "hour"  -  "day"  -  "week"
              -  "month"  -  "year".
            quantity (int): The number of granularity_units between data points
              in the training data. If ``granularity_unit`` is ``minute``, can
              be 1, 5, 10, 15, or 30. For all other values of
