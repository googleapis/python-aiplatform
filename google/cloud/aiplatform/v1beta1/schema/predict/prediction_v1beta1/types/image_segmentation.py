...
class ImageSegmentationPredictionResult(proto.Message):
  r"""Prediction output format for Image Segmentation.
    ...
    Attributes:
        category_mask (str): A PNG image where each pixel in the mask represents
          the category in which the pixel in the original image was predicted to
          ...
          category and if none of the categories reach the confidence threshold,
          the pixel will be marked as background.
        confidence_mask (str): A one channel image which is encoded as an 8bit
