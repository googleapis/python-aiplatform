...
def get_default_column_transformations(
  ...
  """Get default column transformations from the column names, while omitting the target column.
    ...
    Args:
        dataset (_ColumnNamesDataset): Required. The dataset
        target_column (str): Required. The name of the column values of which
          the Model is to predict.
  ...
  column_names = [
      column_name for column_name in dataset.column_names
...
def validate_and_get_column_transformations(
  ...
  """Validates column specs and transformations, then returns processed transformations.
    ...
    Args:
        column_specs (Dict[str, str]): Optional. Alternative to
          column_transformations where the keys of the dict are column names and
          their respective values are one of
          AutoMLTabularTrainingJob.column_data_types. When creating
          transformation for BigQuery Struct column, the column should be
          flattened using "." as the delimiter. Only columns with no child
          should have a transformation. If an input column has no
          transformations on it, such a column is ignored by the training,
          except for the targetColumn, which should have no transformations
          defined on. Only one of column_transformations or column_specs should
          be passed.
        column_transformations (List[Dict[str, Dict[str, str]]]): Optional.
          Transformations to apply to the input columns (i.e. columns other than
          the targetColumn). Each transformation may produce multiple result
          values from the column's value, and all are used for training. When
          ...
          defined on. Only one of column_transformations or column_specs should
          be passed. Consider using column_specs as column_transformations will
    ...
    Raises:
        ValueError: If both column_transformations and column_specs were
        provided.
    """
