...
class _WorkerPoolSpec(NamedTuple):
  ...
  def _get_accelerator_type(self) -> Optional[str]:
    ...
    accelerator_enum = getattr(gca_accelerator_type_compat.AcceleratorType,
...
class _DistributedTrainingSpec(NamedTuple):
  ...
  @property
  def pool_specs(
      self,) -> List[Dict[str, Union[int, str, Dict[str, Union[int, str]]]]]:
    """Return each pools spec in correct order for Vertex AI as a list of

        dicts.

        Also removes specs if they are empty but leaves specs in if there
        unusual
  ...
  def chief_worker_pool(
    ...
    """Parametrizes Config to support only chief with worker replicas.
        ...
        For replica is assigned to chief and the remainder to workers. All spec
        have the
        ...
        Args:
            ...
            accelerator_count (int): The number of accelerators to attach to a
              worker replica.
            boot_disk_type (str): Type of the boot disk (default is `pd-ssd`).
              Valid values: `pd-ssd` (Persistent Disk Solid State Drive) or
              `pd-standard` (Persistent Disk Hard Disk Drive).
            boot_disk_size_gb (int): Size in GB of the boot disk (default is
              100GB). boot disk size must be within the range of [100, 64000].
