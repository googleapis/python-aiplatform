...
def full_resource_name(
  ...
  """Returns fully qualified resource name.
    ...
    Args:
        resource_name (str): Required. A fully-qualified Vertex AI resource name
          or resource ID.
        resource_noun (str): Required. A resource noun to validate the resource
          name against. For example, you would pass "datasets" to validate
          "projects/123/locations/us-central1/datasets/456".
        parse_resource_name_method (Callable[[str], Dict[str,str]]): Required.
          Method that parses a resource name into its segment parts. These are
          generally included with GAPIC clients.
        format_resource_name_method (Callable[..., str]): Required. Method that
          takes segment parts of resource names and returns the formated
          resource name. These are generally included with GAPIC clients.
        parent_resource_name_fields (Dict[str, str]): Optional. Dictionary of
          segment parts where key is the resource noun and values are the
          resource ids. For example: { "metadataStores": "123" }
        project (str): Optional. project to retrieve resource_noun from. If not
          set, project set in aiplatform.init will be used.
        location (str): Optional. location to retrieve resource_noun from. If
          not set, location set in aiplatform.init will be used.
        resource_id_validator (Callable[str, None]): Optional. Function that
          validates the resource ID. Overrides the default validator,
          validate_id. Should take a resource ID as string and raise ValueError
          if invalid.
  ...
  format_args = {
      ...
      "project":
          user_project,
      convert_camel_case_resource_noun_to_snake_case(resource_noun):
          resource_name,
  }
...
def validate_accelerator_type(accelerator_type: str) -> bool:
  """Validates user provided accelerator_type string for training and
    ...
    Args:
        accelerator_type (str): Represents a hardware accelerator type.
...
def extract_bucket_and_prefix_from_gcs_path(
    gcs_path: str) -> Tuple[str, Optional[str]]:
  """Given a complete GCS path, return the bucket name and prefix as a tuple.
    ...
    Args:
        gcs_path (str): Required. A full path to a Google Cloud Storage folder
          or resource. Can optionally include "gs://" prefix or end in a
          trailing slash "/".
...
class ClientWithOverride:
  ...
  class WrappedClient:
    ...
    def __init__(
      ...
      """Stores parameters needed to instantiate client.
            ...
            Args:
                client_class (VertexAiServiceClient): Required. Class of the
                  client to use.
                client_options (client_options.ClientOptions): Required. Client
                  options to pass to client.
                client_info (gapic_v1.client_info.ClientInfo): Required. Client
                  info to pass to client.
                credentials (auth_credentials.credentials): Optional. Client
  ...
  def __init__(
    ...
    """Stores parameters needed to instantiate client.
        ...
        Args:
            client_options (client_options.ClientOptions): Required. Client
              options to pass to client.
            client_info (gapic_v1.client_info.ClientInfo): Required. Client info
              to pass to client.
            credentials (auth_credentials.credentials): Optional. Client
              credentials to pass to client.
    """
    ...
    self._clients = {
        ...
        ) if self._is_temporary else client_class(
            client_options=client_options,
            client_info=client_info,
            credentials=credentials,
  ...
  @classmethod
  def get_gapic_client_class(cls,
    ...
    """Gets the underyilng GAPIC client.
        ...
        Args:
            version (str): Optional. Version of client to retreive otherwise the
              default version is returned.
        Retuns: Underlying GAPIC client for this wrapper and version.
    """
...
def _timestamped_copy_to_gcs(
  ...
  """Copies a local file to a GCS path.
    ...
    Args:
        ...
        gcs_dir (str): Required. The GCS directory to copy to.
        project (str): Project that contains the staging bucket. Default will be
          used if not provided. Model Builder callers should pass this in.
        credentials (auth_credentials.Credentials): Custom credentials to use
          with bucket. Model Builder callers should pass this in.
...
def get_timestamp_proto(
    time: Optional[datetime.datetime] = None,) -> timestamp_pb2.Timestamp:
  """Gets timestamp proto of a given time.

    Args:
        time (datetime.datetime): Optional. A user provided time. Default to
          datetime.datetime.now() if not given.

    Returns:
        timestamp_pb2.Timestamp: timestamp proto of the given time, not have
