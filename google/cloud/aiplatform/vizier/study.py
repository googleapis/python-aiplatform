...
class Study(base.VertexAiResourceNounWithFutureManager, StudyInterface): 
    ...
    @classmethod
    @base.optional_sync()
    def create_or_load(cls, display_name: str, problem: vz.ProblemStatement) -> StudyInterface:
      ...
      """
      api_client = cls._instantiate_client(location=initializer.global_config.location, credentials=initializer.global_config.credentials)
      study = gca_study.Study(
        display_name = display_name,
        study_spec = problem.to_proto()
      )
      
      try:
        study = api_client.create_study(parent = initializer.global_config.common_location_path(initializer.global_config.project, initializer.global_config.location), study = study)
      except exceptions.AlreadyExists:
        _LOGGER.info("The study is aleady created. Using existing study.")
        study = api_client.lookup_study(request = {
          "parent" : initializer.global_config.common_location_path(initializer.global_config.project, initializer.global_config.location),
          "display_name" : display_name})
    ...
    def get_trial(self, uid: int) -> TrialInterface:
      ...
      return Trial(Trial._format_resource_name(
    ...
    def optimal_trials(self) -> Collection[TrialInterface]:
      ...
      optimal_trials_response = self.api_client.list_optimal_trials(request=list_optimal_trials_request)
    ...
    def suggest(self,
      ...
      """
      suggest_trials_lro = self.api_client.suggest_trials(request={ 
