...
class MetricInformationTest(absltest.TestCase):
    ...
    def testMinMaxValueSet(self):
        ...
        self.assertEqual(info.max_value, 1.0)
...
class SearchSpaceTest(parameterized.TestCase):
    ...
    def testAddBooleanParam(self):
        ...
        with self.assertRaisesRegex(ValueError, "Accessing bounds of a categorical.*"):
    ...
    def testMultidimensionalParameters(self):
        ...
        selector1 = space.select_root().add_float_param(
            "f", 2.0, 10.0, default_value=4.0, scale_type=pc.ScaleType.LINEAR, index=1
        )
    ...
    def testConditionalParameters(self):
        ...
        self.assertEmpty(space.parameters)
        root = space.select_root()
        root.add_categorical_param("model_type", ["linear", "dnn"], default_value="dnn")
        # Test the selector.
        ...
        dnn_optimizer = dnn.add_categorical_param("optimizer_type", ["adam", "adagrad"])
        # Test the search space.
        self.assertLen(space.parameters, 1)
        # Test the selector.
        ...
        ko = (
            root.select("model_type", ["dnn"])
            .select("optimizer_type", ["adam"])
            .add_bool_param("use_keras_optimizer", default_value=False)
        )
        # Test the search space.
        self.assertLen(space.parameters, 1)
        # Test the selector.
    ...
    def testConditionalParametersWithReturnedSelectors(self):
        ...
        optimizer_type = model_type.select_values(
            ["linear", "dnn"]
        ).add_categorical_param("optimizer_type", ["adam", "adagrad"])
        # Test the search space.
        self.assertLen(space.parameters, 1)
        # Test the selectors.
...
class SearchSpaceContainsTest(absltest.TestCase):
    ...
    def testFloatCat1(self):
        self._space().assert_contains(
            trial.ParameterDict({"optimizer": "adagrad", "learning-rate": 1e-2})
        )

    def testFloatCat2(self):
        self.assertFalse(
            self._space().contains(
                trial.ParameterDict({"optimizer": "adagrad", "BADPARAM": 1e-2})
            )
        )
    ...
    def testFloatCat3(self):
        self.assertFalse(
            self._space().contains(
                trial.ParameterDict(
                    {"optimizer": "adagrad", "learning-rate": 1e-2, "BADPARAM": 1e-2}
                )
            )
        )
    ...
    def testFloatCat4(self):
        self.assertFalse(
            self._space().contains(
                trial.ParameterDict({"optimizer": "adagrad", "learning-rate": 1e2})
            )
        )
