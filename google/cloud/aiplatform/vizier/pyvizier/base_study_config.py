    ...
    def _assert_names_are_unique(self) -> None:
        ...
        if len(counts) != len(self._metrics):
            for name, count in counts.items():
                if count > 1:
                    raise ValueError(
                        f"Duplicate metric name: {name} in {self._metrics}"
                    )
...
class _PathSelector(Sequence[_PathSegment]):
    ...
    def __add__(
        self, other: Union[Sequence[_PathSegment], _PathSegment]
    ) -> "_PathSelector":
...
class SearchSpaceSelector:
    ...
    _selected_name: str = attr.ib(
        init=True, default="", validator=attr.validators.instance_of(str), kw_only=True
    )
    ...
    def select(
        ...
        if parameter_values is not None:
            if not isinstance(parameter_values, (list, tuple)):
                raise ValueError(
                    "parameter_values should be a list or tuple, given "
                    "{} with type {}".format(parameter_values, type(parameter_values))
                )
        ...
        if self._selected_name:
            ...
            if not self._selected_values:
                raise ValueError(
                    "Cannot call select() again before parameter values "
            ...
            # the path.
            new_path_segment = [
                _PathSegment(name=self._selected_name, value=self._selected_values[0])
            ]
            path = self._selected_path + new_path_segment
            if not self._path_exists(path):
                raise ValueError(
                    "Path {} does not exist in this SearchSpace: "
                    "{}".format((path), self)
                )
    ...
    def select_values(
        self, parameter_values: MonotypeParameterSequence
    ) -> "SearchSpaceSelector":
    ...
    def _path_exists(self, path: _PathSelector) -> bool:
        ...
        for parent in self._configs:
            if path[0].name == parent.name and path[0].value in parent.feasible_values:
                if len(path) == 1:
                    # No need to recurse.
                    return True
    ...
    @classmethod
    def _path_exists_inner(
        cls, current_root: parameter_config.ParameterConfig, current_path: _PathSelector
    ) -> bool:
        ...
        for idx, child in enumerate(current_root.child_parameter_configs):
            if (
                current_path[0].name == child.name
                and current_path[0].value in child.feasible_values
            ):
                child_idx = idx
                break
    ...
    def _get_parameter_names_to_create(
        cls, *, name: str, length: Optional[int] = None, index: Optional[int] = None
    ) -> List[str]:
    ...
    def _add_parameters(
        ...
        if self._selected_name and not self._selected_values:
            raise ValueError(
                ...
                "not selected. Call select_values() first.".format(self._selected_name)
            )
    ...
    def _recursive_add_child_parameters(
        cls,
        configs: List[parameter_config.ParameterConfig],
        path: _PathSelector,
        root: parameter_config.ParameterConfig,
        parent_name: str,
    ...
    def _add_parameters_at_selected_path(
        cls,
        root_configs: List[parameter_config.ParameterConfig],
        complete_path: _PathSelector,
        parent_name: str,
        parent_values: MonotypeParameterSequence,
        new_children: List[parameter_config.ParameterConfig],
    ) -> Tuple[int, parameter_config.ParameterConfig, List["SearchSpaceSelector"]]:
    ...
    def _add_parameters_at_selected_path_inner(
        ...
        if not current_path:
            ...
            if child_idx is None:
                raise ValueError(
                    'Invalid parent_name: after traversing the path "{}", '
                    'failed to find a child parameter with name "{}".'
                    ' Current root="{}"'.format(
                        (complete_path), parent_name, current_root
                    )
                )
    ...
    def _add_child_parameters(
        cls,
        selector_configs: List[parameter_config.ParameterConfig],
        selector_path: _PathSelector,
        parent: parameter_config.ParameterConfig,
        parent_values: MonotypeParameterSequence,
        new_children: List[parameter_config.ParameterConfig],
    ) -> Tuple[parameter_config.ParameterConfig, List["SearchSpaceSelector"]]:
        ...
        return (parent.clone_without_children.add_children(updated_children), selectors)
