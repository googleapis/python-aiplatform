...
class Metric:
    ...
    def _std_not_negative(self, _, stddev):
        if stddev < 0:
            raise ValueError("Standard deviation must be a non-negative finite number.")
...
def _to_local_time(dt: Optional[datetime.datetime]) -> Optional[datetime.datetime]:
...
class ParameterDict(cabc.MutableMapping):
    ...
    _items: MutableMapping[str, ParameterValue] = attr.field(init=False, factory=dict)
...
class Trial:
    ...
    final_measurement: Optional[Measurement] = attr.ib(
        ...
        kw_only=True,
        default=None,
        validator=attr.validators.optional(attr.validators.instance_of(Measurement)),
    )
    ...
    def is_completed(self) -> bool:
        ...
        if self.status == TrialStatus.COMPLETED:
            if self.completion_time is None:
                logging.warning(
                    "Invalid Trial state: status is COMPLETED, but a "
            ...
            if self.status is None:
                logging.warning(
                    "Invalid Trial state: status is not set to COMPLETED, "
    ...
    def complete(self, measurement: Measurement, *, inplace: bool = True) -> "Trial":
    ...
    ids: Optional[FrozenSet[int]] = attr.field(
        ...
        validator=attr.validators.optional(
            attr.validators.deep_iterable(
                attr.validators.instance_of(int), attr.validators.instance_of(frozenset)
            )
        ),
    )
