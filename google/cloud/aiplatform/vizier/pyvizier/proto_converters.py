...
class MeasurementConverter:
  ...
  def to_proto(cls, measurement: trial.Measurement) -> study_pb2.Measurement:
    ...
    for name, metric in measurement.metrics.items():
      proto.metrics.append(study_pb2.Measurement.Metric(metric_id=name, value=metric.value))

    proto.step_count = measurement.steps
    int_seconds = int(measurement.elapsed_secs)
    proto.elapsed_duration = duration_pb2.Duration(seconds=int_seconds, nanos=int(1e9 * (measurement.elapsed_secs - int_seconds)))
    return proto
