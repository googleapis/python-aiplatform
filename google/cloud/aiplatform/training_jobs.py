...
class _TrainingJob(base.VertexAiStatefulResource):
    ...
    def get_model(self, sync=True) -> models.Model:
        """Returns the Vertex AI model produced by this training job. A runtime
        error is raised if a model wasn't produced by this training.
        ...
        Returns:
            model: The Vertex AI model that was produced by this training job.
    ...
    def has_failed(self) -> bool:
        """Returns `true` if the training job failed, otherwise `false` is
        returned.
        """
        self._assert_has_run()
    ...
    def cancel(self) -> None:
        """Asynchronously attempts to cancel a training job. The server
        makes a best effort to cancel the job, but the training job can't always
        be cancelled. If the training job is canceled, its state transitions to
        `CANCELLED` and it's not deleted.
...
class _CustomTrainingJob(_TrainingJob):
    ...
    def web_access_uris(self) -> Dict[str, str]:
        ...
        Returns:
            (Dict[str, str]):
                The URIs used to access the custom training job.
        """
...
class CustomTrainingJob(_CustomTrainingJob):
    ...
    def run(
        ...
        * [Chronological
        ...
        Returns:
            model: The trained Vertex AI model resource or None if the training
...
class AutoMLImageTrainingJob(_TrainingJob):
    ...
    def __init__(
        ...
        Args:
            ...
            multi_label: If `false`, a single-label (multi-class) model is
              trained (i.e. assuming that for each image just up to one
              annotation may be applicable). If `true`, a multi-label model is
              trained. If you pass in `true`, it's assumed multiple annotations
              might apply to each image. Multi-label models are supported only
              when `prediction_type` is set to `classification`. If
              `multi_label` is `true` and `prediction_type` is
              `object_detection`, then then `multi_label` is ignored. The
              default value is `false`.
            model_type: The type of model to create. The following are the valid values:

              `CLOUD` - The default for image classification. This model type is
              deigned to work in Google Cloud and can't be exported.

              `CLOUD_1` - This model type is deigned to work in Google Cloud and
              can't be exported. This model type is expected to have higher
              prediction accuracy than `CLOUD`.
              ...
              Google Cloud and can't be exported. It's also designed to have low
              latency, but might might lower prediction quality than other cloud
              model types.

              `MOBILE_TF_LOW_LATENCY_1` - This model type is deigned to work in
              Google Cloud and can be exported as a TensorFlow or Core ML model
              for use on a mobile or edge device. It's also
              designed to have to have low latency, but might have lower
              prediction quality than other mobile models types.

              `MOBILE_TF_VERSATILE_1` - This model type is deigned to work in
              Google Cloud and can be exported as a TensorFlow or Core ML model
              for use on a mobile or edge device.
              ...
              model for use on a mobile or edge device. It's designed to have a
              higher latency, but should also have a higher prediction quality
              than other mobile model types.

              `EFFICIENTNET` - This model type is available in the Vertex Model
              Garden image classification training with customizable
              hyperparameters. It's deigned to work in Google Cloud and can't be
              exported.
              ...
              `MAXVIT` - This model type is available in the Vertex Model Garden
              ...
              `YOLO` - This model type is available in the Vertex Model
              Garden image object detection training with customizable
              hyperparameters. It's deigned to work in Google Cloud and can't be
              exported.
            ...
            incremental_train_base_model: Optional[models.Model] = Optional. You
              ...
              existing model as the starting point. This can reduce the training
              time. If not specified, the new model is trained from scratch. The
              model specified in `incremental_train_base_model` model must be in
              the same Google Project and region as the new model to train, and
              it must have the same `prediction_type` and `model_type`.
            ...
            credentials that are used to train the model.
              These credentials override the credentials set by
              `aiplatform.init`.
            labels (Dict[str, str]): Optional. Labels with user-defined metadata
              ...
              and value is 64 unicode characters. Labels and keys can
              contain only lowercase letters, numeric characters, underscores,
              and dashes. International characters are allowed. For more
            ...
            training_encryption_spec_key_name (Optional[str]): Optional. The Cloud
              ...
              `projects/my-project/locations/my-region/keyRings/my-kr/cryptoKeys/my-key`.
              The key needs to be in the same region as where the compute
              resource is created. If set, the key secures the training pipeline
              and overrides the key set using `aiplatform.init`. The model
              trained by this training pipeline is also secured by this key if
              `model_encryption_spec_key_name` is not set separately.
            model_encryption_spec_key_name (Optional[str]): Optional. The Cloud
              KMS resource identifier of the customer managed encryption key
              used to protect the model. The key has the following format:
              `projects/my-project/locations/my-region/keyRings/my-kr/cryptoKeys/my-key`.
              The key needs to be in the same region as where the compute
              resource is created. If set, the key secures the trained model and
              overrides the key set using `aiplatform.init`.
            ...
            metric_spec: Dict[str, str] Required. A dictionary that represents
              ...
              `metric_id` that's reported by your training job and can be 'loss'
              or 'accuracy'. The dictionary value is the optimization goal of
              the metric, and can be 'minimize' or 'maximize'. The following is
              an example of a `metric_spec`: `metric_spec = {'loss': 'minimize',
              'accuracy': 'maximize'}`. `metric_spec` is required for only Model
              Garden models when `model_type` is `EFFICIENTNET`, `VIT`, `COCA`,
              `SPINENET`, or `YOLO`.
            parameter_spec (Dict[str, hpt._ParameterSpec]): Required. A dictionary
              representing parameters to use for optimization. The dictionary key is the
              `metric_id` that's reported by your training job as a command line
              keyword argument. The dictionary value is the parameter
              specification of the metric. The following is an example of how to
              specify a `parameter_spec`:
            ...
            search_algorithm (str): The search algorithm that's specified for the study.
            ...
            measurement_selection (str): Indicates which measurement to use
              ...
              reported intermediate measurements. `measurement_selection` can be
              `best` or `last`. If you expect your measurements to monotonically
              improve, then choose `last`. If your system can over-train and you
              expect the performance to improve and then start to decline, then
              choose `best`. If your measurements are significantly noisy or not
              reproducible, then `best` is likely to be over-optimistic, so in
              this case `last` is the preferred option.
              `measurement_selection` is required for only Model Garden models
              when `model_type` is `EFFICIENTNET`, `VIT`, `COCA`, `SPINENET`, or
              `YOLO`.
