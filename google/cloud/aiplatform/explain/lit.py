...
class _VertexLitDataset(lit_dataset.Dataset):
  ...
  def __init__(
    ...
    """Construct a VertexLitDataset.
        ...
        Args:
            dataset: Required. A Pandas DataFrame that includes feature column
              names and data.
            column_types: Required. An OrderedDict of string names matching the
              columns of the dataset as the key, and the associated LitType of
              the column.
    """
...
class _EndpointLitModel(lit_model.Model):
  ...
  def __init__(
    ...
    """Construct a VertexLitModel.
        ...
        Args:
            model: Required. The name of the Endpoint resource. Format:
              ``projects/{project}/locations/{location}/endpoints/{endpoint}``
            input_types: Required. An OrderedDict of string names matching the
              features of the model as the key, and the associated LitType of
              the feature.
            output_types: Required. An OrderedDict of string names matching the
              labels of the model as the key, and the associated LitType of the
              label.
            model_id: Optional. A string of the specific model in the endpoint
              to create the LIT model from. If this is not set, any usable model
              in the endpoint is used to create the LIT model.
    ...
    if model_id:
      deployed_model = next(
          filter(lambda model: model.id == model_id,
                 self._endpoint.list_models()),
          None,
      )
      if not deployed_model:
        raise ValueError(
            "A model with id {model_id} was not found in the endpoint {endpoint}."
            .format(model_id=model_id, endpoint=endpoint))
  ...
  def predict_minibatch(
      self, inputs: List[lit_types.JsonDict]) -> List[lit_types.JsonDict]:
    """Retun predictions based on a batch of inputs.

        Args:
            inputs: Requred. a List of instances to predict on based on the
              input spec.
...
class _TensorFlowLitModel(lit_model.Model):
  ...
  def __init__(
    ...
    """Construct a VertexLitModel.
        ...
        Args:
            model: Required. A string reference to a local TensorFlow saved
              model directory. The model must have at most one input and one
              output tensor.
            input_types: Required. An OrderedDict of string names matching the
              features of the model as the key, and the associated LitType of
              the feature.
            output_types: Required. An OrderedDict of string names matching the
              labels of the model as the key, and the associated LitType of the
              label.
            attribution_method: Optional. A string to choose what attribution
              configuration to set up the explainer with. Valid options are
              'sampled_shapley' or 'integrated_gradients'.
    """
  ...
  def attribution_explainer(
      self,) -> Optional["AttributionExplainer"]:  # noqa: F821
  ...
  def _load_model(self, model: str):
    """Loads a TensorFlow saved model and populates the input and output signature attributes of the class.
        ...
        Args:
            model: Required. A string reference to a TensorFlow saved model
              directory.

        Raises:
            ValueError if the model has more than one input tensor or more than
            one output tensor.
        """
...
def create_lit_dataset(
  ...
  """Creates a LIT Dataset object.
    ...
    Args:
        dataset: Required. A Pandas DataFrame that includes feature column names
          and data.
        column_types: Required. An OrderedDict of string names matching the
          columns of the dataset as the key, and the associated LitType of the
          column.
...
def create_lit_model_from_endpoint(
  ...
  """Creates a LIT Model object.
    ...
    Args:
        model: Required. The name of the Endpoint resource or an Endpoint
          instance. Endpoint name format:
          ``projects/{project}/locations/{location}/endpoints/{endpoint}``
        input_types: Required. An OrderedDict of string names matching the
          features of the model as the key, and the associated LitType of the
          feature.
        output_types: Required. An OrderedDict of string names matching the
          labels of the model as the key, and the associated LitType of the
          label.
        model_id: Optional. A string of the specific model in the endpoint to
          create the LIT model from. If this is not set, any usable model in the
          endpoint is used to create the LIT model.

    Returns:
        A LIT Model object that has the same functionality as the model
        provided.
    """
...
def create_lit_model(
  ...
  """Creates a LIT Model object.
    ...
    Args:
        model: Required. A string reference to a local TensorFlow saved model
          directory. The model must have at most one input and one output
          tensor.
        input_types: Required. An OrderedDict of string names matching the
          features of the model as the key, and the associated LitType of the
          feature.
    ...
    Returns:
        ...
        provided.
    """
  return _TensorFlowLitModel(model, input_types, output_types,
                             attribution_method)
...
def open_lit(
  ...
  """Open LIT from the provided models and datasets.
    ...
    Args:
        models: Required. A list of LIT models to open LIT with.
        input_types: Required. A lit of LIT datasets to open LIT with.
        open_in_new_tab: Optional. A boolean to choose if LIT open in a new tab
          or not.
