...
class SavedModelMetadataBuilder(metadata_builder.MetadataBuilder):
  ...
  def __init__(
    ...
    """Initializes a SavedModelMetadataBuilder object.
        ...
        Args:
          model_path: Required. Local or GCS path to load the saved model from.
          tags: Optional. Tags to identify the model graph. If None or empty,
            TensorFlow's default serving tag will be used.
          signature_name: Optional. Name of the signature to be explained.
            Inputs and outputs of this signature will be written in the
            metadata. If not provided, the default signature will be used.
          outputs_to_explain: Optional. List of output names to explain. Only
            single output is supported for now. Hence, the list should contain
            one element. This parameter is required if the model signature
            (provided via signature_name) specifies multiple outputs.
...
def _create_input_metadata_from_signature(
  ...
  """Creates InputMetadata from signature inputs.
    ...
    Args:
      signature_inputs: Required. Inputs of the signature to be explained. If
        not provided, the default signature will be used.
...
def _create_output_metadata_from_signature(
  ...
  """Creates OutputMetadata from signature inputs.
    ...
    Args:
      signature_outputs: Required. Inputs of the signature to be explained. If
        not provided, the default signature will be used.
      output_to_explain: Optional. Output name to explain.
