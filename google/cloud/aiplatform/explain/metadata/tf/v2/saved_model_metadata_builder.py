...
class SavedModelMetadataBuilder(metadata_builder.MetadataBuilder):
  ...
  def _infer_metadata_entries_from_model(
  ...
  ) -> Tuple[Dict[str, explanation_metadata.ExplanationMetadata.InputMetadata],
             Dict[str,
                  explanation_metadata.ExplanationMetadata.OutputMetadata],]:
    """Infers metadata inputs and outputs.

        Args:
          signature_name: Required. Name of the signature to be explained.
            Inputs and outputs of this signature will be written in the
            metadata. If not provided, the default signature will be used.
    ...
    for name in output_sig:
      if not self._explain_output or self._explain_output[0] == name:
        output_mds[
            name] = explanation_metadata.ExplanationMetadata.OutputMetadata(
