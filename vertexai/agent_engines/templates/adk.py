...
class AdkApp:
    ...
    async def _convert_response_events(
        ...
        result = _StreamingRunResponse(events=events, artifacts=[], session_id=session_id)
    ...
    async def streaming_agent_run_with_events(self, request_json: str):
        ...
        try:
            async for event in runner.run_async(
                ...
                _LOGGER.info("converted_event: %s", converted_event)
            ...
            if session and not request.session_id:
                ...
                _LOGGER.info("Deleted session: %s", session)
