...
class TuningJob(jobs._Job):
    ...
    def __init__(
        ...
        super().__init__(job_name=tuning_job_name)
    ...
    @property
    def experiment(self) -> aiplatform.Experiment:
        if self._experiment_name:
            return aiplatform.Experiment(experiment_name=self._experiment_name)

    @property
    def _tuned_model_name(self) -> Optional[str]:
        return self._gca_resource.tuned_model.model

    @property
    def tuned_model(self) -> Optional[aiplatform.Model]:
        if self._tuned_model_name:
            return aiplatform.Model(model_name=self._tuned_model_name)

    @property
    def _tuned_model_endpoint_name(self) -> Optional[str]:
        return self._gca_resource.tuned_model.endpoint

    @property
    def tuned_model_endpoint(self) -> Optional[aiplatform.Endpoint]:
        if self._tuned_model_endpoint_name:
            return aiplatform.Endpoint(endpoint_name=self._tuned_model_endpoint_name)
    ...
    def submit(
        ...
        *,
        ...
        api_client = aiplatform_initializer.global_config.create_client(
        ...
        created_gca_tuning_job = api_client.create_tuning_job(
            parent=parent,
        ...
        tuning_job: TuningJob = cls._construct_sdk_resource_from_gapic(created_gca_tuning_job)
