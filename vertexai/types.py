...
`isinstance` and does not type check, even though both describe the same
message.
...
if _typing.TYPE_CHECKING:
    # Read statically and never executed, so every name below is a real class
    # to a type checker instead of `__getattr__`'s return type. `agentplatform`
    # comes second so it wins the names both modules define, which is the order
    # `__getattr__` applies at runtime.
    from vertexai._genai.types import *  # noqa: F401,F403
    from agentplatform._genai.types import *  # noqa: F401,F403
