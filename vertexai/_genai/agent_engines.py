...
def _CheckQueryJobResult_from_vertex(
    ...
    if getv(parent_object, ["inputGcsUri"]) is not None:
        setv(to_object, ["input_gcs_uri"], getv(parent_object, ["inputGcsUri"]))
...
class AgentEngines(_api_module.BaseModule):
    ...
    def check_query_job(
        ...
        *,
        ...
        try:
            operation = self._get_agent_operation(operation_name=name)
        except Exception as e:
            raise ValueError(f"Failed to retrieve operation {name}: {e}")

        status = "RUNNING"
        if operation.done:
            status = "FAILED" if operation.error else "SUCCESS"

        output_gcs_uri = None
        input_gcs_uri = None

        print(f"Operation={operation}")
        print(f"DEBUG CheckQueryJob: Status={status}")
        print(f"DEBUG CheckQueryJob metadata type: {type(operation.metadata)}")
        print(
            f"DEBUG CheckQueryJob metadata dir: {dir(operation.metadata) if operation.metadata else None}"
        )

        if operation.metadata:
            if isinstance(operation.metadata, dict):
                output_gcs_uri = operation.metadata.get("output_gcs_uri")
                input_gcs_uri = operation.metadata.get("input_gcs_uri")
                print(f"DEBUG CheckQueryJob dict metadata: {operation.metadata}")
            else:
                output_gcs_uri = getattr(operation.metadata, "output_gcs_uri", None)
                input_gcs_uri = getattr(operation.metadata, "input_gcs_uri", None)
                print(
                    f"DEBUG CheckQueryJob object metadata: output_gcs_uri={output_gcs_uri}, input_gcs_uri={input_gcs_uri}"
                )

        print(f"DEBUG CheckQueryJob response type: {type(operation.response)}")
        print(
            f"DEBUG CheckQueryJob response dir: {dir(operation.response) if operation.response else None}"
        )

        if not output_gcs_uri and operation.response:
            if isinstance(operation.response, dict):
                output_gcs_uri = output_gcs_uri or operation.response.get(
                    "output_gcs_uri"
                )
                input_gcs_uri = input_gcs_uri or operation.response.get("input_gcs_uri")
                print(f"DEBUG CheckQueryJob dict response: {operation.response}")
            else:
                output_gcs_uri = output_gcs_uri or getattr(
                    operation.response, "output_gcs_uri", None
                )
                input_gcs_uri = input_gcs_uri or getattr(
                    operation.response, "input_gcs_uri", None
                )
                print(
                    f"DEBUG CheckQueryJob object response: output_gcs_uri={output_gcs_uri}, input_gcs_uri={input_gcs_uri}"
                )

        print(
            f"DEBUG CheckQueryJob resolved URIs: input={input_gcs_uri}, output={output_gcs_uri}"
        )
        ...
        elif status == "FAILED" and operation.error:
            result_str = str(operation.error)
    ...
    def run_query_job(
        ...
        *,
        ...
        is_supported = False
        print(f"DEBUG api_resource: api_resource={api_resource}")
        ...
        try:
            print(f"DEBUG bucket: bucket={bucket}")
        ...
        if not bucket_exists:
            try:
                print(f"DEBUG create bucket: bucket={bucket}")
                bucket.create()
            except exceptions.Forbidden as e:
                raise ValueError(
        ...
        print(f"DEBUG uploading to bucket: bucket={blob}")
        ...
        print(f"DEBUG triggering _run_query_job with config: {new_config}")
