...
class PromptManagement(_api_module.BaseModule):
    ...
    def list_prompts(
        ...
        *,
        config: Optional[types.ListPromptsConfigOrDict] = None,
    ) -> Iterator[types.Prompt]:
        """Lists prompt resources in a project.

        This method retrieves all the prompts from the project provided in the
        vertexai.Client constructor. By default, the returned types.Prompt objects
        will only have the prompt.prompt_data.model, prompt.prompt_id, and prompt.dataset fields set.

        If config.return_full_prompt_resources is True, then the returned
        types.Prompt objects will have the prompt contents, variables, dataset, and dataset version fields set.
        If setting config.return_full_prompt_resources to True, it is recommended to use a filter to reduce the latency of this request.

        Args:
          config: Optional configuration for listing prompts.

        Returns:
            An interable of types.Prompt objects representing the prompt resources from this project.
        """
        if isinstance(config, dict):
        ...
        for dataset in self._list_prompts_pager(config=config):
            ...
            if config and config.return_full_prompt_resources:
                prompt_dataset_resource = self._get_dataset_resource(
                ...
                prompt = types.Prompt(
                    prompt_data=types.SchemaPromptSpecPromptMessage(
                        model=dataset.model_reference
                    )
                )
    ...
    def list_versions(
        ...
        *,
        ...
        This method retrieves all the prompt versions for a provided prompt_id. By default, the returned types.Prompt objects
        will only have the prompt.prompt_data.model, prompt.prompt_id, and prompt.dataset_version fields set.
        ...
        Returns:
            An interable of types.Prompt objects representing the prompt version resources for the provided prompt_id.
        ...
        for dataset_version in self._list_versions_pager(
            ...
            prompt = types.Prompt()
            if config and config.return_full_prompt_resources:
                prompt_dataset_version_resource = self._get_dataset_version_resource(
                    dataset_id=prompt_id,
                    dataset_version_id=dataset_version.name.split("/")[-1],
                )
                prompt = _prompt_management_utils._create_prompt_from_dataset_metadata(
                    prompt_dataset_version_resource,
                )
                prompt._dataset_version = prompt_dataset_version_resource
            else:
                prompt = types.Prompt(
                    prompt_data=types.SchemaPromptSpecPromptMessage(
                ...
                prompt._dataset_version = dataset_version
            yield prompt
