...
class PromptManagement(_api_module.BaseModule):
    ...
    def _wait_for_operation(
        ...
        if (
            ...
            if response_body.get("error"):
                raise ValueError(
                    "Error creating prompt version resource:"
                    f" {response_body['error']}"
                )
            if not response_body.get("response") or not response_body.get(
                "response"
            ).get("name"):
                raise ValueError("Error creating prompt version resource.")
    ...
    def get(
        ...
        *,
        ...
        Args:
          prompt_id: The id of the Vertex Dataset resource containing the
            prompt. For example, if the prompt resource name is
            "projects/123/locations/us-central1/datasets/456", then the
            prompt_id is "456".
