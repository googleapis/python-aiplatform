...
class MetricPromptBuilder(PromptTemplate):
    ...
    def _prepare_fields_and_construct_text(cls, data: Any) -> Any:
        ...
        if "criteria" not in data or "rating_scores" not in data:
            raise ValueError(
        ...
        instruction = data.get(
            "instruction", cls.model_fields["instruction"].default_factory()
        )
        metric_definition = data.get("metric_definition")
        evaluation_steps = data.get(
            "evaluation_steps",
            cls.model_fields["evaluation_steps"].default_factory(),
        )
