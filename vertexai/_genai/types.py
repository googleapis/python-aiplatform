...
class Prompt(_common.BaseModel):
    ...
    def assemble_contents(self) -> genai_types.Content:
        """Transforms a Prompt object into a list of Part objects by substituting variables.

        This function processes the `prompt_data` from a Prompt object. For any
        text-based Part, it finds placeholders like `{variable_name}` and
        replaces
        them with the corresponding values from the `variables` field for each
        variable set. The values can be strings or Part objects. If the variable
        is
        a Part with inlineData or fileData, it appends the Part to the result
        list
        without changing the variable in the template.

        Args:
            prompt: The Prompt object to parse.

        Returns:
            A list of Part objects with variables substituted into the text.
            Non-text parts are passed through unchanged for each variable set.
        """
        if not self.prompt_data or not self.variables:
            return []
        ...
        return genai_types.Content(parts=all_rendered_parts)
