    ...
    def _build_pointwise_input(
        ...
        for key, value in instance_data.items():
            if isinstance(value, genai_types.Content):
                content_map_values[key] = [value]
            elif isinstance(value, types.ResponseCandidate):
                if value.response:
                    content_map_values[key] = [value.response]
            elif (
                isinstance(value, list)
                and value
                and isinstance(value[0], genai_types.Content)
            ):
                content_map_values[key] = value
            elif (
                isinstance(value, list)
                and value
                and isinstance(value[0], types.Message)
            ):
                history_texts = []
                for msg_obj in value:
                    if msg_obj.content and msg_obj.content.parts:
                        # Assuming single text part for history messages
                        msg_text = msg_obj.content.parts[0].text
                        if msg_text:
                            role = msg_obj.content.role or msg_obj.author or "user"
                            history_texts.append(f"{role}: {msg_text}")
                content_map_values[key] = [
