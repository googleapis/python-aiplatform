...
def _to_function_calls(
    ...
    if isinstance(contents, str):
        ...
        for content in gapic_contents:
            yield from [
                part.function_call for part in content.parts if "function_call" in part
            ]
...
class _TextsAccumulator:
    ...
    def add_function_call(self, function_call: gapic_tool_types.FunctionCall) -> None:
        self.texts.append(function_call.name)
    ...
    def add_function_response(
        ...
        self.texts.append(function_response.name)
    ...
    def _function_declaration_traverse(
        ...
        self.texts.append(function_declaration.name)
        if function_declaration.description:
            self.texts.append(function_declaration.description)
        if function_declaration.parameters:
            self._schema_traverse(function_declaration.parameters)

    def _schema_traverse(self, schema: openapi.Schema) -> None:
        if schema.format:
            self.texts.append(schema.format)
        if schema.description:
            self.texts.append(schema.description)
        if schema.enum:
            self.texts.extend(schema.enum)
        if schema.required:
            self.texts.extend(schema.required)
        if schema.items:
    ...
    def _struct_traverse(self, struct: struct_pb2.Struct) -> None:
        self.texts.extend(list(struct.keys()))
...
class Tokenizer:
    ...
    def count_tokens(
        ...
        *,
        ...
        text_accumulator.texts.extend(_to_canonical_contents_texts(contents))
        ...
        return self._sentencepiece_adapter.count_tokens(text_accumulator.texts)
