...
class _GetTunedModelMixin(_model_garden_models._ModelGardenModel):
  ...
  def list_tuned_model_names(self) -> Sequence[str]:
    ...
    Returns:
        A list of tuned models that can be used with the `get_tuned_model`
        method.
    """
  ...
  def get_tuned_model(cls, tuned_model_name: str) -> "_LanguageModel":
    ...
    if _TUNING_BASE_MODEL_ID_LABEL_KEY not in tuned_model_labels:
      raise ValueError(
          f"The provided model {tuned_model_name} does not have a base"
          " model ID."
      )
...
class _TunableModelMixin(_LanguageModel, _GetTunedModelMixin):
  ...
  def tune_model(  #
      ...
      *,
      corpus_data: Optional[
          Union[str, "pandas.core.frame.DataFrame"]
      ] = None,  ###
      queries_data: Optional[
          Union[str, "pandas.core.frame.DataFrame"]
      ] = None,  ###
      test_data: Union[str, "pandas.core.frame.DataFrame"] = None,  ###
      validation_data: Union[str, "pandas.core.frame.DataFrame"] = None,  ###
      batch_size: Optional[int] = None,  ###
      train_steps: Optional[int] = None,
      learning_rate: Optional[float] = None,
      learning_rate_multiplier: Optional[float] = None,
      tuning_job_location: Optional[
          str
      ] = None,  ### to be supported for embedding models.
    ...
    if eval_spec is not None:
      ...
      tensorboard_resource_id = (
          _get_tensorboard_resource_id_from_evaluation_spec(
  ...
  def _tune_model(
      self,
      training_data: Union[str, "pandas.core.frame.DataFrame"],
      corpus_data: Union[str, "pandas.core.frame.DataFrame"],
      queries_data: Union[str, "pandas.core.frame.DataFrame"],
      test_data: Union[str, "pandas.core.frame.DataFrame"],
      validation_data: Union[str, "pandas.core.frame.DataFrame"],
      *,
    ...
    if tuned_model_location not in _TUNED_MODEL_LOCATIONS:
      raise ValueError(
          "Tuned model deployment is only supported in the following"
          f" locations: {_TUNED_MODEL_LOCATIONS}"
      )
    model_info = _model_garden_models._get_model_info(
        model_id=self._model_id,
        schema_to_class_map={self._INSTANCE_SCHEMA_URI: type(self)},
...
class _RlhfTunableModelMixin(_LanguageModel, _GetTunedModelMixin):
  ...
  def tune_model_rlhf(
    ...
    ... do some other work
    tuned_model = tuning_job.get_tuned_model()  # Blocks until tuning is
    complete
    ```

    Args:
        prompt_data: A Pandas DataFrame or a URI pointing to data in JSON lines
          format. The dataset schema is model-specific. See
          https://cloud.google.com/vertex-ai/docs/generative-ai/models/tune-text-models-rlhf#prompt-dataset
        preference_data: A Pandas DataFrame or a URI pointing to data in JSON
          lines format. The dataset schema is model-specific. See
          https://cloud.google.com/vertex-ai/docs/generative-ai/models/tune-text-models-rlhf#human-preference-dataset
        model_display_name: Custom display name for the tuned model. If not
          provided, a default name will be created.
        prompt_sequence_length: Maximum tokenized sequence length for input
          text. Higher values increase memory overhead. This value should be at
          most 8192. Default value is 512.
        target_sequence_length: Maximum tokenized sequence length for target
          text. Higher values increase memory overhead. This value should be at
          most 1024. Default value is 64.
        reward_model_learning_rate_multiplier: Constant used to adjust the base
          learning rate used when training a reward model. Multiply by a number
          > 1 to increase the magnitude of updates applied at each training step
          or multiply by a number < 1 to decrease the magnitude of updates.
          Default value is 1.0.
        reinforcement_learning_rate_multiplier: Constant used to adjust the base
          learning rate used during reinforcement learning. Multiply by a number
          > 1 to increase the magnitude of updates applied at each training step
          or multiply by a number < 1 to decrease the magnitude of updates.
          Default value is 1.0.
        reward_model_train_steps: Number of steps to use when training a reward
          model. Default value is 1000.
        reinforcement_learning_train_steps: Number of reinforcement learning
          steps to perform when tuning a base model. Default value is 1000.
        kl_coeff: Coefficient for KL penalty. This regularizes the policy model
          and penalizes if it diverges from its initial distribution. If set to
          0, the reference language model is not loaded into memory. Default
          value is 0.1.
    ...
    Returns:
        ...
        Calling `job.result()` blocks until the tuning is complete and returns a
...
class _TunableTextModelMixin(_TunableModelMixin):
  ...
  def tune_model(
    ...
    Args:
        training_data: A Pandas DataFrame or a URI pointing to data in JSON
          lines format. The dataset schema is model-specific. See
          https://cloud.google.com/vertex-ai/docs/generative-ai/models/tune-models#dataset_format
        train_steps: Number of training batches to tune on (batch size is 8
          samples).
        learning_rate_multiplier: Learning rate multiplier to use in tuning.
        tuning_job_location: GCP location where the tuning job should be run.
        tuned_model_location: GCP location where the tuned model should be
          deployed.
        model_display_name: Custom display name for the tuned model.
        tuning_evaluation_spec: Specification for the model evaluation during
          tuning.
        accelerator_type: Type of accelerator to use. Can be "TPU" or "GPU".
        max_context_length: The max context length used for tuning. Can be
          either '8k' or '32k'
...
class _TunableChatModelMixin(_TunableModelMixin):
  ...
  def tune_model(
    ...
    Raises:
        ...
        ValueError: If the "tuned_model_location" value is not supported
        RuntimeError: If the model does not support tuning
        AttributeError: If any attribute in the "tuning_evaluation_spec" is not
        supported
    """
    ...
    if tuning_evaluation_spec is not None:
      unsupported_chat_model_tuning_eval_spec = {
          ...
          "enable_checkpoint_selection": (
              tuning_evaluation_spec.enable_checkpoint_selection
          ),
      }
      ...
      for (
          att_name,
          att_value,
      ) in unsupported_chat_model_tuning_eval_spec.items():
        if att_value is not None:
          raise AttributeError((
              "ChatModel and CodeChatModel only support tensorboard as"
              " attribute for TuningEvaluationSpecfound attribute name"
              f" {att_name} with value {att_value}, please leave {att_name} to"
              " None"
          ))
    return super().tune_model(
        training_data=training_data,
...
class CountTokensResponse:
  ...
  Attributes:
      total_tokens (int): The total number of tokens counted across all
        instances passed to the request.
      total_billable_characters (int): The total number of billable characters
...
class _CountTokensMixin(_LanguageModel):
  ...
  def count_tokens(
    ...
    Note: this does not make a prediction request to the model, it only counts
    the tokens
    in the request.

    Args:
        prompts (List[str]): Required. A list of prompts to ask the model. For
          example: ["What should I do today?", "How's it going?"]
...
class TuningEvaluationSpec:
  ...
  Attributes:
      evaluation_data: GCS URI of the evaluation dataset. This will run model
        evaluation as part of the tuning job.
      evaluation_interval: The evaluation will run at every evaluation_interval
        tuning steps. Default: 20.
      ...
      enable_checkpoint_selection: If set to True, the tuning process returns
        the best model checkpoint (based on model evaluation). If set to False,
        the latest model checkpoint is returned. If unset, the selection is only
        enabled for `*-bison@001` models.
      tensorboard: Vertex Tensorboard where to write the evaluation metrics. The
        Tensorboard must be in the same location as the tuning job.
  """
...
def _check_unused_rlhf_eval_specs(
...
class WebSearch(_GroundingSourceBase):
  ...
  Attributes:
      disable_attribution: If set to `True`, skip finding claim attributions
        (i.e not generate grounding citation). Default: False.
  """
...
class VertexAISearch(_GroundingSourceBase):
  ...
  Attributes:
      ...
      location: GCP multi region where you have set up your Vertex AI Search
        data store. Possible values can be `global`, `us`, `eu`, etc. Learn more
        about Vertex AI Search location here:
          https://cloud.google.com/generative-ai-app-builder/docs/locations
      project: The project where you have set up your Vertex AI Search. If not
        specified, will assume that your Vertex AI Search is within your current
        project.
      disable_attribution: If set to `True`, skip finding claim attributions
        (i.e not generate grounding citation). Default: False.
  """
  ...
  _type: str = dataclasses.field(
      default="VERTEX_AI_SEARCH", init=False, repr=False
  )
...
class GroundingCitation:
  ...
  Attributes:
      ...
      license: License associated with this citation. If present, it refers to
        the license of the source of this citation. Possible licenses include
        code licenses, e.g., mit license.
      publication_date: Publication date associated with this citation. If
...
class TextGenerationResponse:
  ...
  Attributes:
      ...
      errors: The error codes indicate why the response was blocked. Learn more
        information about safety errors here: this documentation
        https://cloud.google.com/vertex-ai/docs/generative-ai/learn/responsible-ai#safety_errors
      safety_attributes: Scores for safety attributes. Learn more about the
        safety attributes here:
...
class MultiCandidateTextGenerationResponse(TextGenerationResponse):
  ...
  Attributes:
      ...
      candidates: The candidate responses. Usually contains a single candidate
        unless `candidate_count` is used.
  """

  __module__ = "vertexai.language_models"

  candidates: List[TextGenerationResponse] = dataclasses.field(
...
class _TextGenerationModel(_LanguageModel):
  ...
  def predict(
    ...
    Args:
        ...
        max_output_tokens: Max length of the output text in tokens. Range: [1,
          1024].
        temperature: Controls the randomness of predictions. Range: [0, 1].
          Default: 0.
        top_k: The number of highest probability vocabulary tokens to keep for
          top-k-filtering. Range: [1, 40]. Default: 40.
        top_p: The cumulative probability of parameter highest probability
          vocabulary tokens to keep for nucleus sampling. Range: [0, 1].
          Default: 0.95.
        ...
        grounding_source: If specified, grounding feature will be enabled using
          the grounding source. Default: None.
        logprobs: Returns the top `logprobs` most likely candidate tokens with
          their log probabilities at each generation step. The chosen tokens and
          their log probabilities at each step are always returned. The chosen
          token may or may not be in the top `logprobs` most likely candidates.
          The minimum value for `logprobs` is 0, which means only the chosen
          tokens and their log probabilities are returned. The maximum value for
          `logprobs` is 5.
        presence_penalty: Positive values penalize tokens that have appeared in
          the generated text, thus increasing the possibility of generating more
          diversed topics.
            Range: [-2.0, 2.0]
        frequency_penalty: Positive values penalize tokens that repeatedly
          appear in the generated text, thus decreasing the possibility of
          repeating the same content.
            Range: [-2.0, 2.0]
        logit_bias: Mapping from token IDs (integers) to their bias values
          (floats). The bias values are added to the logits before sampling.
    ...
    Returns:
        A `MultiCandidateTextGenerationResponse` object that contains the text
...
def _create_text_generation_prediction_request(
  ...
  Args:
      ...
      top_k: The number of highest probability vocabulary tokens to keep for
        top-k-filtering. Range: [1, 40]. Default: 40.
      top_p: The cumulative probability of parameter highest probability
        vocabulary tokens to keep for nucleus sampling. Range: [0, 1]. Default:
        0.95.
      stop_sequences: Customized stop sequences to stop the decoding process.
      ...
      presence_penalty: Positive values penalize tokens that have appeared in
        ...
        diversed topics.
          Range: [-2.0, 2.0]
      frequency_penalty: Positive values penalize tokens that repeatedly appear
        in the generated text, thus decreasing the possibility of repeating the
        same content.
          Range: [-2.0, 2.0]
      logit_bias: Mapping from token IDs (integers) to their bias values
        (floats). The bias values are added to the logits before sampling.
  ...
  Returns:
      A `_PredictionRequest` object that contains prediction instance and
      parameters.
  """
...
class _ModelWithBatchPredict(_LanguageModel):
  ...
  def batch_predict(
    ...
    Args:
        dataset: The location of the dataset. `gs://` and `bq://` URIs are
          supported.
        destination_uri_prefix: The URI prefix for the prediction. `gs://` and
          `bq://` URIs are supported.
...
class _PreviewModelWithBatchPredict(_ModelWithBatchPredict):
  ...
  def batch_predict(
    ...
    Args:
        dataset: Required. The location of the dataset. `gs://` and `bq://` URIs
          are supported.
        destination_uri_prefix: The URI prefix for the prediction. `gs://` and
          `bq://` URIs are supported.
        model_parameters: Model-specific parameters to send to the model.
...
class _ChatSession:
  ...
  Within a chat session, the model keeps context and remembers the previous
  conversation.
  """
  ...
  def send_message(
    ...
    Args:
        ...
        max_output_tokens: Max length of the output text in tokens. Range: [1,
          1024]. Uses the value specified when calling `ChatModel.start_chat` by
          default.
        temperature: Controls the randomness of predictions. Range: [0, 1].
          Default: 0. Uses the value specified when calling
            `ChatModel.start_chat` by default.
        top_k: The number of highest probability vocabulary tokens to keep for
          top-k-filtering. Range: [1, 40]. Default: 40. Uses the value specified
          when calling `ChatModel.start_chat` by default.
        top_p: The cumulative probability of parameter highest probability
          vocabulary tokens to keep for nucleus sampling. Range: [0, 1].
          Default: 0.95. Uses the value specified when calling
            `ChatModel.start_chat` by default.

    Returns:
        A `TextGenerationResponse` object that contains the text produced by the
        model.
    """
    new_history_text = ""
...
class TextEmbeddingInput:
  ...
  Attributes:
      ...
      task_type: The name of the downstream task the embeddings will be used
        for. Valid values: RETRIEVAL_QUERY Specifies the given text is a query
        in a search/retrieval setting. RETRIEVAL_DOCUMENT Specifies the given
        text is a document from the corpus being searched. SEMANTIC_SIMILARITY
        Specifies the given text will be used for STS. CLASSIFICATION Specifies
        that the given text will be classified. CLUSTERING Specifies that the
        embeddings will be used for clustering. QUESTION_ANSWERING Specifies
        that the embeddings will be used for question answering.
...
class _TextEmbeddingModel(_LanguageModel):
  ...
  def _prepare_text_embedding_request(
    ...
    Args: texts(str): A list of texts or `TextEmbeddingInput` objects to embed.
    auto_truncate(bool): Whether to automatically truncate long texts. Default:
    True.
        output_dimensionality: Optional dimensions of embeddings. Range: [1,
        768]. Default: None.
  ...
  def get_embeddings(
    ...
    Args:
        ...
        auto_truncate: Whether to automatically truncate long texts. Default:
          True.
        output_dimensionality: Optional dimensions of embeddings. Range: [1,
          768]. Default: None.
...
class _TunableEmbeddingModelMixin(_TunableModelMixin):
  ...
  def tune_model(
    ...
    return super().tune_model(
        ...
        train_steps=train_steps,
        # tuning_job_location=tuned_model_location,
        tuned_model_location=tuned_model_location,
        model_display_name=model_display_name,
...
class _ChatModelBase(_LanguageModel):
  ...
  def start_chat(
    ...
    Args:
        context: Context shapes how the model responds throughout the
          conversation. For example, you can use context to specify words the
          model can or cannot use, topics to focus on or avoid, or the response
          format or style
        examples: List of structured messages to the model to learn how to
          respond to the conversation. A list of `InputOutputTextPair` objects.
        max_output_tokens: Max length of the output text in tokens. Range: [1,
          1024].
        temperature: Controls the randomness of predictions. Range: [0, 1].
          Default: 0.
...
class ChatModel(_ChatModelBase, _TunableChatModelMixin, _RlhfTunableModelMixin):
  ...
  Examples::
      ...
      chat = chat_model.start_chat(
          context="My name is Ned. You are my personal assistant. My favorite
          movies are Lord of the Rings and Hobbit.",
          examples=[
              InputOutputTextPair(
...
class CodeChatModel(_ChatModelBase, _TunableChatModelMixin):
  ...
  Examples:
      ...
      code_chat.send_message("Please help write a function to calculate the min
      of two numbers")
  """
    ...
    Args:
        ...
        top_p: The cumulative probability of parameter highest probability
          ...
          Default: 0.95. Uses the value specified when calling
            `ChatModel.start_chat` by default.
        stop_sequences: Customized stop sequences to stop the decoding process.
          Uses the value specified when calling `ChatModel.start_chat` by
          default.
...
class _ChatSessionBaseWithCountTokensMixin(_ChatSessionBase):
  ...
  def count_tokens(
    ...
    If you've called `send_message()` in the current chat session before calling
    `count_tokens()`, the
    response will include the total tokens and characters for the previously
    sent message and the one in the
    `count_tokens()` request. To count the tokens for a single message, call
    `count_tokens()` right after
    ...
    Args:
        message (str): Required. A chat message to count tokens or. For example:
          "How's it going?"
    ...
    count_tokens_response = (
        self._model._endpoint._prediction_client.select_version(
            "v1beta1"
        ).count_tokens(
...
class CodeChatSession(_ChatSessionBase):
  ...
  Within a code chat session, the model keeps context and remembers the previous
  converstion.
  """
  ...
  def send_message(
    ...
    Args:
        message: Message to send to the model
        max_output_tokens: Max length of the output text in tokens. Range: [1,
          1000]. Uses the value specified when calling
          `CodeChatModel.start_chat` by default.
        temperature: Controls the randomness of predictions. Range: [0, 1]. Uses
          the value specified when calling `CodeChatModel.start_chat` by
          default.
        stop_sequences: Customized stop sequences to stop the decoding process.
        candidate_count: Number of candidates to return.
...
class _CodeGenerationModel(_LanguageModel):
  ...
  def _create_prediction_request(
    ...
    return _PredictionRequest(
        instance=instance, parameters=prediction_parameters
    )
...
class _LanguageModelTuningJob:
  ...
  def get_tuned_model(self) -> "_LanguageModel":
    ...
    if not model_task:
      raise RuntimeError(
          "Failed to get the model name from the tuning pipeline:"
          f" {self._job.name}"
      )
    ...
    _LOGGER.info(
        f"Tuning has completed. Created Vertex Model: {vertex_model_name}"
    )
...
def _get_tuned_models_dir_uri(model_id: str) -> str:
  ...
  return (
      staging_gcs_bucket.replace(
          "/output_artifacts/", "/tuned_language_models/"
      )
      + model_id
  )
...
def _list_tuned_model_names(model_id: str) -> List[str]:
  tuned_models = aiplatform.Model.list(
      filter=(
          f'labels.{_TUNING_BASE_MODEL_ID_LABEL_KEY}="{model_id.replace("@", "-")}"'
      ),
...
def _maybe_upload_training_data(
  ...
  elif isinstance(training_data, pandas.core.frame.DataFrame):
...
def _launch_tuning_job(
  ...
  corpus_path = _maybe_upload_training_data(
      training_data=corpus_data, model_id=model_id
  )
  queries_path = _maybe_upload_training_data(
      training_data=queries_data, model_id=model_id
  )
  test_data_path = _maybe_upload_training_data(
      training_data=test_data, model_id=model_id
  )
  validation_data_path = _maybe_upload_training_data(
      training_data=validation_data, model_id=model_id
  )
  ...
  if corpus_path and queries_path and training_data_path:
    pipeline_arguments["training_label_path"] = training_data_path
