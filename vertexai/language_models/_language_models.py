...
class _TunableTextEmbeddingModelMixin(_TunableModelMixin):
    ...
    def _bundle_up_tuning_job(self, pipeline_job):
        return _EmbeddingLanguageModelTuningJob(
            base_model=self,
            job=pipeline_job,
        )
...
class _PreviewTextEmbeddingModel(
    TextEmbeddingModel, _ModelWithBatchPredict, _CountTokensMixin, _TunableTextEmbeddingModelMixin
):
...
class _EmbeddingLanguageModelTuningJob(_LanguageModelTuningJob):
    """_EmbeddingLanguageModelTuningJob represents a tuning job."""
    ...
    def deploy_tuned_model(
        ...
        vertex_model_name = self._get_tuned_model_name()
        _LOGGER.info(f"Tuning has completed. Created Vertex Model: {vertex_model_name}")
        self._model = _PreviewTextEmbeddingModel.deploy_tuned_model(
            tuned_model_name=vertex_model_name,
            machine_type=machine_type,
            accelerator=accelerator,
            accelerator_count=accelerator_count,
