...
class ChatSession:
    ...
    def _send_message(
        ...
        *,
        ...
        message_responder = self._responder._create_responder_for_message(
            tools=tools or self._model._tools
        ) if self._responder else None
        ...
        while True:
            ...
            auto_responder_content = message_responder.respond_to_model_response(response) if message_responder else None
...
class FunctionDeclaration:
    ...
    @classmethod
    def from_func(cls, func: Callable) -> "CallableFunctionDeclaration":  # pylint:disable=g-bare-generic
        return CallableFunctionDeclaration.from_func(func)

    def to_dict(self) -> Dict[str, Any]:
        return type(self._raw_function_declaration).to_dict(self._raw_function_declaration)
...
class CallableFunctionDeclaration(FunctionDeclaration):
    ...
    def __init__(
        self,
        name: str,
        function: Callable,  # pylint:disable=g-bare-generic
        parameters: Dict[str, Any],
        description: Optional[str] = None,
    ):
        super().__init__(
    ...
    @classmethod
    def from_func(cls, func: Callable) -> "CallableFunctionDeclaration":  # pylint:disable=g-bare-generic
        from vertexai.generative_models import _function_calling_utils  # pylint: disable=g-import-not-at-top

        function_schema = (
            _function_calling_utils.generate_json_schema_from_function(func=func)
        )
...
class AutomaticFunctionCallingResponder:
    def __init__(self, max_number_of_automatic_function_calls: int = 1):
        if not (max_number_of_automatic_function_calls > 0):
            raise ValueError(
                "max_number_of_automatic_function_calls must be positive."
            )
        self._max_number_of_automatic_function_calls = max_number_of_automatic_function_calls
    ...
    def _create_responder_for_message(self, tools: List[Tool]) -> "_MessageResponder":
        return AutomaticFunctionCallingResponder._MessageResponder(
            tools=tools,
            max_number_of_automatic_function_calls=self._max_number_of_automatic_function_calls,
        )

    class _MessageResponder:
        """AFC Responder for a single user message."""

        def __init__(
            self,
            *,
            tools: List[Tool],
            max_number_of_automatic_function_calls: int = 1,
            # Potential future parameters:
            # automatic_function_calls_exhausted_behavior: "error" | "return"
            # non_callable_function_behavior: "error" | "return"
            **_,
        ):
            self._tools = tools
            self._max_number_of_automatic_function_calls = max_number_of_automatic_function_calls
            self._remaining_function_calls = max_number_of_automatic_function_calls
        ...
        def respond_to_model_response(
            ...
            for function_call in function_calls:
                if self._remaining_function_calls > 0:
                    ...
                    raise RuntimeError(
                        f"Exceeded the maximum number of automatic function calls ({self._max_number_of_automatic_function_calls})."
                        " If more automatic function calls are needed, set `max_number_of_automatic_function_calls` to a higher number."
