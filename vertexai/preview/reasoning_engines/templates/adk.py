...
class AdkApp:
    ...
    async def bidi_stream_query(
        ...
        first_request_dict = json_format.MessageToDict(first_request)
        user_id = first_request_dict.get("user_id")
        if not user_id:
            raise ValueError("The first request must have a user_id.")

        session_id = first_request_dict.get("session_id")
        run_config = first_request_dict.get("run_config")
        first_live_request = first_request_dict.get("live_request")
        ...
        async def _forward_requests():
            while True:
                ...
                live_request = LiveRequest.model_validate(
